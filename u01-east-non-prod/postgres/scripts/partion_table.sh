...
is_writer=$(aws --region us-east-2 rds describe-db-clusters --db-cluster-identifier ${cluster_postgres} --query "DBInstances[0].ReadReplicaSourceDBInstanceIdentifier" --output text)
...
./run_postgres_partition.sh ${instance_name1} $db_name chk_instance_db_mode.sql "-v dbname=${db_name}" > ${logdir}/${instance_name}_${db_name}_chk_instance_db_mode.log
 if [ "`grep "WRITER" ${logdir}/${instance_name}_${db_name}_chk_instance_db_mode.log | wc -l`" -eq "2" ] ; then
    ...
    record_count=`tail -3 ${logdir}/${instance_name}_${db_name}_partition_table.txt |head -1 |tr -s ' ' ' '`
