...
def run_psql_password(db_host, db_name, db_user, target_user, old_password, new_password):
    try:
        child = pexpect.spawn(
            '/usr/bin/psql',
...
def drop_replication_slots(db_host, db_user, db_password):
    try:
        conn = psycopg2.connect(host=db_host, database='postgres', user=db_user, password=db_password, connect_timeout=10)
        conn.autocommit = True
        cur = conn.cursor()
...
def execute_sql_file(db_host, db_name, db_user, db_password, sql_file):
    try:
        cmd = [
            'psql',
            '-h', db_host,
...
def database_exists(db_host, db_user, db_password, db_name):
    try:
...
def main(prod_cluster_identifier, stage_cluster_identifier, db_name, new_db_name):
    ...
    password_file_path = '/u01/prvn/stg_refresh/db_password.txt'
    new_password_file_path = '/u01/prvn/stg_refresh/intuadmin_new_password.txt'
    sql_file = '/u01/prvn/stg_refresh/postgres_postgres.sql'
    appusers_file_path = '/u01/prvn/stg_refresh/appusers_password.txt'
    pitparmo_sql_file = '/u01/prvn/stg_refresh/pitparmo_postgres.sql'
    vacuum_script_path = '/u01/prvn/stg_refresh/vacuum/vacuum/vacuum_ppsp-ppd--new.sh'
    analyze_script_path = '/u01/prvn/stg_refresh/vacuum/analyze/ppsp-ppd-new.sh'
    pspadm_sql_file = '/u01/prvn/stg_refresh/pspadmowner_pitparmo.sql'
    ...
    intuadmin_password = new_passwords.get(db_user)
...
#        new_db_cluster_identifier = f"{stage_cluster_identifier.lower().replace('_', '-')}-new".strip('-')
        ...
        if current_password and intuadmin_password:
            try:
                run_psql_password(db_host, db_name, db_user, db_user, current_password, intuadmin_password)
                drop_replication_slots(db_host, db_user, intuadmin_password)
                kill_active_connections(db_host, db_name, db_user, intuadmin_password)
                rename_database(db_host, db_user, intuadmin_password, old_name=db_name, new_name=new_db_name)
                rename_success = True
            except Exception as e:
                print(f"Error during password reset/rename: {e}")

        if rename_success and database_exists(db_host, db_user, intuadmin_password, new_db_name):
            execute_sql_file(db_host, def_db_name, db_user, intuadmin_password, sql_file)
            manage_pspadm_owner(db_host, def_db_name, db_user, intuadmin_password, pspadm_password)
        else:
        ...
        manage_app_users(db_host, new_db_name, db_user, intuadmin_password, appusers_file_path)

        # STEP 4: Execute pitparmo_postgres.sql
        print("\n=== STEP 4: Executing pitparmo_postgres.sql ===")
        try:
            execute_sql_file(db_host, new_db_name, db_user, intuadmin_password, pitparmo_sql_file)
        except Exception as e:
        ...
        print("‚
 Tag 'org_awsbackup_local_0700UTC=7continuous' applied successfully.")

        # 6. Run VACUUM script if exists
        print("\n=== STEP 6: Running VACUUM script ===")
        if os.path.exists(vacuum_script_path):
          print("üßπ Running VACUUM script for all tables...")
        ...
        print(f"‚
 VACUUM completed successfully. Log saved to {log_path}")

        # 7. Run ANALYZE script if exists
        print("\n=== STEP 7: Running ANALYZE script ===")
       # if os.path.exists(analyze_script_path):
        #    print("üìä Running ANALYZE script for all tables...")
         #   subprocess.run(
 ...
 ANALYZE completed successfully.")
        #else:
         #   print(f"‚ö†Ô∏ Analyze script not found at {analyze_script_path}")
