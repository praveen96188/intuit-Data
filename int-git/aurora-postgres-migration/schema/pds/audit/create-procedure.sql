...
CREATE PROCEDURE ibobadm_pds.batch_fill_comp_col$batch_fill_ceep(IN p_batch_id DOUBLE PRECISION)
AS 
$BODY$
/* p_batch_id     PLS_INTEGER := 1; */
DECLARE
    v_sql CHARACTER VARYING(500);
    v_upd_sql CHARACTER VARYING(500);
    v_status CHARACTER VARYING(30) := 'DONE';
    t_seq IBOBADM_PDS.UNRESOLVED_TYPE [];
    tab_seq t_seq%TYPE;
    i RECORD;
BEGIN
    FOR i IN
    SELECT
        ...
        FROM pspadm.t_backfill_metadata
        WHERE batch_id = p_batch_id AND table_name = 'T_PCEEP' AND status IS NULL
    LOOP
        UPDATE pspadm.t_backfill_metadata
        SET status = 'P'
            WHERE partition_name = i.partition_name;
        /*
        ...
        COMMIT
        */
        v_sql := CONCAT_WS('', 'SELECT COMPANY_EVENT_EMAIL_PARAM_SEQ from PSPADM.T_PCEEP PARTITION (', i.partition_name, ')');
        /*
        ...
        EXECUTE IMMEDIATE v_sql BULK COLLECT INTO tab_seq
        */
        RAISE DEBUG USING MESSAGE = CONCAT_WS('', 'partition_name --> ', i.partition_name);

        FOR k IN array_lower(tab_seq, 1)..array_upper(tab_seq, 1) LOOP
            EXECUTE ('UPDATE pspadm.psp_company_event_email_param AS ceep SET ceep.company_fk = (SELECT ce.company_fk AS company FROM pspadm.psp_company_event AS ce, pspadm.psp_company_event_email AS cee WHERE cee.company_event_email_seq = ceep.company_event_email_fk AND cee.company_event_fk = ce.company_event_seq) WHERE ceep.company_event_email_param_seq = $1') USING tab_seq[k];
            /*
            [5035 - Severity CRITICAL - A transaction cannot be ended inside a block with exception handlers. Revise your code to try move transaction control on side of application.]
            COMMIT
            */
    ...
    EXCEPTION
        WHEN others THEN
            ...
            ROLLBACK
            */
            RAISE USING hint = -20701, message = CONCAT_WS('', 'Error in updating PSP_COMPANY_EVENT_EMAIL_PARAM table ', SQLERRM), detail = 'User-defined exception';
END;
$BODY$
LANGUAGE plpgsql;

CREATE PROCEDURE ibobadm_pds.batch_fill_comp_col$batch_fill_datl(IN p_batch_id DOUBLE PRECISION)
AS 
$BODY$
/* p_batch_id     PLS_INTEGER := 1; */
DECLARE
...
BEGIN
    ...
    SELECT
        table_name, partition_name
        FROM pspadm.t_backfill_metadata
        WHERE batch_id = p_batch_id AND table_name = 'T_PDATL' AND status IS NULL
    LOOP
        UPDATE pspadm.t_backfill_metadata
        SET status = 'P'
            WHERE partition_name = i.partition_name;
        ...
        COMMIT
        */
        v_sql := CONCAT_WS('', 'SELECT DISBURSE_ADVICE_TAX_LIAB_SEQ from PSPADM.T_PDATL PARTITION (', i.partition_name, ')');
        /*
        [5088 - Severity CRITICAL - PostgreSQL doesn't support the EXECUTE IMMEDIATE statement with BULK COLLECT. Perform a manual conversion.]
        EXECUTE IMMEDIATE v_sql BULK COLLECT INTO tab_seq
        */
        ...
        FOR k IN array_lower(tab_seq, 1)..array_upper(tab_seq, 1) LOOP
            EXECUTE ('UPDATE pspadm.psp_disburse_advice_tax_liab AS datl SET datl.company_fk = (SELECT da.company_fk FROM pspadm.psp_disburse_advice AS da WHERE datl.disburse_advice_fk = da.disburse_advice_seq) WHERE datl.disburse_advice_tax_liab_seq = $1') USING tab_seq[k];
            /*
            [5035 - Severity CRITICAL - A transaction cannot be ended inside a block with exception handlers. Revise your code to try move transaction control on side of application.]
            COMMIT
            */
    ...
    EXCEPTION
        WHEN others THEN
            ...
            ROLLBACK
            */
            RAISE USING hint = -20701, message = CONCAT_WS('', 'Error in updating PSP_DISBURSE_ADVICE_TAX_LIAB table ', SQLERRM), detail = 'User-defined exception';
END;
$BODY$
LANGUAGE plpgsql;

CREATE PROCEDURE ibobadm_pds.batch_fill_comp_col$batch_fill_ppi(IN p_batch_id DOUBLE PRECISION)
AS 
$BODY$
/* p_batch_id     PLS_INTEGER := 1; */
DECLARE
...
BEGIN
    ...
    SELECT
        table_name, partition_name
        FROM pspadm.t_backfill_metadata
        WHERE batch_id = p_batch_id AND table_name = 'T_PPPI' AND status IS NULL
    LOOP
        UPDATE pspadm.t_backfill_metadata
        SET status = 'P'
            WHERE partition_name = i.partition_name;
        ...
        COMMIT
        */
        v_sql := CONCAT_WS('', 'SELECT PSTUB_PAY_ITEM_SEQ from PSPADM.T_PPPI PARTITION (', i.partition_name, ')');
        /*
        [5088 - Severity CRITICAL - PostgreSQL doesn't support the EXECUTE IMMEDIATE statement with BULK COLLECT. Perform a manual conversion.]
        EXECUTE IMMEDIATE v_sql BULK COLLECT INTO tab_seq
        */
        RAISE DEBUG USING MESSAGE = CONCAT_WS('', 'L1 -->', i.partition_name);

        FOR k IN array_lower(tab_seq, 1)..array_upper(tab_seq, 1) LOOP
            EXECUTE ('UPDATE pspadm.psp_pstub_pay_item AS pppi SET pppi.company_fk = (SELECT ppc.company_fk FROM pspadm.psp_paycheck AS ppc, pspadm.psp_paystub AS pps WHERE pps.paystub_seq = pppi.paystub_fk AND pps.paycheck_fk = ppc.paycheck_seq) WHERE pppi.pstub_pay_item_seq = $1 ') USING tab_seq[k];
            /*
            [5035 - Severity CRITICAL - A transaction cannot be ended inside a block with exception handlers. Revise your code to try move transaction control on side of application.]
            COMMIT
            */
    ...
    EXCEPTION
        WHEN others THEN
            ...
            ROLLBACK
            */
            RAISE USING hint = -20701, message = CONCAT_WS('', 'Error in updating psp_pstub_pay_item table ', SQLERRM), detail = 'User-defined exception';
END;
$BODY$
LANGUAGE plpgsql;

CREATE PROCEDURE ibobadm_pds.batch_fill_comp_col$batch_fill_pu(IN p_batch_id DOUBLE PRECISION)
AS 
$BODY$
/* p_batch_id     PLS_INTEGER := 1; */
DECLARE
...
BEGIN
    ...
    SELECT
        table_name, partition_name
        FROM pspadm.t_backfill_metadata
        WHERE batch_id = p_batch_id AND table_name = 'T_PPU' AND status IS NULL
    LOOP
        UPDATE pspadm.t_backfill_metadata
        SET status = 'P'
            WHERE partition_name = i.partition_name;
        ...
        COMMIT
        */
        v_sql := CONCAT_WS('', 'SELECT PAYCHECK_USAGE_SEQ from PSPADM.T_PPU PARTITION (', i.partition_name, ')');
        /*
        [5088 - Severity CRITICAL - PostgreSQL doesn't support the EXECUTE IMMEDIATE statement with BULK COLLECT. Perform a manual conversion.]
        EXECUTE IMMEDIATE v_sql BULK COLLECT INTO tab_seq
        */
        ...
        FOR k IN array_lower(tab_seq, 1)..array_upper(tab_seq, 1) LOOP
            EXECUTE ('UPDATE pspadm.psp_paycheck_usage AS ppu SET company_fk = (SELECT pc.company_seq AS company FROM pspadm.psp_company AS pc, pspadm.psp_bill AS pb, pspadm.psp_company_usage AS pcu WHERE pb.bill_seq = ppu.bill_fk AND pb.company_usage_fk = pcu.company_usage_seq AND pcu.source_company_id = pc.source_company_id AND pcu.source_system_cd = pc.source_system_cd) WHERE ppu.paycheck_usage_seq = $1') USING tab_seq[k];
            /*
            [5035 - Severity CRITICAL - A transaction cannot be ended inside a block with exception handlers. Revise your code to try move transaction control on side of application.]
            COMMIT
            */
    ...
    EXCEPTION
        WHEN others THEN
            ...
            ROLLBACK
            */
            RAISE USING hint = -20701, message = CONCAT_WS('', 'Error in updating psp_paycheck_usage table ', SQLERRM), detail = 'User-defined exception';
END;
$BODY$
LANGUAGE plpgsql;

CREATE PROCEDURE ibobadm_pds.pk_gems_accounts_receivable$prc_assoc_fin_txn_states(IN p_user_id TEXT, IN p_app_server_date TIMESTAMP WITHOUT TIME ZONE)
AS 
$BODY$
/* for audit purposes */
/* UTC Date */
BEGIN
    PERFORM ibobadm_pds.pk_gems_accounts_receivable$init();
    UPDATE psp_financial_trans_state AS fts
    SET fts.gems_upload_batch_fk = aws_oracle_ext.getglobalvariable(proutinename => 'ibobadm_pds.pk_gems_accounts_receivable', pvariable => 'g_gems_upload_batch_key', ptp => NULL::CHARACTER VARYING(100)), version = version + 1, modifier_id = p_user_id, modified_date = p_app_server_date
        WHERE fts.gems_upload_batch_fk IS NULL AND TRUNC(fts.transaction_state_eff_date::TIMESTAMP WITHOUT TIME ZONE) >= DATE(aws_oracle_ext.getglobalvariable(proutinename => 'ibobadm_pds.pk_gems_accounts_receivable', pvariable => 'g_psp_date', ptp => NULL::TIMESTAMP(6) WITHOUT TIME ZONE)) - (aws_oracle_ext.getglobalvariable(proutinename => 'ibobadm_pds.pk_gems_accounts_receivable', pvariable => 'g_lookback_days', ptp => NULL::DOUBLE PRECISION)::NUMERIC || ' days')::INTERVAL AND fts.transaction_state_eff_date >= DATE(aws_oracle_ext.getglobalvariable(proutinename => 'ibobadm_pds.pk_gems_accounts_receivable', pvariable => 'g_psp_date', ptp => NULL::TIMESTAMP(6) WITHOUT TIME ZONE)) - (aws_oracle_ext.getglobalvariable(proutinename => 'ibobadm_pds.pk_gems_accounts_receivable', pvariable => 'g_lookback_days', ptp => NULL::DOUBLE PRECISION)::NUMERIC || ' days')::INTERVAL AND fts.transaction_state_fk IN ('Executed', 'Voided', 'Completed') AND EXISTS (SELECT
            'T'
            FROM psp_financial_transaction AS ft, psp_posting_rule AS pr, psp_ledger_account AS la
...
CREATE PROCEDURE ibobadm_pds.pk_gems_accounts_receivable$prc_calc_reveivables(INOUT p_cur_gems_daily_upload REFCURSOR)
AS 
$BODY$
...
BEGIN
    ...
    SELECT
        aws_oracle_ext.getglobalvariable(proutinename => 'ibobadm_pds.pk_gems_accounts_receivable', pvariable => 'g_batch_id', ptp => NULL::DOUBLE PRECISION) AS batch_id, ft.sku, SUM(CASE la.ledger_account_type
        ...
        END) AS income_amt, SUM(CASE la.ledger_account_type
            ...
            ELSE 0::CHARACTER VARYING
        END) AS tax_amt
        FROM psp_financial_transaction AS ft, psp_financial_trans_state AS fts, psp_posting_rule AS pr, psp_ledger_account AS la
        WHERE ft.financial_transaction_seq = fts.financial_transaction_fk AND fts.transaction_state_eff_date >= DATE(aws_oracle_ext.getglobalvariable(proutinename => 'ibobadm_pds.pk_gems_accounts_receivable', pvariable => 'g_psp_date', ptp => NULL::TIMESTAMP(6) WITHOUT TIME ZONE)) - (aws_oracle_ext.getglobalvariable(proutinename => 'ibobadm_pds.pk_gems_accounts_receivable', pvariable => 'g_lookback_days', ptp => NULL::DOUBLE PRECISION)::NUMERIC || ' days')::INTERVAL AND
        /* AND ft.current_transaction_state_fk = fts.transaction_state_fk */
        fts.transaction_state_fk = pr.transaction_state_fk AND fts.gems_upload_batch_fk = aws_oracle_ext.getglobalvariable(proutinename => 'ibobadm_pds.pk_gems_accounts_receivable', pvariable => 'g_gems_upload_batch_key', ptp => NULL::CHARACTER VARYING(100)) AND pr.ledger_account_fk = la.ledger_account_cd AND pr.transaction_type_fk = ft.transaction_type_fk AND ft.sku IS NOT NULL AND la.reporting_frequency = 'Daily' AND la.ledger_account_type IN ('Income', 'SUTax')
        GROUP BY ft.sku
        ORDER BY ft.sku;
...
CREATE PROCEDURE ibobadm_pds.pk_gems_accounts_receivable$prc_create_upload_batch(IN p_user_id TEXT, IN p_app_server_date TIMESTAMP WITHOUT TIME ZONE)
AS 
$BODY$
/* for audit purposes */
/* UTC Date */
BEGIN
    PERFORM ibobadm_pds.pk_gems_accounts_receivable$init();
    INSERT INTO psp_gems_upload_batch (gems_upload_batch_seq, version, creator_id, created_date, modifier_id, modified_date, realm_id, batch_id, batch_type, upload_status, status_effective_date)
    VALUES (aws_oracle_ext.getglobalvariable(proutinename => 'ibobadm_pds.pk_gems_accounts_receivable', pvariable => 'g_gems_upload_batch_key', ptp => NULL::CHARACTER VARYING(100)), 0, p_user_id, p_app_server_date, p_user_id, p_app_server_date, - 1, aws_oracle_ext.getglobalvariable(proutinename => 'ibobadm_pds.pk_gems_accounts_receivable', pvariable => 'g_batch_id', ptp => NULL::DOUBLE PRECISION), 'Daily', 'InProcess', aws_oracle_ext.getglobalvariable(proutinename => 'ibobadm_pds.pk_gems_accounts_receivable', pvariable => 'g_psp_date', ptp => NULL::TIMESTAMP(6) WITHOUT TIME ZONE));
END;
$BODY$
LANGUAGE plpgsql;

CREATE PROCEDURE ibobadm_pds.pk_gems_accounts_receivable$prc_main(INOUT p_cur_gems_daily_upload REFCURSOR, IN p_old_upload_batch_id DOUBLE PRECISION, IN p_user_id TEXT, IN p_app_server_date TIMESTAMP WITHOUT TIME ZONE)
AS 
$BODY$
/* result set to return to client */
...
BEGIN
    ...
    SELECT SYS_EXTRACT_UTC (fn_get_psp_timestamp)
            ...
            FROM DUAL
    */
    PERFORM aws_oracle_ext.setglobalvariable(proutinename => 'ibobadm_pds.pk_gems_accounts_receivable', pvariable => 'g_psp_date', pval => pk_gems_accounts_receivable$g_psp_date);
    ...
    BEGIN
        ...
        PERFORM aws_oracle_ext.setglobalvariable(proutinename => 'ibobadm_pds.pk_gems_accounts_receivable', pvariable => 'g_lookback_days', pval => pk_gems_accounts_receivable$g_lookback_days);
        EXCEPTION
            WHEN no_data_found THEN
                PERFORM aws_oracle_ext.setglobalvariable(proutinename => 'ibobadm_pds.pk_gems_accounts_receivable', pvariable => 'g_lookback_days', pval => 5);
            WHEN others THEN
                PERFORM aws_oracle_ext.setglobalvariable(proutinename => 'ibobadm_pds.pk_gems_accounts_receivable', pvariable => 'g_lookback_days', pval => 5);
    END;
    SELECT
        ibobadm_pds.fn_format_sysguid(aws_oracle_ext.sys_guid())
        INTO STRICT pk_gems_accounts_receivable$g_gems_upload_batch_key;
    PERFORM aws_oracle_ext.setglobalvariable(proutinename => 'ibobadm_pds.pk_gems_accounts_receivable', pvariable => 'g_gems_upload_batch_key', pval => pk_gems_accounts_receivable$g_gems_upload_batch_key);
    SELECT
        nextval('ibobadm_pds.seq_gems_upload_batch_id')
        INTO STRICT pk_gems_accounts_receivable$g_batch_id;
    PERFORM aws_oracle_ext.setglobalvariable(proutinename => 'ibobadm_pds.pk_gems_accounts_receivable', pvariable => 'g_batch_id', pval => pk_gems_accounts_receivable$g_batch_id);
    CALL ibobadm_pds.pk_gems_accounts_receivable$prc_supercede_upload_batch(p_old_upload_batch_id, p_user_id, p_app_server_date);
    CALL ibobadm_pds.pk_gems_accounts_receivable$prc_create_upload_batch(p_user_id, p_app_server_date);
    CALL ibobadm_pds.pk_gems_accounts_receivable$prc_assoc_fin_txn_states(p_user_id, p_app_server_date);
    CALL ibobadm_pds.pk_gems_accounts_receivable$prc_calc_reveivables(p_cur_gems_daily_upload);
END;
$BODY$
LANGUAGE plpgsql;

CREATE PROCEDURE ibobadm_pds.pk_gems_accounts_receivable$prc_supercede_upload_batch(IN p_old_upload_batch_id DOUBLE PRECISION, IN p_user_id TEXT, IN p_app_server_date TIMESTAMP WITHOUT TIME ZONE)
AS 
$BODY$
...
CREATE PROCEDURE ibobadm_pds.pk_payroll_item_totals$prc_comp_qtr_payroll_item_tot(IN p_company_seq TEXT, IN p_qtr_start_date TIMESTAMP WITHOUT TIME ZONE)
AS 
$BODY$
/* Company seq */
...
BEGIN
    ...
    */
    RAISE DEBUG USING MESSAGE = CONCAT_WS('', 'Merging PRC_COMP_QTR_PAYROLLITEM_TOT, starting merge  - Started at ', aws_oracle_ext.TO_CHAR(aws_oracle_ext.systimestamp(), 'hh24:mi:ss'));
    /* Deleting records for voided payrolls */
    DELETE FROM psp_ee_payrollitem_qtrtotals AS eepayrollitemqtrtotals
    ...
    SELECT
        ...
        FROM (SELECT
            ...
            SELECT
                ...
                FROM psp_employer_contribution AS pec
                JOIN psp_company_payroll_item AS pcpi
            ...
            SELECT
                ...
                END) AS amount
                FROM psp_qbdt_payroll_trans_line AS ptransline
                JOIN psp_company_payroll_item AS pcpi
                    ON pcpi.company_payroll_item_seq = ptransline.company_payroll_item_fk
                JOIN psp_qbdt_payroll_transaction AS ptrans
                    ON ptrans.qbdt_payroll_transaction_seq = ptransline.qbdt_payroll_transaction_fk
                JOIN psp_payroll_item AS pitem
                    ON pitem.payroll_item_code = pcpi.payroll_item_fk
                WHERE TRUNC(ptrans.period_end_date::TIMESTAMP WITHOUT TIME ZONE, 'Q') = DATE(p_qtr_start_date) AND
                /* :qtrStartDate */
    ...
    RAISE DEBUG USING MESSAGE = CONCAT_WS('', 'Finished merging PRC_COMP_QTR_PAYROLLITEM_TOT  - ', aws_oracle_ext.TO_CHAR(aws_oracle_ext.systimestamp(), 'hh24:mi:ss'));
END;
$BODY$
LANGUAGE plpgsql;

CREATE PROCEDURE ibobadm_pds.pk_payroll_item_totals$prc_qtr_payroll_item_tot(IN p_qtr_start_date TIMESTAMP WITHOUT TIME ZONE)
AS 
$BODY$
/* Quarter start date in UTC */
DECLARE
...
BEGIN
    ...
    SELECT SYS_EXTRACT_UTC (fn_get_psp_timestamp) INTO v_utc_date FROM DUAL
    */
    RAISE DEBUG USING MESSAGE = CONCAT_WS('', 'Merging PRC_QTR_PAYROLL_ITEM_TOT, starting merge  - Started at ', aws_oracle_ext.TO_CHAR(aws_oracle_ext.systimestamp(), 'hh24:mi:ss'));
    /* Deleting records for voided payrolls */
    DELETE FROM psp_ee_payrollitem_qtrtotals AS eepayrollitemqtrtotals
        WHERE eepayrollitemqtrtotals.quarter = (SELECT
            aws_oracle_ext.TO_NUMBER(aws_oracle_ext.TO_CHAR(p_qtr_start_date, 'Q'))) AND eepayrollitemqtrtotals.year = (SELECT
    ...
    SELECT
        ...
        FROM (SELECT
            ...
            FROM (SELECT
                ...
                WHERE TRUNC(pr.paycheck_date::TIMESTAMP WITHOUT TIME ZONE, 'Q') = DATE(p_qtr_start_date) AND
                /* :qtrStartDate */
                pcheck.source_employee_fk IS NOT NULL AND pcpi.is_archived = 0 AND pcheck.status = 'Active' AND pcheck.source_paycheck_id::TEXT NOT LIKE '-%' AND pr.payroll_run_status != 'Superseded' AND pcpi.tax_form_line IN ('ALLOCTIPS', 'DPDNTCARE', 'DPDNTCARECO', 'NONQUALPLAN', 'SEC457', 'ADOPTION', 'GROUPTERMLIFE', 'MEDSAVING', 'NONTAXSICK', 'QUALMVEX', 'ROTH401K', 'ROTH403B', 'SIMPLE', 'Q125POP', 'Q401K', 'Q403B', 'Q408K', 'Q457B', 'TTT14', 'Q501C', 'TTT3', 'TTT7', 'TTT8', 'FRNGBNFTS', 'OTHER', 'OTHMVEXP', 'TTT1', 'TTT2', 'TTT4', 'TTT5', 'LTAX1', 'LTAX2', 'SECLOCAL', 'TTT11', 'TTT6', 'TTT9', 'TIPS', 'TTT10', 'TTT17', 'TTT19')
                /* (:taxFormLines) */
                GROUP BY pcpi.company_payroll_item_seq, pcheck.source_employee_fk
            UNION ALL
            SELECT
            ...
            SELECT
                ...
                GROUP BY pcpi.company_payroll_item_seq, ptrans.employee_fk) AS var_sbq
            GROUP BY company_payroll_item_seq, source_employee_fk) AS src;
    RAISE DEBUG USING MESSAGE = CONCAT_WS('', 'Finished merging PRC_QTR_PAYROLL_ITEM_TOT  - ', aws_oracle_ext.TO_CHAR(aws_oracle_ext.systimestamp(), 'hh24:mi:ss'));
END;
$BODY$
LANGUAGE plpgsql;

CREATE PROCEDURE ibobadm_pds.pk_payroll_item_totals$prc_year_payroll_item_tot(IN p_year TEXT)
AS 
$BODY$
/* Quarter start date in UTC */
DECLARE
...
BEGIN
    ...
    SELECT SYS_EXTRACT_UTC (fn_get_psp_timestamp) INTO v_utc_date FROM DUAL
    */
    RAISE DEBUG USING MESSAGE = CONCAT_WS('', 'Merging PRC_YEAR_PAYROLL_ITEM_TOT, starting merge  - Started at ', aws_oracle_ext.TO_CHAR(aws_oracle_ext.systimestamp(), 'hh24:mi:ss'));
    /* Deleting records for voided payrolls */
    DELETE FROM psp_ee_payrollitem_qtrtotals AS eepayrollitemqtrtotals
    ...
    INSERT INTO psp_ee_payrollitem_qtrtotals (ee_payrollitem_qtrtotals_seq, version, creator_id, created_date, modified_date, realm_id, quarter, year, amount, taxable_wages, total_wages, tips_taxable_wages_amount, company_payroll_item_fk, employee_fk)
    SELECT
        ibobadm_pds.fn_format_sysguid(aws_oracle_ext.sys_guid()), 1, 'PK_PAYROLL_ITEM_TOTALS-INS', v_utc_date, v_utc_date, - 1, aws_oracle_ext.TO_NUMBER(src.qtr), aws_oracle_ext.TO_NUMBER(p_year), src.amount, 0, 0, 0, src.company_payroll_item_seq, src.source_employee_fk
        FROM (SELECT
            ...
            FROM (SELECT
                ...
                JOIN psp_payroll_run AS pr
                    ON pr.payroll_run_seq = pcheck.payroll_run_fk
                WHERE (SELECT
                    aws_oracle_ext.TO_CHAR(pr.paycheck_date, 'YYYY')) = p_year AND
                /* :year */
                pcheck.source_employee_fk IS NOT NULL AND pcpi.is_archived = 0 AND pcheck.status = 'Active' AND pcheck.source_paycheck_id::TEXT NOT LIKE '-%' AND pr.payroll_run_status != 'Superseded' AND pcpi.tax_form_line IN ('ALLOCTIPS', 'DPDNTCARE', 'DPDNTCARECO', 'NONQUALPLAN', 'SEC457', 'ADOPTION', 'GROUPTERMLIFE', 'MEDSAVING', 'NONTAXSICK', 'QUALMVEX', 'ROTH401K', 'ROTH403B', 'SIMPLE', 'Q125POP', 'Q401K', 'Q403B', 'Q408K', 'Q457B', 'TTT14', 'Q501C', 'TTT3', 'TTT7', 'TTT8', 'FRNGBNFTS', 'OTHER', 'OTHMVEXP', 'TTT1', 'TTT2', 'TTT4', 'TTT5', 'LTAX1', 'LTAX2', 'SECLOCAL', 'TTT11', 'TTT6', 'TTT9', 'TIPS', 'TTT10', 'TTT17', 'TTT19')
                /* (:taxFormLines) */
            ...
            SELECT
                ...
                WHERE (SELECT
                    aws_oracle_ext.TO_CHAR(pr.paycheck_date, 'YYYY')) = p_year AND
                /* :year */
                pcheck.source_employee_fk IS NOT NULL AND pcpi.is_archived = 0 AND pr.payroll_run_status != 'Superseded' AND pcheck.status = 'Active' AND pcheck.source_paycheck_id::TEXT NOT LIKE '-%' AND pcpi.tax_form_line IN ('ALLOCTIPS', 'DPDNTCARE', 'DPDNTCARECO', 'NONQUALPLAN', 'SEC457', 'ADOPTION', 'GROUPTERMLIFE', 'MEDSAVING', 'NONTAXSICK', 'QUALMVEX', 'ROTH401K', 'ROTH403B', 'SIMPLE', 'Q125POP', 'Q401K', 'Q403B', 'Q408K', 'Q457B', 'TTT14', 'Q501C', 'TTT3', 'TTT7', 'TTT8', 'FRNGBNFTS', 'OTHER', 'OTHMVEXP', 'TTT1', 'TTT2', 'TTT4', 'TTT5', 'LTAX1', 'LTAX2', 'SECLOCAL', 'TTT11', 'TTT6', 'TTT9', 'TIPS', 'TTT10', 'TTT17', 'TTT19')
                /* (:taxFormLines) */
                GROUP BY pcpi.company_payroll_item_seq, pcheck.source_employee_fk, aws_oracle_ext.TO_CHAR(pr.paycheck_date, 'Q')
            UNION ALL
            SELECT
            ...
            SELECT
                ...
                JOIN psp_payroll_item AS pitem
                    ON pitem.payroll_item_code = pcpi.payroll_item_fk
                WHERE (SELECT
                    aws_oracle_ext.TO_CHAR(ptrans.period_end_date, 'YYYY')) = p_year AND
                /* :year */
                ptrans.employee_fk IS NOT NULL AND pcpi.is_archived = 0 AND ptrans.is_voided = 0 AND pcpi.tax_form_line IN ('ALLOCTIPS', 'DPDNTCARE', 'DPDNTCARECO', 'NONQUALPLAN', 'SEC457', 'ADOPTION', 'GROUPTERMLIFE', 'MEDSAVING', 'NONTAXSICK', 'QUALMVEX', 'ROTH401K', 'ROTH403B', 'SIMPLE', 'Q125POP', 'Q401K', 'Q403B', 'Q408K', 'Q457B', 'TTT14', 'Q501C', 'TTT3', 'TTT7', 'TTT8', 'FRNGBNFTS', 'OTHER', 'OTHMVEXP', 'TTT1', 'TTT2', 'TTT4', 'TTT5', 'LTAX1', 'LTAX2', 'SECLOCAL', 'TTT11', 'TTT6', 'TTT9', 'TIPS', 'TTT10', 'TTT17', 'TTT19')
                /* (:taxFormLines) */
    ...
    RAISE DEBUG USING MESSAGE = CONCAT_WS('', 'Finished merging PRC_YEAR_PAYROLL_ITEM_TOT  - ', aws_oracle_ext.TO_CHAR(aws_oracle_ext.systimestamp(), 'hh24:mi:ss'));
END;
$BODY$
LANGUAGE plpgsql;

CREATE PROCEDURE ibobadm_pds.prc_achtransactionprocessor(IN p_source_system_cd TEXT, IN p_user_id TEXT, IN p_processing_date TIMESTAMP WITHOUT TIME ZONE, IN p_application_server_date TIMESTAMP WITHOUT TIME ZONE, INOUT p_number_of_records NUMERIC)
AS 
$BODY$
/* For audit purposes */
/* UTC Date */
...
BEGIN
    ...
    */
    ...
    INSERT INTO psp_financial_trans_state (financial_trans_state_seq, version, creator_id, created_date, modifier_id, modified_date, realm_id, transaction_state_eff_date, insert_user_id, gems_upload_batch_fk, financial_transaction_fk, transaction_state_fk, transaction_response_fk, company_fk, transaction_type_fk)
    SELECT
        ibobadm_pds.fn_format_sysguid(aws_oracle_ext.sys_guid()), 0, p_user_id, p_application_server_date, p_user_id, p_application_server_date, - 1, DATE(v_psp_date) + (.5::NUMERIC || ' days')::INTERVAL, NULL, NULL, ft.financial_transaction_seq, 'Completed', NULL, ft.company_fk, ft.transaction_type_fk
        FROM psp_financial_transaction AS ft
        INNER JOIN psp_company AS c
    ...
    UPDATE psp_financial_transaction AS ft
...
CREATE PROCEDURE ibobadm_pds.prc_calculate_ledger_balance(IN p_ledger_update_date TIMESTAMP WITHOUT TIME ZONE DEFAULT (CLOCK_TIMESTAMP() AT TIME ZONE COALESCE(CURRENT_SETTING('aws_oracle_ext.tz', TRUE), 'UTC'))::TIMESTAMP(0) - (1::NUMERIC || ' days')::INTERVAL)
AS 
$BODY$
/*
*****************************************************************************
...
BEGIN
    ...
    */
    ...
    MERGE INTO psp_ledger_balance tgt
                 USING (SELECT  fts.company_fk,
                                 TRUNC (fts.transaction_state_eff_date) newbal_date,
...
CREATE PROCEDURE ibobadm_pds.prc_calculate_w2_totals(IN p_processing_year NUMERIC, IN p_company_id TEXT)
AS 
$BODY$
/*
*****************************************************************************
...
BEGIN
    ...
    */
    IF p_company_id IS NULL THEN
        ...
        INSERT INTO psp_company_tfssubmission (company_tfssubmission_seq, version, creator_id, created_date, modifier_id, modified_date, realm_id, submission_status, status_effective_date, company_fk, year)
        SELECT
            ibobadm_pds.fn_format_sysguid(aws_oracle_ext.sys_guid()), 1, 'W2ANNUALCALCULATIONINS', v_utc_date, 'W2ANNUALCALCULATIONINS', v_utc_date, - 1, 'Pending', v_utc_date, company_fk, p_processing_year
            FROM (SELECT DISTINCT
                ...
                SELECT DISTINCT
                    company_fk AS company_fk
                    FROM psp_ee_payrollitem_qtrtotals AS pit
                    INNER JOIN psp_company_payroll_item AS cpi
    ...
    ELSE
        ...
        INSERT INTO psp_company_tfssubmission (company_tfssubmission_seq, version, creator_id, created_date, modifier_id, modified_date, realm_id, submission_status, status_effective_date, company_fk, year)
        VALUES (ibobadm_pds.fn_format_sysguid(aws_oracle_ext.sys_guid()), 1, 'W2ANNUALCALCULATIONINS', v_utc_date, 'W2ANNUALCALCULATIONINS', v_utc_date, - 1, 'Pending', v_utc_date, p_company_id, p_processing_year);
        /*
        [9996 - Severity CRITICAL - Transformer error occurred. Please submit report to developers.]
        MERGE INTO psp_employee_w2_totals tgt
                     USING (
...
BEGIN
    ...
    */
    INSERT INTO psp_company_event (company_event_seq, version, creator_id, created_date, modifier_id, modified_date, realm_id, event_time_stamp, status_effective_date, status_cd, event_type_cd, event_token, source_id, note_last_updated_date, company_fk)
    SELECT
        ibobadm_pds.fn_format_sysguid(aws_oracle_ext.sys_guid()), 0, p_user_id, p_app_server_date, p_user_id, p_app_server_date, - 1, p_app_server_date, p_app_server_date, 'Active', 'TaxPaymentStatusChanged', 0, mmt.money_movement_transaction_seq, NULL, mmt.company_fk
        FROM psp_eftps_payment_detail AS efpd
        JOIN psp_money_movement_transaction AS mmt
...
CREATE PROCEDURE ibobadm_pds.prc_eftps_payments_mmt_status(IN p_calling_procedure TEXT, IN p_user_id TEXT, IN p_app_server_date TIMESTAMP WITHOUT TIME ZONE, IN p_payment_file_seq TEXT, IN p_payment_initiation_date TIMESTAMP WITHOUT TIME ZONE)
AS 
$BODY$
/* for audit purposes */
/* UTC Date */
...
BEGIN
    ...
    */
    ...
    CALL ibobadm_pds.prc_recalc_atf_payments_eftps(p_calling_procedure, p_user_id, p_app_server_date, p_payment_file_seq, p_payment_initiation_date);
...
CREATE PROCEDURE ibobadm_pds.prc_eftps_payments_response(IN p_user_id TEXT, IN p_app_server_date TIMESTAMP WITHOUT TIME ZONE, IN p_response_file_id DOUBLE PRECISION, IN p_complete_fin_txns DOUBLE PRECISION DEFAULT 0)
AS 
$BODY$
/* for audit purposes */
/* UTC Date */
...
BEGIN
    ...
    BEGIN
        SELECT
            ...
            FROM psp_eftps_file AS ef_response
            JOIN psp_eftps_payment_detail AS efpd
                ON efpd.response_file_fk = ef_response.eftps_file_seq
            JOIN psp_eftps_file AS ef_payment
                ON ef_payment.eftps_file_seq = efpd.parent_file_fk
            JOIN psp_money_movement_transaction AS mmt
                ON mmt.money_movement_transaction_seq = efpd.money_movement_transaction_fk
            JOIN psp_edi_tax_file AS etf
    ...
    END;
    CALL ibobadm_pds.prc_eftps_payments_mmt_status('PRC_EFTPS_PAYMENTS_RESPONSE'::TEXT, p_user_id, p_app_server_date, v_payment_file_seq, v_payment_initiation_date);
    /*
    [5340 - Severity CRITICAL - PostgreSQL doesn't support the STANDARD.SYS_EXTRACT_UTC(TIMESTAMP_TZ_UNCONSTRAINED) function. Use suitable function or create user defined function.]
    PRC_SET_PSP_EVENT_LOG (
                   v_RETURN_CD,
    ...
    */
    /* company events */
    CALL ibobadm_pds.prc_eftps_payments_events('PRC_EFTPS_PAYMENTS_RESPONSE'::TEXT, p_user_id, p_app_server_date, v_payment_file_seq, v_payment_initiation_date, 'AcknowledgedByAgency'::TEXT, 'RejectedByAgency'::TEXT);
END;
$BODY$
LANGUAGE plpgsql;

CREATE PROCEDURE ibobadm_pds.prc_eftps_payments_return(IN p_user_id TEXT, IN p_app_server_date TIMESTAMP WITHOUT TIME ZONE, IN p_return_file_id DOUBLE PRECISION)
AS 
$BODY$
/* for audit purposes */
/* UTC Date */
    ...
    FOR payment_detail_rec IN payment_file_cursor LOOP
        ...
        BEGIN
            ...
            SELECT
                ...
                FROM psp_eftps_payment_detail AS epd
                JOIN psp_money_movement_transaction AS mmt
            ...
            EXCEPTION
                ...
                WHEN others THEN
                    ...
                    RAISE;
        END;
        CALL ibobadm_pds.prc_eftps_payments_mmt_status('PRC_EFTPS_PAYMENTS_RETURN'::TEXT, p_user_id, p_app_server_date, v_payment_file_seq, v_payment_initiation_date);
        /*
        [5340 - Severity CRITICAL - PostgreSQL doesn't support the STANDARD.SYS_EXTRACT_UTC(TIMESTAMP_TZ_UNCONSTRAINED) function. Use suitable function or create user defined function.]
        PRC_SET_PSP_EVENT_LOG (
                   v_RETURN_CD,
        ...
        */
        ...
        SET current_transaction_state_fk = 'Returned', modifier_id = p_user_id, modified_date = p_app_server_date, version = version + 1
            WHERE TRUNC(settlement_date::TIMESTAMP WITHOUT TIME ZONE) >= DATE(v_payment_initiation_date) AND settlement_date >= DATE(v_payment_initiation_date) AND current_transaction_state_fk IN ('Executed', 'Completed') AND money_movement_transaction_fk IN (SELECT
                money_movement_transaction_fk
                FROM psp_eftps_payment_detail AS efpd
                JOIN psp_money_movement_transaction AS mmt
                    ON mmt.money_movement_transaction_seq = efpd.money_movement_transaction_fk
        ...
        CALL ibobadm_pds.prc_eftps_payments_events('PRC_EFTPS_PAYMENTS_RETURN'::TEXT, p_user_id, p_app_server_date, v_payment_file_seq, v_payment_initiation_date, 'ReturnedTaxPaid, ReturnedTaxNotPaid'::TEXT);
    END LOOP;
END;
$BODY$
LANGUAGE plpgsql;

CREATE PROCEDURE ibobadm_pds.prc_eftps_payments_sent(IN p_user_id TEXT, IN p_app_server_date TIMESTAMP WITHOUT TIME ZONE, IN p_payment_file_id DOUBLE PRECISION)
AS 
$BODY$
/* for audit purposes */
/* UTC Date */
...
BEGIN
    BEGIN
        SELECT
            ...
            FROM psp_eftps_file AS ef
            JOIN psp_eftps_payment_detail AS epd
    ...
    */
    ...
    SET current_transaction_state_fk = 'Executed', modifier_id = p_user_id, modified_date = p_app_server_date, version = version + 1
        WHERE TRUNC(settlement_date::TIMESTAMP WITHOUT TIME ZONE) >= DATE(p_app_server_date) AND settlement_date >= p_app_server_date AND current_transaction_state_fk = 'Created' AND money_movement_transaction_fk IN (SELECT
...
CREATE PROCEDURE ibobadm_pds.prc_eftps_payments_sent_events(IN p_user_id TEXT, IN p_app_server_date TIMESTAMP WITHOUT TIME ZONE, IN p_payment_file_id DOUBLE PRECISION, IN p_tax_payment_status TEXT)
AS 
$BODY$
/* for audit purposes */
/* UTC Date */
    ...
    BEGIN
        ...
        EXCEPTION
            ...
            WHEN others THEN
                ...
                RAISE DEBUG USING MESSAGE = aws$frmt_err_stck::TEXT;
    END;
    CALL ibobadm_pds.prc_eftps_payments_events('PRC_EFTPS_PAYMENTS_SENT'::TEXT, p_user_id, p_app_server_date, v_payment_file_seq, v_payment_initiation_date, p_tax_payment_status);
END;
$BODY$
LANGUAGE plpgsql;

CREATE PROCEDURE ibobadm_pds.prc_offload(IN p_offload_batch_id TEXT, IN p_offload_date TIMESTAMP WITHOUT TIME ZONE, IN p_file_type TEXT, IN p_user_id TEXT, IN p_app_server_date TIMESTAMP WITHOUT TIME ZONE)
AS 
$BODY$
...
BEGIN
    PERFORM aws_oracle_ext.packageinitialize(proutinename => 'ibobadm_pds.prc_offload', pforce => TRUE);
    /*
    [5340 - Severity CRITICAL - PostgreSQL doesn't support the TO_TIMESTAMP_TZ function. Use suitable function or create user defined function., 5340 - Severity CRITICAL - PostgreSQL doesn't support the STANDARD.SYS_EXTRACT_UTC(TIMESTAMP_TZ_UNCONSTRAINED) function. Use suitable function or create user defined function., 5578 - Severity CRITICAL - Unable to automatically transform the SELECT statement. Try rewriting the statement.]
    SELECT SYS_EXTRACT_UTC (to_timestamp_tz(concat(fn_get_psp_timestamp(0), ' US/Pacific')))
         INTO v_psp_date
         FROM DUAL
    */
    PERFORM aws_oracle_ext.setglobalvariable(proutinename => 'ibobadm_pds.prc_offload', pvariable => 'v_psp_date', pval => v_psp_date$1937460023);
    PERFORM aws_oracle_ext.setglobalvariable(proutinename => 'ibobadm_pds.prc_offload', pvariable => 'v_utc_date', pval => p_app_server_date);
    UPDATE psp_offload_batch
    SET status_cd = 'Completed', status_effecive_date = aws_oracle_ext.getglobalvariable(proutinename => 'ibobadm_pds.prc_offload', pvariable => 'v_psp_date', ptp => NULL::TIMESTAMP(6) WITHOUT TIME ZONE), version = version + 1, modifier_id = aws_oracle_ext.getglobalvariable(proutinename => 'ibobadm_pds.prc_offload', pvariable => 'P_USER_ID', ptp => NULL::VARCHAR(8000)), modified_date = aws_oracle_ext.getglobalvariable(proutinename => 'ibobadm_pds.prc_offload', pvariable => 'v_utc_date', ptp => NULL::TIMESTAMP(6) WITHOUT TIME ZONE)
        WHERE offload_batch_seq = aws_oracle_ext.getglobalvariable(proutinename => 'ibobadm_pds.prc_offload', pvariable => 'P_OFFLOAD_BATCH_ID', ptp => NULL::VARCHAR(8000));

    IF p_file_type = 'DD' THEN
        CALL ibobadm_pds.prc_offload$update_nacha_file_trace_number$afcc73f2('CCD'::TEXT);
        CALL ibobadm_pds.prc_offload$update_nacha_file_trace_number$afcc73f2('PPD'::TEXT);
    END IF;
    /* 'DD' File Type */

    IF p_file_type = 'Tax' THEN
        CALL ibobadm_pds.prc_offload$update_nacha_file_trace_number$afcc73f2('CCDPlus'::TEXT);
    END IF;
    /* 'Tax' File Type */
END;
$BODY$
...
CREATE PROCEDURE ibobadm_pds.prc_offload$update_nacha_file_trace_number$afcc73f2(IN p_nacha_file_type TEXT)
AS 
$BODY$
/* ibobadm_pds.prc_offload.update_nacha_file_trace_number */
DECLARE
...
BEGIN
    prc_offload$v_return_cd := aws_oracle_ext.get_package_variable('ibobadm_pds', 'prc_offload', 'v_return_cd')::DOUBLE PRECISION;
    prc_offload$v_error_desc := aws_oracle_ext.get_package_variable('ibobadm_pds', 'prc_offload', 'v_error_desc')::CHARACTER VARYING(100);
    /*
    [5340 - Severity CRITICAL - PostgreSQL doesn't support the STANDARD.SYS_EXTRACT_UTC(TIMESTAMP_TZ_UNCONSTRAINED) function. Use suitable function or create user defined function.]
    PRC_SET_PSP_EVENT_LOG (
                        v_RETURN_CD,
    ...
    */
    PERFORM aws_oracle_ext.set_package_variable('ibobadm_pds', 'prc_offload', 'v_error_desc', prc_offload$v_error_desc);
    PERFORM aws_oracle_ext.set_package_variable('ibobadm_pds', 'prc_offload', 'v_return_cd', prc_offload$v_return_cd);
    SELECT
        nachafile_seq
        INTO STRICT v_nacha_file_id
        FROM psp_nachafile
        WHERE offload_batch_fk = aws_oracle_ext.getglobalvariable(proutinename => 'ibobadm_pds.prc_offload', pvariable => 'P_OFFLOAD_BATCH_ID', ptp => NULL::VARCHAR(8000)) AND file_type = p_nacha_file_type;
    ...
    */
    ...
    IF sql$rowcount != 0 THEN
        ...
        SET status = 'Finalized', version = version + 1, modifier_id = aws_oracle_ext.getglobalvariable(proutinename => 'ibobadm_pds.prc_offload', pvariable => 'P_USER_ID', ptp => NULL::VARCHAR(8000)), modified_date = aws_oracle_ext.getglobalvariable(proutinename => 'ibobadm_pds.prc_offload', pvariable => 'v_utc_date', ptp => NULL::TIMESTAMP(6) WITHOUT TIME ZONE), finalization_date = aws_oracle_ext.getglobalvariable(proutinename => 'ibobadm_pds.prc_offload', pvariable => 'v_psp_date', ptp => NULL::TIMESTAMP(6) WITHOUT TIME ZONE), status_effective_date = aws_oracle_ext.getglobalvariable(proutinename => 'ibobadm_pds.prc_offload', pvariable => 'v_psp_date', ptp => NULL::TIMESTAMP(6) WITHOUT TIME ZONE)
...
CREATE PROCEDURE ibobadm_pds.prc_offload_insert_fts(IN p_user_id TEXT, IN p_app_server_date TIMESTAMP WITHOUT TIME ZONE, IN p_offload_batch_id TEXT, IN p_offload_date TIMESTAMP WITHOUT TIME ZONE)
AS 
$BODY$
/* For audit purposes */
/* UTC Date */
...
CREATE PROCEDURE ibobadm_pds.prc_offload_upd_agency_status(IN p_user_id TEXT, IN p_app_server_date TIMESTAMP WITHOUT TIME ZONE, IN p_offload_batch_id TEXT, IN p_offload_date TIMESTAMP WITHOUT TIME ZONE, IN p_file_type TEXT)
AS 
$BODY$
/* For audit purposes */
/* UTC Date */
...
BEGIN
    ...
    */
    ...
    IF p_file_type = 'Tax' THEN
        ...
        SELECT
            mmt.*
            FROM psp_money_movement_transaction AS mmt
        ...
        UPDATE psp_money_movement_transaction AS mmt
...
CREATE PROCEDURE ibobadm_pds.prc_offload_update_ft(IN p_user_id TEXT, IN p_app_server_date TIMESTAMP WITHOUT TIME ZONE, IN p_offload_batch_id TEXT, IN p_offload_date TIMESTAMP WITHOUT TIME ZONE)
AS 
$BODY$
/* For audit purposes */
/* UTC Date */
...
BEGIN
    ...
    */
    ...
    UPDATE psp_financial_transaction AS ft0
    SET current_transaction_state_fk = 'Executed', version = version + 1, modifier_id = p_user_id, modified_date = v_utc_date
        WHERE ft0.current_transaction_state_fk = 'Created' AND ft0.settlement_date >= p_offload_date AND EXISTS (SELECT
            'T'
            FROM psp_money_movement_transaction AS mmt1
...
CREATE PROCEDURE ibobadm_pds.prc_offload_update_mmt(IN p_user_id TEXT, IN p_app_server_date TIMESTAMP WITHOUT TIME ZONE, IN p_offload_batch_id TEXT, IN p_offload_date TIMESTAMP WITHOUT TIME ZONE, IN p_file_type TEXT)
AS 
$BODY$
/* For audit purposes */
/* UTC Date */
...
CREATE PROCEDURE ibobadm_pds.prc_offload_update_payroll(IN p_user_id TEXT, IN p_app_server_date TIMESTAMP WITHOUT TIME ZONE, IN p_offload_batch_id TEXT, IN p_offload_date TIMESTAMP WITHOUT TIME ZONE)
AS 
$BODY$
/* For audit purposes */
/* UTC Date */
...
BEGIN
    ...
    */
    ...
    SET payroll_run_status = 'OffloadedDebit', version = version + 1, modifier_id = p_user_id, modified_date = v_utc_date
        WHERE pr.payroll_run_status != 'OffloadedAll' AND EXISTS (SELECT
            'T'
            FROM psp_money_movement_transaction AS mmt1, psp_financial_transaction AS ft1, psp_transaction_type AS tt1
            WHERE ft1.payroll_run_fk = pr.payroll_run_seq AND mmt1.offload_batch_fk = p_offload_batch_id AND mmt1.initiation_date = p_offload_date AND mmt1.money_movement_transaction_seq = ft1.money_movement_transaction_fk AND ft1.settlement_date >= mmt1.initiation_date AND TRUNC(ft1.settlement_date::TIMESTAMP WITHOUT TIME ZONE) >= TRUNC(mmt1.initiation_date::TIMESTAMP WITHOUT TIME ZONE) AND ft1.current_transaction_state_fk = 'Executed' AND ft1.transaction_type_fk = tt1.transaction_type_cd AND tt1.association_type = 'Impound');
    UPDATE psp_payroll_run AS pr
    ...
    SET payroll_run_status = 'OffloadedAll', version = version + 1, modifier_id = p_user_id, modified_date = v_utc_date
        WHERE EXISTS (SELECT
            'T'
            FROM psp_money_movement_transaction AS mmt1, psp_financial_transaction AS ft1
    ...
    */
    ...
    SELECT
        ibobadm_pds.fn_format_sysguid(aws_oracle_ext.sys_guid()), 0, p_user_id, v_psp_date, p_user_id, v_psp_date, - 1, pr.payroll_run_seq
        FROM psp_payroll_run AS pr
    ...
    SET payroll_run_status = 'OffloadedAll', version = version + 1, modifier_id = p_user_id, modified_date = v_utc_date
        WHERE payroll_run_status != 'AutoRedebitOffloaded' AND EXISTS (SELECT
            ...
            'T'
            FROM psp_financial_transaction AS ft1
    ...
    SET payroll_run_status = 'Complete', version = version + 1, modifier_id = p_user_id, modified_date = v_utc_date
        WHERE pr.payroll_run_status = 'OffloadedAll' AND EXISTS (SELECT
            'T'
            FROM psp_money_movement_transaction AS mmt1, psp_financial_transaction AS ft1, psp_financial_transaction AS ft2
...
CREATE PROCEDURE ibobadm_pds.prc_recalc_atf_payments_eftps(IN p_calling_procedure TEXT, IN p_user_id TEXT, IN p_app_server_date TIMESTAMP WITHOUT TIME ZONE, IN p_payment_file_seq TEXT, IN p_payment_initiation_date TIMESTAMP WITHOUT TIME ZONE)
AS 
$BODY$
/* for audit purposes */
/* UTC Date */
...
BEGIN
    ...
    */
    ...
    INSERT INTO psp_atfpayments_to_process (atfpayments_to_process_seq, version, creator_id, created_date, modifier_id, modified_date, realm_id, payment_date, quarter_end_date, law_fk, money_movement_transaction_fk, company_fk, amount)
    SELECT
        ibobadm_pds.fn_format_sysguid(aws_oracle_ext.sys_guid()), 1, p_user_id, p_app_server_date, p_user_id, p_app_server_date, - 1, settlement_date, quarter_end, law_fk, money_movement_transaction_seq, company_fk, amount
        FROM (SELECT
            mmt.company_fk, mmt.money_movement_transaction_seq, ibobadm_pds.fn_get_last_day_of_quarter(mmt.payment_period_end) AS quarter_end, ft.law_fk, PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY ft.settlement_date::NUMERIC) AS settlement_date, SUM(CASE
            ...
            JOIN psp_money_movement_transaction AS mmt
                ON mmt.money_movement_transaction_seq = epd.money_movement_transaction_fk
            JOIN psp_financial_transaction AS ft
            ...
            GROUP BY mmt.company_fk, mmt.money_movement_transaction_seq, ibobadm_pds.fn_get_last_day_of_quarter(mmt.payment_period_end), ft.law_fk) AS var_sbq;
    /* rejected payments */
    ...
    SELECT
        ibobadm_pds.fn_format_sysguid(aws_oracle_ext.sys_guid()), 1, p_user_id, p_app_server_date, p_user_id, p_app_server_date, - 1, settlement_date, quarter_end, law_fk, money_movement_transaction_seq, company_fk, amount
        FROM (SELECT
            mmt.company_fk, mmt.money_movement_transaction_seq, ibobadm_pds.fn_get_last_day_of_quarter(mmt.payment_period_end) AS quarter_end, ft.law_fk, PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY ft.settlement_date::NUMERIC) AS settlement_date, 0 AS amount
            FROM psp_eftps_payment_detail AS epd
            JOIN psp_money_movement_transaction AS mmt
                ON mmt.money_movement_transaction_seq = epd.money_movement_transaction_fk
            JOIN psp_financial_transaction AS ft
            ...
            GROUP BY mmt.company_fk, mmt.money_movement_transaction_seq, ibobadm_pds.fn_get_last_day_of_quarter(mmt.payment_period_end), ft.law_fk) AS var_sbq_2;
END;
$BODY$
LANGUAGE plpgsql;

CREATE PROCEDURE ibobadm_pds.prc_recalculate_atf_payments(IN p_user_id TEXT, IN p_app_server_date TIMESTAMP WITHOUT TIME ZONE, IN p_mmt_seq CHARACTER VARYING, IN p_payment_template_cd CHARACTER VARYING, IN p_payment_method CHARACTER VARYING, IN p_payment_status CHARACTER VARYING, IN p_pay_period_end CHARACTER VARYING, IN p_initiation_date CHARACTER VARYING, IN p_company_seq CHARACTER VARYING, IN p_bulk_loading BOOLEAN DEFAULT FALSE)
AS 
$BODY$
/* for audit purposes */
/* UTC Date */
...
BEGIN
    IF p_payment_method IN ('EFTPS', 'EFTPSDirectDebit', 'HPDE', 'HPDERefund', 'CheckPayment', 'ACHCredit', 'ACHDebit', 'EDI', 'SuperCheck') AND p_payment_status IN ('AcknowledgedByAgency', 'ReturnedTaxPaid', 'RejectedByAgency', 'ReturnedTaxNotPaid', 'None') THEN
        ...
        SELECT
            psp_law.law_id
            FROM psp_law
            JOIN psp_payment_template AS pt
                ON pt.payment_template_cd = psp_law.payment_template_fk
        ...
        LOOP
            ...
            SELECT
                ...
                FROM psp_financial_transaction AS ft
                WHERE ft.money_movement_transaction_fk = p_mmt_seq AND ft.law_fk = lawRec.law_id AND ft.settlement_date >= p_initiation_date AND TRUNC(ft.settlement_date::TIMESTAMP WITHOUT TIME ZONE) >= TRUNC(p_initiation_date::TIMESTAMP WITHOUT TIME ZONE)
            LOOP
            ...
            IF v_settlement_date IS NOT NULL THEN
                ...
                INSERT INTO psp_atfpayments_to_process (atfpayments_to_process_seq, version, creator_id, created_date, modifier_id, modified_date, realm_id, payment_date, quarter_end_date, law_fk, money_movement_transaction_fk, company_fk, amount)
                VALUES (ibobadm_pds.fn_format_sysguid(aws_oracle_ext.sys_guid()), 1, p_user_id, p_app_server_date, p_user_id, p_app_server_date, - 1, v_settlement_date, ibobadm_pds.fn_get_last_day_of_quarter(p_pay_period_end), lawRec.law_id, p_mmt_seq, p_company_seq, v_amount);
            END IF;
        END LOOP;
    END IF;
END;
...
CREATE PROCEDURE ibobadm_pds.prc_remove_company(IN uniqueid TEXT, IN tablename TEXT)
AS 
$BODY$
DECLARE
    ...
    userconstraints ibobadm_pds.prc_remove_company$c1 [];
    child_constraint_collection userconstraints%TYPE;
    parent_constraint_collection userconstraints%TYPE;
    parentrecs ibobadm_pds.prc_remove_company$parentrec [];
...
BEGIN
            ...
            */
            ...
            IF (COALESCE(array_length(id_collection, 1), 0) > 0) THEN
                FOR j IN 1..COALESCE(array_length(id_collection, 1), 0) LOOP
                    CALL ibobadm_pds.prc_remove_company(id_collection[j], child_constraint_collection[i].table_name);
                END LOOP;
            END IF;
            EXCEPTION
                WHEN no_data_found THEN
    ...
    */
    ...
    FOR l IN 1..COALESCE(array_length(parent_recs, 1), 0) LOOP
        CALL ibobadm_pds.prc_remove_company(parent_recs[l].uniqueid, parent_recs[l].tablename);
    END LOOP;
    EXCEPTION
        WHEN no_data_found THEN
            /* DBMS_OUTPUT.PUT_LINE('@@@@@@@@@ EXCEPTION'); */
...
CREATE PROCEDURE ibobadm_pds.prc_remove_company_fast(IN uniqueid TEXT)
AS 
$BODY$
BEGIN
    ...
    DELETE FROM psp_cdcompany_service_info
        WHERE cdcompany_service_info_seq IN (SELECT
            ...
            FROM psp_company_service
            WHERE company_fk IN (uniqueId));
    DELETE FROM psp_racompany_service_info
    ...
    DELETE FROM psp_check_print_signature
        WHERE company_fk IN (uniqueId);
...
BEGIN
    SELECT
        *
        FROM aws_oracle_ext.autonomous_exec('CALL ibobadm_pds.prc_set_psp_event_log$at('||quote_nullable(aws_params)||','||quote_nullable(aws_session_env)||')')
...
CREATE PROCEDURE ibobadm_pds.prc_set_psp_event_log$at(INOUT aws_params JSON, INOUT aws_session_env JSON)
AS 
$BODY$
...
$BODY$
LANGUAGE plpgsql;
COMMENT ON PROCEDURE ibobadm_pds.prc_set_psp_event_log$at(INOUT aws_params JSON, INOUT aws_session_env JSON)
     IS 'Belongs to: ibobadm_pds.prc_set_psp_event_log';

CREATE PROCEDURE ibobadm_pds.prc_test()
AS 
$BODY$
DECLARE
    update_count NUMERIC(38);
    skip_count NUMERIC(38);
    updated_fileid DOUBLE PRECISION;
    fileid_guid CHARACTER VARYING(40);
    err_num DOUBLE PRECISION;
    err_msg CHARACTER VARYING(200);
    start_time TIMESTAMP(6) WITHOUT TIME ZONE;
    unset_fileid_companies_cur CURSOR FOR
    SELECT
        qbi.company_fk, qbi.rowid
        FROM pspadm.psp_quickbooks_info AS qbi
        JOIN pspadm.psp_company_service AS cs
            ON cs.company_fk = qbi.company_fk AND cs.service_fk IN ('DirectDeposit', 'Tax')
        WHERE COALESCE(LENGTH(qbi.file_id::TEXT), 0) < 32::TEXT;
    unset_fileid_companies_cur$ATTRIBUTES aws_oracle_data.TCursorAttributes := ROW (FALSE, NULL, NULL, NULL);
    unset_fileid_rec ibobadm_pds.prc_test$unset_fileid_companies_cur;
BEGIN
    SELECT
    ...
    FOR unset_fileid_rec IN unset_fileid_companies_cur LOOP
        updated_fileid := 0;
        ...
        BEGIN
            ...
            SELECT
                guid
                INTO STRICT fileid_guid
                FROM (SELECT
                    CAST (aws_oracle_ext.regexp_substr((sst1.request_document::TEXT)::TEXT, ('<I.QBFILEID>[[:print:]]+')::TEXT) AS CHARACTER VARYING(200)) AS guid, ROW_NUMBER() OVER (ORDER BY sst1.created_date DESC) AS rn
                    FROM pspadm.psp_source_system_transmission AS sst1
                    WHERE sst1.company_fk = unset_fileid_rec.company_fk AND sst1.from_source_system = 'QBDT' AND sst1.created_date BETWEEN DATE '2011-06-01' AND DATE '2012-02-01' AND CAST (aws_oracle_ext.regexp_substr((sst1.request_document::TEXT)::TEXT, ('<I.QBFILEID>[[:print:]]+')::TEXT) AS CHARACTER VARYING(200)) IS NOT NULL) AS a
                WHERE rn = 1;
            EXCEPTION
                WHEN no_data_found THEN
                    err_num := SQLSTATE;
                    err_msg := aws_oracle_ext.substr(SQLERRM, 1, 200);
        /* dbms_output.put_line(unset_fileid_rec.company_fk || ' - no SSTs found for company'); */
        END;

        IF fileid_guid IS NOT NULL THEN
            SELECT
                replace(fileid_guid, '<I.QBFILEID>', '')
                INTO STRICT fileid_guid;

            IF LENGTH(fileid_guid) = 32 THEN
                BEGIN
                    UPDATE psp_quickbooks_info AS pqi
                    SET pqi.file_id = fileid_guid
                        WHERE pqi.rowid = unset_fileid_rec.rowid;
                    /*
                    [5035 - Severity CRITICAL - A transaction cannot be ended inside a block with exception handlers. Revise your code to try move transaction control on side of application.]
                    commit
                    */
                    ...
                    EXCEPTION
                        WHEN others THEN
                            err_num := SQLSTATE;
                            err_msg := aws_oracle_ext.substr(SQLERRM, 1, 200);
                            RAISE DEBUG USING MESSAGE = CONCAT_WS('', unset_fileid_rec.company_fk, ' - error: ', err_num, ':: ', err_msg);
                END;
            ELSE
                RAISE DEBUG USING MESSAGE = CONCAT_WS('', unset_fileid_rec.company_fk, ' - bad file_id length - ', fileid_guid);
            END IF;
        END IF;

        IF updated_fileid = 1 THEN
            update_count := update_count + 1;
        ELSE
            skip_count := skip_count + 1;
        END IF;

        IF MOD(update_count + skip_count, 100) = 0 THEN
            RAISE DEBUG USING MESSAGE = CONCAT_WS('', 'processed ', (update_count + skip_count), ' in ', (aws_oracle_ext.systimestamp() - start_time));
        END IF;
    END LOOP;
    RAISE DEBUG USING MESSAGE = CONCAT_WS('', 'finished ', (update_count + skip_count), ' in ', (aws_oracle_ext.systimestamp() - start_time));
END;
$BODY$
LANGUAGE plpgsql;

CREATE PROCEDURE ibobadm_pds.prc_upd_company_ledger_balance(IN p_company_fk TEXT DEFAULT NULL)
AS 
$BODY$
DECLARE
    v_utc_date TIMESTAMP(6) WITHOUT TIME ZONE;
    /* current system UTC date and time */
    v_end_date TIMESTAMP(0) WITHOUT TIME ZONE := ibobadm_pds.fn_get_psp_timestamp() - 1;
    v_ledger_balance_max_date TIMESTAMP(0) WITHOUT TIME ZONE;
    financial_trans_state_cursor CURSOR FOR
    SELECT DISTINCT
        (aws_oracle_ext.TRUNC(transaction_state_eff_date)) AS transaction_state_eff_date
        FROM psp_financial_trans_state
        WHERE company_fk = p_company_fk AND transaction_state_eff_date < v_end_date + (1::NUMERIC || ' days')::INTERVAL AND aws_oracle_ext.TRUNC(transaction_state_eff_date) > (SELECT
            TRUNC(COALESCE(MAX(balance_date), (SELECT
                aws_oracle_ext.TO_DATE('01-JAN-2005')))::NUMERIC)
            FROM psp_ledger_balance AS lb
            WHERE lb.company_fk = p_company_fk)::TIMESTAMP WITHOUT TIME ZONE
...
BEGIN
    ...
    */
    ...
    FOR financial_trans_state_rec IN financial_trans_state_cursor LOOP
        INSERT INTO psp_ledger_balance (ledger_balance_seq, version, creator_id, created_date, modified_date, realm_id, balance_amount, balance_date, ledger_account_fk, company_fk, reporting_type)
        SELECT
            ibobadm_pds.fn_format_sysguid(aws_oracle_ext.sys_guid()), 1, 'COMPANYLEDGERBALANCERECALCJOB', v_utc_date, v_utc_date, - 1, COALESCE((SELECT
                ilb.balance_amount
                FROM psp_ledger_balance AS ilb
                WHERE ilb.ledger_account_fk = pr.ledger_account_fk AND ilb.company_fk = fts.company_fk AND ilb.reporting_type = po.reporting_type AND TRUNC(ilb.balance_date::TIMESTAMP WITHOUT TIME ZONE) = (SELECT
                    MAX(TRUNC(olb.balance_date::TIMESTAMP WITHOUT TIME ZONE))
                    FROM psp_ledger_balance AS olb
            ...
            CASE pr.credit_debit_ind
                ...
                CASE la.balance_calculation_rule
                    ...
                    ELSE - 1
                END
            END) AS amount, TRUNC(fts.transaction_state_eff_date::TIMESTAMP WITHOUT TIME ZONE) AS newbal_date, pr.ledger_account_fk, fts.company_fk, po.reporting_type
            FROM psp_financial_trans_state AS fts, psp_posting_rule AS pr, psp_ledger_account AS la, psp_company AS pc, psp_company_offering AS pco, psp_offering AS po
            ...
            /* AND PO.REPORTING_TYPE IN ('DirectDeposit','Tax') */
            po.service_code = 'DirectDeposit' AND TRUNC(fts.transaction_state_eff_date::TIMESTAMP WITHOUT TIME ZONE) = aws_oracle_ext.TRUNC(financial_trans_state_rec.transaction_state_eff_date) AND fts.transaction_state_eff_date BETWEEN financial_trans_state_rec.transaction_state_eff_date - (1::NUMERIC || ' days')::INTERVAL AND financial_trans_state_rec.transaction_state_eff_date + (1::NUMERIC || ' days')::INTERVAL AND fts.company_fk = p_company_fk
            GROUP BY fts.company_fk, TRUNC(fts.transaction_state_eff_date::TIMESTAMP WITHOUT TIME ZONE), pr.ledger_account_fk, po.reporting_type;
    END LOOP;
...
CREATE PROCEDURE ibobadm_pds.prc_update_ledger_balance()
AS 
$BODY$
DECLARE
...
BEGIN
    ...
    FOR i IN 1..((EXTRACT (EPOCH FROM aws_oracle_ext.TRUNC(v_end_date) - aws_oracle_ext.TRUNC(v_start_date)) / 86400)::NUMERIC) + 1 LOOP
        CALL ibobadm_pds.prc_calculate_ledger_balance(v_ledger_date);
