DECLARE
    ...
    RECORD_COUNT       INTEGER   := 570000;
    MAX_RECORDS        INTEGER   := 57000;
    PROCESSED_RECORDS  INTEGER   := 0;
    ERROR_RECORDS      INTEGER   := 0;
    MAX_FETCH_RECORDS  INTEGER   := 5000;
    BATCH_COUNT        INTEGER   := 0;
    START_DATE         DATE := TO_DATE('2010-05-26', 'yyyy-mm-dd');
    END_DATE           DATE := TO_DATE('2011-01-01', 'yyyy-mm-dd');
            ...
            WHILE (BATCH_COUNT > 0 AND PROCESSED_RECORDS < MAX_RECORDS)
                ...
                LOOP
                    BEGIN
                        ...
                        MERGE INTO PSPADM.PSP_TAX t
                        using (SELECT ti.TAX_SEQ, pi.COMPANY_FK
                               FROM PSPADM.PSP_TAX ti,
                                    PSPADM.PSP_PAYCHECK pi
                               WHERE pi.CREATED_DATE between START_DATE and PARTITION_END_DATE
                                 AND ti.PAYCHECK_FK = pi.PAYCHECK_SEQ
                                 AND ti.COMPANY_FK is null
                                 and ROWNUM <= MAX_FETCH_RECORDS) p
                        on (t.TAX_SEQ = p.TAX_SEQ)
                        when matched then
                            update set t.COMPANY_FK = p.COMPANY_FK;
