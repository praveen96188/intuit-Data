DECLARE
    RECORD_COUNT       INTEGER   := 13269484;
    MAX_RECORDS        INTEGER   := 13269484;
    PROCESSED_RECORDS  INTEGER   := 0;
    ERROR_RECORDS      INTEGER   := 0;
    MAX_FETCH_RECORDS  INTEGER   := 5000;
    BATCH_COUNT        INTEGER   := 0;
    START_DATE         DATE := TO_DATE('2010-05-28', 'yyyy-mm-dd');
            ...
            WHILE (BATCH_COUNT > 0 AND PROCESSED_RECORDS < MAX_RECORDS)
                ...
                LOOP
                    BEGIN
                        ...
                        MERGE INTO PSPADM.PSP_DEDUCTION c
                        using (SELECT ci.DEDUCTION_SEQ, pi.COMPANY_FK
                               FROM PSPADM.PSP_DEDUCTION ci,
                        ...
                        on (c.DEDUCTION_SEQ = p.DEDUCTION_SEQ)
                        when matched then
