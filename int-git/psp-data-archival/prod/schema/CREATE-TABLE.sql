...
CREATE TABLE pspadm.payroll_hist(
    ...
    payroll_submit_cnt int,
    tot_err_cnt int,
    distinct_err_type_cnt int
)
        WITH (
        OIDS=FALSE
        );
...
CREATE TABLE pspadm.psp_auth_user(
    ...
    nbr_of_failed_login_attempts BIGINT NOT NULL DEFAULT 0
)
        WITH (
        OIDS=FALSE
        );
...
CREATE TABLE pspadm.psp_company(
    ...
    account_locked_until TIMESTAMP(6) WITHOUT TIME ZONE,
    nbr_of_failed_login_attempts BIGINT,
    ...
    nbr_failed_authentications BIGINT,
...
CREATE TABLE pspadm.psp_company_usage(
    ...
    entitlement_id CHARACTER VARYING(20),
    license_id CHARACTER VARYING(20),
    billing_day_of_month BIGINT,
    start_day_of_usage_month BIGINT
)
        WITH (
        OIDS=FALSE
        );
...
CREATE TABLE pspadm.psp_employee_bank_account(
    ...
    account_order BIGINT DEFAULT - 1,
    session_id CHARACTER VARYING(100),
...
CREATE TABLE pspadm.psp_forecast(
    ...
    modified_date TIMESTAMP(6) WITHOUT TIME ZONE NOT NULL,
    realm_id int NOT NULL DEFAULT - 1,
    estimated_transaction_count BIGINT,
    status CHARACTER VARYING(255),
...
CREATE TABLE pspadm.psp_pstub_employee_info(
    ...
    fed_tax_filing_status_code BIGINT,
    ...
    state_tax_filing_status_code BIGINT,
...
CREATE TABLE pspadm.psp_source_system_transmission(
...
CREATE TABLE pspadm.psp_tax_table_misc_data(
    ...
    company_fk CHARACTER VARYING(255),
    misc_data_order BIGINT DEFAULT - 1
)
        WITH (
        OIDS=FALSE
        );
...
CREATE TABLE pspadm.psp_wc_paycheck(
    ...
    current_state_cd CHARACTER VARYING(255),
    paycheck_fk CHARACTER VARYING(255),
    paycheck_version int NOT NULL DEFAULT 1
)
        WITH (
        OIDS=FALSE
        );
...
CREATE TABLE pspadm.t_pceep_old(
    company_event_email_param_seq CHARACTER VARYING(255)
)
    PARTITION BY HASH (company_event_email_param_seq)
        WITH (
        OIDS=FALSE
        );

CREATE TABLE pspadm.t_pdatl_old(
    disburse_advice_tax_liab_seq CHARACTER VARYING(255)
)
    PARTITION BY HASH (disburse_advice_tax_liab_seq)
        WITH (
        OIDS=FALSE
        );

CREATE TABLE pspadm.t_pppi_old(
    pstub_pay_item_seq CHARACTER VARYING(255)
)
    PARTITION BY HASH (pstub_pay_item_seq)
        WITH (
        OIDS=FALSE
        );

CREATE TABLE pspadm.t_ppu(
    paycheck_usage_seq CHARACTER VARYING(255)
)
    PARTITION BY HASH (paycheck_usage_seq)
        WITH (
        OIDS=FALSE
        );

CREATE TABLE pspadm.t_ppu_old(
    paycheck_usage_seq CHARACTER VARYING(255)
)
    PARTITION BY HASH (paycheck_usage_seq)
        WITH (
...
CREATE TABLE pspadm.temp_guid_company(
    ...
    row_count_guid BIGINT,
    row_count_company BIGINT
)
        WITH (
        OIDS=FALSE
        );
