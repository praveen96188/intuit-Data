...
for instance_name in `aws --region us-east-2 rds describe-db-clusters --db-cluster-identifier ${cluster_postgres} --query 'DBClusters[0].DBClusterMembers' | grep DBInstanceIdentifier | awk '{print $2}'|sed 's/"//'|sed 's/"//' | rev | cut -c2- | rev`
do
    for instance_name1 in `echo ${instance_name} | awk '{print $1}'| rev | cut -c1- | rev`
    do
    ./run_postgres.sh ${instance_name} $db_name chk_active_session_pg.sql "-v dbname=${db_name}" > ${logdir}/${instance_name}_${db_name}__chk_active_connection.txt
