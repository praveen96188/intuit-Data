...
--region us-west-2 --replication-task-identifier ${source}-${target}-${task_identifier}-val-cdc \
...
aws dms describe-replication-tasks --region us-west-2 --filter=Name=migration-type,Values=cdc |grep ReplicationTaskIdentifier|awk {'print $2'}|sed 's/\"//g; s/,//'|grep ^$source > ${source}-${target}-${task_identifier}-val-cdc_tasks.txt
