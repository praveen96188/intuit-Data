...
def main():
    ...
    lock_users_sql_path = '/u01/scripts/stage_db_refreh/lock_users.sql'
    grants_sql_path = '/u01/scripts/stage_db_refreh/grants.sql'
    appuser_grants_sql_path = '/u01/scripts/stage_db_refreh/appuser_grants.sql'
    ...
    try:
        ...
        reset_password_with_psql(
            user='pspadm_owner',
            new_password=user_passwords['pspadm_owner'],
            db_host=db_host,
            db_name='psppdarc1'
        )
        ...
        # 9. Create other users and reset passwords
        other_users = [u for u in user_passwords.keys() if u != 'pspadm_owner']
        check_and_create_users(
            db_host=db_host,
            db_port=5432,
            db_name='psppdarc1',
            postgres='intuadmin',
            password_file_path=password_file_path,
            required_users=other_users
        )
        
        for user in other_users:
            reset_password_with_psql(
                user=user,
                new_password=user_passwords[user],
                db_host=db_host,
                db_name='psppdarc1'
            )
