...
def manage_psparc_owner(db_host, db_name, master_user, db_password, new_password):
    try:
        ...
        run_psql_password(db_host, db_name, master_user, 'psparc_owner', db_password, new_password)
    except Exception as e:
...
# Check the value of new_db_name and configure paths accordingly
    ...
    try:
        ...
        if rename_success and database_exists(db_host, master_user, intuadmin_password, new_db_name):
            ...
            manage_psparc_owner(db_host, def_db_name, master_user, intuadmin_password, pspadm_password)
        else:
            print("Skipping SQL execution and user management due to earlier errors.")

        # STEP 2: Execute psparc_owner SQL file
            print(f"Checking if user 'psparc_owner' can connect to database '{new_db_name}'...")
        if database_exists(db_host, 'psparc_owner', pspadm_password, new_db_name):
            ...
            try:
                execute_sql_file(db_host, new_db_name, 'psparc_owner', pspadm_password, pspadm_sql_file)
            except Exception as e:
