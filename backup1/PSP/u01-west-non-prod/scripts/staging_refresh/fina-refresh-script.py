...
def kill_active_connections(db_host, db_name, master_user, db_password):
    try:
        conn = psycopg2.connect(host=db_host, database=db_name, user=master_user, password=db_password, connect_timeout=10)
...
def run_psql_password(db_host, db_name, master_user, target_user, old_password, new_password):
    try:
        child = pexpect.spawn(
            '/usr/bin/psql',
...
def drop_replication_slots(db_host, master_user, db_password):
    try:
        conn = psycopg2.connect(host=db_host, database='postgres', user=master_user, password=db_password, connect_timeout=10)
        conn.autocommit = True
        cur = conn.cursor()
...
def database_exists(db_host, master_user, db_password, db_name):
    try:
        conn = psycopg2.connect(host=db_host, user=master_user, password=db_password, dbname='postgres', connect_timeout=10)
        cur = conn.cursor()
...
def manage_app_users(db_host, db_name, admin_user, admin_password, user_password_file):
    ...
    for user, password in user_passwords.items():
        try:
            conn = psycopg2.connect(host=db_host, database=db_name, user=admin_user, password=admin_password, connect_timeout=10)
            conn.autocommit = True
            cur = conn.cursor()
            ...
            run_psql_password(db_host, db_name, admin_user, user, admin_password, password)
...
# Check the value of new_db_name and configure paths accordingly
    if new_db_name == 'pitparmo':
        ...
        pitparmo_sql_file = '/u01/scripts/staging_refresh/mon_pitparmo_postgres.sql'
        vacuum_script_path = '/u01/scripts/staging_refresh/vacuum/vacuum_pitparmo.sh'
        ...
        sql_file = '/u01/scripts/staging_refresh/au_postgres_postgres.sql'
        pitparmo_sql_file = '/u01/scripts/staging_refresh/au_paragib_postgres.sql'
        pspadm_sql_file = '/u01/scripts/staging_refresh/ar_ibobadmowner_paragib.sql'
        def_db_name = 'postgres'
        master_user = 'postgres'
    ...
    print(f"✏️ STAGING DATABASE CONFIGURATION FOR: {new_db_name}")
    print(f"✏️ OLD PASSWORDFILE PATH: {password_file_path}")
    print(f"✏️ NEW PASSWORDFILEPATH: {new_password_file_path}")
    print(f"✏️ APPUSERS PASSWORD FILE PATH: {appusers_file_path}")
    print(f"✏️ SQL SCRIPTS-1: {sql_file}")
    print(f"✏️ SQL SCRIPTS-2: {pitparmo_sql_file}")
    print(f"✏️ SQL SCRIPTS-3: {pspadm_sql_file}")
    print(f"✏️ DEFAULT DBNAME: {def_db_name}")
    print(f"✏️ MASTER USER: {master_user}")
    print(f"✏️ SCHEMA OWNER: {schema_owner}")
    ...
    if new_db_name in [ 'pitparmo', 'paragib', 'psparcdb']: 
     ...
     try:
      ...
      # new_db_cluster_identifier = f"{stage_cluster_identifier.lower().replace('_', '-')}-new".strip('-')
        ...
        print("\n🟢=== STEP 2: Master User and Schema_owner Password Reseting===")
        if current_password and intuadmin_password:
            try:
                run_psql_password(db_host, db_name, master_user, master_user, current_password, intuadmin_password)
               #run_psql_password(db_host, db_name, master_user, schema_owner, current_password, pspadm_password)
                print("\n🟢=== STEP 3: Drop Rep slots and kill connections ===")
                drop_replication_slots(db_host, master_user, intuadmin_password)
                kill_active_connections(db_host, db_name, master_user, intuadmin_password)
                print("\n🟢=== STEP 4: Renaming Database===")
                rename_database(db_host, master_user, intuadmin_password, old_name=db_name, new_name=new_db_name)
                rename_success = True
            except Exception as e:
                print(f"🔴Error during password reset/rename: {e}")

        print("\n🟢=== STEP 5:Managing app users and reseting app users passwords===")
        manage_app_users(db_host, new_db_name, master_user, intuadmin_password, appusers_file_path)

        print("\n🟢=== STEP 6: Executing Scripts cnnecting to master-user and postgres Database===")
        if rename_success and database_exists(db_host, master_user, intuadmin_password, new_db_name):
            execute_sql_file(db_host, def_db_name, master_user, intuadmin_password, sql_file)
        else:
            print("🔴Skipping SQL execution and user management due to earlier errors.")
        
        print("\n🟢=== STEP 7: Executing Scripts cnnecting to masterUser and Database===")
        try:
            execute_sql_file(db_host, new_db_name, master_user, intuadmin_password, pitparmo_sql_file)
        except Exception as e:
            print(f"🔴Error executing psparcdb_postgres.sql: {e}")

        print("\n🟢=== STEP 8: Executing Scripts cnnecting to masterUser and Database===")
        if database_exists(db_host, schema_owner, pspadm_password, new_db_name):
            try:
                execute_sql_file(db_host, new_db_name, schema_owner, pspadm_password, pspadm_sql_file)
            except Exception as e:
                print(f"🔴Error executing SQL file as : {e}")
        else:
            print("Connection failed for user '{schema_owner}'. Skipping SQL execution.")
        
        # 9. Add AWS backup tag
        print("\n🟢=== STEP 9: Adding AWS backup tag===")
        def get_account_id():
       ...
       ##update port
        if new_db_name == 'paragib':
             print("\n🟢Port number Updating from 5432 to 6543 - auditDB ")
        ...
        if new_db_name == 'pitparmo':
          ...
          print(f"✅ VACUUM completed successfully. Log saved to {log_path}")
            ...
            print("\n🟢 vaccum not required for audit and monolith")
