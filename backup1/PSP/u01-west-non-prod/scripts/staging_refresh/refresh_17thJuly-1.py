...
def manage_schema_owner(db_host, db_name, master_user, db_password, new_password):
    try:
        conn = psycopg2.connect(host=db_host, database=db_name, user=master_user, password=db_password, connect_timeout=10)
        conn.autocommit = True
        cur = conn.cursor()
        cur.execute("SELECT 1 FROM pg_roles WHERE rolname=%s;", (schema_owner,))
        if cur.fetchone():
        ...
        run_psql_password(db_host, db_name, master_user, schema_owner, db_password, new_password)
    except Exception as e:
        print(f"Error managing {schema_owner} user: {e}")
...
# Check the value of new_db_name and configure paths accordingly
    ...
    try:
        ...
        if rename_success and database_exists(db_host, master_user, intuadmin_password, new_db_name):
            ...
            manage_schema_owner(db_host, def_db_name, master_user, intuadmin_password, pspadm_password)
        else:
       ...
       # STEP 3: Manage app users
        ...
        print("\n=== STEP 3: Executing psparcdb_postgres.sql ===")
        try:
