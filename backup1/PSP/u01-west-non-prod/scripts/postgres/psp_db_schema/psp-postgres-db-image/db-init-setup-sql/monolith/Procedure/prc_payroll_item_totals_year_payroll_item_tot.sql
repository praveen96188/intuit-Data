/******************************************
     NAME:       PRC_YEAR_PAYROLL_ITEM_TOT
     PURPOSE:    To calculate payroll item totals for W2 data, this is for all companies and one year (all 4 quarters)
******************************************/

CREATE OR REPLACE PROCEDURE prc_payroll_item_totals_year_payroll_item_tot (
    p_year               IN   VARCHAR -- Quarter start date in UTC
)
    LANGUAGE plpgsql AS
$$
DECLARE
...
WHERE
        eePayrollItemQtrTotals.year = cast(p_year as numeric);
...
select gen_random_uuid(), 1, 'PK_PAYROLL_ITEM_TOTALS-INS', v_utc_date,
       v_utc_date, -1, cast(src.qtr as numeric), cast(p_year as numeric),
       src.amount, 0, 0, 0,
...
from  (select company_payroll_item_seq, source_employee_fk, sum(amount) amount, qtr
       from ((select pcpi.company_payroll_item_seq company_payroll_item_seq, pcheck.source_employee_fk source_employee_fk, sum(pcomp.compensation_amount) amount, cast(EXTRACT(QUARTER FROM pr.paycheck_date) as varchar) qtr
              ...
              where
                      cast(EXTRACT(YEAR FROM pr.paycheck_date) as varchar) = p_year -- :year
              ...
              group by
                  pcpi.company_payroll_item_seq, pcheck.source_employee_fk, cast(EXTRACT(QUARTER FROM pr.paycheck_date) as varchar))
             union all
             (select pcpi.company_payroll_item_seq company_payroll_item_seq , pcheck.source_employee_fk source_employee_fk, sum(pded.deduction_amount) amount, cast(EXTRACT(QUARTER FROM pr.paycheck_date) as varchar) qtr
              ...
              group by
                  pcpi.company_payroll_item_seq, pcheck.source_employee_fk, cast(EXTRACT(QUARTER FROM pr.paycheck_date) as varchar))
             union all
             (select pcpi.company_payroll_item_seq company_payroll_item_seq , pcheck.source_employee_fk source_employee_fk, sum(pec.contribution_amount) amount, cast(EXTRACT(QUARTER FROM pr.paycheck_date) as varchar) qtr
             ...
             (select pcpi.company_payroll_item_seq company_payroll_item_seq , ptrans.employee_fk source_employee_fk, sum(ptransline.amount * (case when pitem.payroll_item_type = 'Deduction' then -1
                                                                                                                                                   ...
                                                                                                                                                   else 1 end)) amount, cast(EXTRACT(QUARTER FROM ptrans.period_end_date) as varchar) qtr
              ...
              where
                      cast(EXTRACT(YEAR FROM ptrans.period_end_date) as varchar) = p_year-- :year
                ...
                and pcpi.tax_form_line in ('ALLOCTIPS', 'DPDNTCARE', 'DPDNTCARECO', 'NONQUALPLAN', 'SEC457', 'ADOPTION',
                                           ...
                                           'TTT22','TTT23','TTT24','TTT25','TTT26','TTT27','TTT28','TTT29','TTT30','TTT31','TTT32','TTT33')  --(:taxFormLines)
              group by
                  pcpi.company_payroll_item_seq, ptrans.employee_fk, cast(EXTRACT(QUARTER FROM ptrans.period_end_date) as varchar))) AS derivedTable
       group by
