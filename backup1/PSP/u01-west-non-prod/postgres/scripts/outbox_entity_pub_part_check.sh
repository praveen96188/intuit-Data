...
if [ `cat ${logdir}/${cluster_postgres}_${db_name}__rep_slot_check.log |grep -i "0 rows" |wc -l` -ne 1 ]; then
/usr/bin/aws sns publish --topic-arn ${topic_arn} --subject "${the_subject3}" --message "`cat ${logdir}/${cluster_postgres}_${db_name}__rep_slot_check.log`"
fi
