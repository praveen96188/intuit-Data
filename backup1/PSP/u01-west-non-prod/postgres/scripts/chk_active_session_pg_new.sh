...
#    for instance_name1 in `echo ${instance_name} | awk '{print $1}'| rev | cut -c1- | rev`
 #   do
    ...
    aws cloudwatch put-metric-data --metric-name --region us-west-2  ${instance_name} --namespace "DatabaseActiveConnection" --value $chk_active_connection --timestamp `date +\%Y-%m-%dT\%H:%M:%S%z`
#done
done
