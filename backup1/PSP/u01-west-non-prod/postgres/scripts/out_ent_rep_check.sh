...
if [ "$row_count3" -ne 0 ] ; then
			/usr/bin/aws sns publish --topic-arn ${topic_arn} --region us-west-2  --subject "${the_subject3}" --message " Replication slot issue: Inactive Replica Slot Available = ${row_count3}"
			#/usr/bin/aws sns publish --topic-arn arn:aws:sns:us-west-2:152430470825:DB-sbg-psp-warning-ppd-a-intuit-com --region us-west-2  --subject "Test-subject" --message " Test Replication slot
fi
