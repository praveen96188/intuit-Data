...
do
    ...
    aws cloudwatch put-metric-data --region us-west-2 --metric-name ${instance_name} --namespace "DatabaseActiveConnection" --value $chk_active_connection --timestamp `date +\%Y-%m-%dT\%H:%M:%S%z`
done
