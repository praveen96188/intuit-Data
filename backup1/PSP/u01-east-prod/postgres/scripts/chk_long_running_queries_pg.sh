...
#pd_topic_arn="arn:aws:sns:us-east-2:893547637742:db-pspprod-slow-query"
#stg_topic_arn="arn:aws:sns:us-east-2:893547637742:psp-staging-slack"
#topic_arn="arn:aws:sns:us-east-2:893547637742:apg-db-sbg-psp-warning-stagin-a-intuit-com"
...
#cluster_instances= aws --region us-east-2 rds describe-db-clusters --db-cluster-identifier ${cluster_postgres} --query 'DBClusters[0].DBClusterMembers' | grep DBInstanceIdentifier | awk '{print $2}'|sed 's/"//'|sed 's/"//'
#echo ${cluster_instances}
for instance_name in `aws --region us-east-2 rds describe-db-clusters --db-cluster-identifier ${cluster_postgres} --query 'DBClusters[0].DBClusterMembers' | grep DBInstanceIdentifier | awk '{print $2}'|sed 's/"//'|sed 's/"//'`
do
    ./run_postgres_instance.sh ${instance_name} $db_name chk_instance_db_mode.sql "-v dbname=${db_name}" > ${logdir}/${instance_name}_${db_name}_chk_instance_db_mode.log
    if [ "`grep "WRITER" ${logdir}/${instance_name}_${db_name}_chk_instance_db_mode.log | wc -l`" -eq "1" ] ; then
        ...
        ./run_postgres_instance.sh ${instance_name} $db_name chk_long_running_queries_pg.sql "-v dbname=${db_name}" > ${logdir}/${instance_name}_${db_name}_chk_long_running_queries.log
        if [ `cat ${logdir}/${instance_name}_${db_name}_chk_long_running_queries.log |grep -i "0 rows" |wc -l` -ne 1 ]; then
            ...
            while read usename query_duration_secs; do
                ...
                if [[ $tr_query_duration_secs -ge 300 ]]; then
                    if [[ $usename == "psp"*"app" || $usename == "psp"*"read" || $usename == "ibob"*"app" ]]; then
                ...
                if [[ $tr_query_duration_secs -ge 10800 ]]; then
                    if [[ $usename == "pspbatch"*"user" || $usename == "psprjf" || $usename == "psp_payroll_dm"  ]]; then
                        the_subject="Long running query (> 180 Mins) - OLAP "
                    if [ "${db_name}" == "pitparmo" ] || [ "${db_name}" == "parapgib" ]; then
                        /usr/bin/aws sns publish --topic-arn ${topic_arn} --subject "${the_subject}" --message "`cat ${logdir}/${instance_name}_${db_name}_chk_long_running_queries.log`"
                    elif [ "${db_name}" == "pspapg02" ] || [ "${db_name}" == "prodapgib" ]; then
                        /usr/bin/aws sns publish --topic-arn ${topic_arn} --subject "${the_subject}" --message "`cat ${logdir}/${instance_name}_${db_name}_chk_long_running_queries.log`"
                ...
                if [[ $tr_query_duration_secs -ge 3600 ]]; then
                    if [[ $usename != "pspbatch"*"user" && $usename != "psprjf" && $usename != "psp_payroll_dm" && $usename != "psp"*"app" && $usename != "psp"*"read" && $usename != "ibob*app" ]]; then
        ...
        if [ `cat ${logdir}/${instance_name}_${db_name}_chk_long_running_queries.log |grep -i "0 rows" |wc -l` -ne 1 ]; then
            ...
            while read usename query_duration_secs; do        
                ...
                if [[ $tr_query_duration_secs -ge 10800 ]]; then
                    if [[ $usename == "pspbatch"*"user" || $usename == "psprjf" || $usename == "psp_payroll_dm" ]]; then
                        the_subject="Long running query (> 180 Mins) - OLAP on reader instance:  ${instance_name}"
                    if [ "${db_name}" == "pitparmo" ] || [ "${db_name}" == "parapgib" ]; then
                        /usr/bin/aws sns publish --topic-arn ${topic_arn} --subject "${the_subject}" --message "`cat ${logdir}/${instance_name}_${db_name}_chk_long_running_queries.log`"
                    elif [ "${db_name}" == "pspapg02" ] || [ "${db_name}" == "prodapgib" ]; then
                        /usr/bin/aws sns publish --topic-arn ${topic_arn} --subject "${the_subject}" --message "`cat ${logdir}/${instance_name}_${db_name}_chk_long_running_queries.log`"
                ...
                if [[ $tr_query_duration_secs -ge 3600 ]]; then
                    if [[ $usename != "pspbatch"*"user" && $usename != "psprjf" && $usename != "psp_payroll_dm" && $usename != "psp"*"app" && $usename != "psp"*"read" ]]; then
