...
def update_cron_jobs(db_host, db_port, new_db_name, postgres, password_file_path):
    ...
    try:
        ...
        # Update cron jobs
        try:
            cursor.execute(f"UPDATE cron.job SET database = '{new_db_name}' WHERE jobid = 73;")
            cursor.execute(f"UPDATE cron.job SET database = '{new_db_name}' WHERE jobid = 74;")
...
def main():
    ...
    appuser_password_file_path = '/u01/prvn/stg_refresh/appusers_password.txt'
    lock_users_sql_path = '/u01/prvn/stg_refresh/postgres_postgres.sql'
    grants_sql_path = '/u01/prvn/stg_refresh/pspadmowner_pitparmo.sql'
    appuser_grants_sql_path = '/u01/prvn/stg_refresh/postgres_pitparmo.sql'
    vacuum_script_path = '/u01/prvn/stg_refresh/vacuum_pitparmo-new.sh'
    analyze_script_path = '/u01/prvn/stg_refresh/analyze_pitparmo-new.sh'
