...
def create_staging_db_from_snapshot(rds_client, snapshot_identifier, stage_cluster_identifier, existing_stage_cluster_info):
    ...
    try:
        ...
        response = rds_client.restore_db_cluster_from_snapshot(
            ...
            DBClusterParameterGroupName=existing_stage_cluster_info['ClusterParameterGroup']  # ⬅️ NEW
        )
        ...
        writer_instance_response = rds_client.create_db_instance(
            ...
            DBParameterGroupName=existing_stage_cluster_info['InstanceParameterGroup'],  # ⬅️ NEW
            Tags=filtered_tags
        )
...
def reset_password_with_psql(user, password):
    ...
    new_db_cluster_identifier = 'ppsp-ppd-sqo-new'
   ...
   #             ['psql', '-h', db_host, '-d', 'pspsqodb', '-U', user, '-W'],
...
def main():
    ...
    stage_cluster_identifier = 'ppsp-ppd-sqo'  # Staging cluster identifie
    #new_db_cluster_identifier = 'ppsp-ppd-arc-new'
    password_file_path = '/u01/prvn/db_password.txt'  # Path to password file
    sql_file_path = '/u01/prvn/sql_file.sql'  # Path to the SQL file
    ...
    try:
        ...
        rename_db(
            ...
            new_db_name='pspsqodb',
            postgres='intuadmin',
            password_file_path=password_file_path
        )

        # Check and create the required users and reset their passwords
        required_users = ['psparc_owner','testusr','psparcapp','psp_prl_pspapp','ibob_prl_pspapp','ibobadm_readonly']
        check_and_create_users(
            db_host=rds_client.describe_db_clusters(DBClusterIdentifier=new_db_cluster_identifier)['DBClusters'][0]['Endpoint'],
            db_port=5432,
            db_name='pspsqodb',  # The newly renamed database
            postgres='intuadmin',  # The username to connect
            password_file_path=password_file_path,
   ...
   # Execute SQL commands from the file
        execute_sql_from_file(
            ...
            placeholders={'$new_db_name': 'pspsqodb'}
        )
