...
then
    ...
    echo "success:db-solo prod validation completed & out-of-sync report"
    /usr/bin/aws sns publish --region us-west-2 --topic-arn "arn:aws:sns:us-west-2:893547637742:dms-sbg-psp-oracle-exit-warning-prod-a-intuit-com" --subject "success:db-solo prod validation completed & out-of-sync report" --message "`cat results_PROD-AUD-MON-48hrs_${RUN_DATE}.sql`"
else
    echo "success:db-solo prod validation completed & no out-of-sync"
   /usr/bin/aws sns publish --region us-west-2 --topic-arn "arn:aws:sns:us-west-2:893547637742:dms-sbg-psp-oracle-exit-warning-prod-a-intuit-com" --subject "success: db-solo prod validation completed & no out-of-sync" --message "no out-of-sync records"
fi
