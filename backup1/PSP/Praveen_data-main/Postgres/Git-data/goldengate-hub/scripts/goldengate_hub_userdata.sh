...
export ACCOUNT_ID=`curl --noproxy "*" -s http://169.254.169.254/latest/dynamic/instance-identity/document | /usr/local/bin/jq -r '.accountId'`
...
if [ "$GoldenGateVolumeStatus" == "available" ]; then 
  PrivateIpAddress=`curl http://169.254.169.254/latest/meta-data/local-ipv4`
  if [ "$HA" == "yes" ]; then
...
then
  ...
  curl 169.254.169.254/latest/user-data >> /var/log/userdata.log
