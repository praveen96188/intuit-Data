...
do 
  ...
  # Get old AMI:
  old_ami=`aws --profile ${profile} --region ${region} cloudformation describe-stacks --stack-name $stack_id --query 'Stacks[].Parameters[?ParameterKey==\`AmiId\`'].ParameterValue |grep ami |cut -d\" -f2`
