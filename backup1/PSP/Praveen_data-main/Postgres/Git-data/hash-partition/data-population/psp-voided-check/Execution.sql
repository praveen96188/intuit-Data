DECLARE
    RECORD_COUNT       INTEGER   := 12871;
    MAX_RECORDS        INTEGER   := 12871;
    PROCESSED_RECORDS  INTEGER   := 0;
    ERROR_RECORDS      INTEGER   := 0;
    MAX_FETCH_RECORDS  INTEGER   := 5000;
    BATCH_COUNT        INTEGER   := 0;
    START_DATE         DATE := TO_DATE('2011-11-17', 'yyyy-mm-dd');
            ...
            WHILE (BATCH_COUNT > 0 AND PROCESSED_RECORDS < MAX_RECORDS)
                ...
                LOOP
                    BEGIN
                        ...
                        MERGE INTO PSPADM.PSP_VOIDED_CHECK c
                        using (SELECT vc.VOIDED_CHECK_SEQ, mmt.COMPANY_FK
                               ...
                               WHERE mmt.INITIATION_DATE between START_DATE and PARTITION_END_DATE
                                 AND mmt.MONEY_MOVEMENT_TRANSACTION_SEQ= vc.MONEY_MOVEMENT_TRANSACTION_FK
                                 AND vc.COMPANY_FK is null
                                 and ROWNUM <= MAX_FETCH_RECORDS) p
                        on (c.VOIDED_CHECK_SEQ = p.VOIDED_CHECK_SEQ)
                        when matched then
