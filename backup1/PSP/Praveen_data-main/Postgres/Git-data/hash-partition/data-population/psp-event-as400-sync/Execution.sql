DECLARE
    ...
    START_DATE         DATE := TO_DATE('2012-05-26', 'yyyy-mm-dd');
    END_DATE           DATE := TO_DATE('2023-03-01', 'yyyy-mm-dd');
    PARTITION_END_DATE DATE := START_DATE + 2;
BEGIN
    ...
    WHILE (START_DATE < END_DATE AND RECORD_COUNT > 0 AND PROCESSED_RECORDS < MAX_RECORDS)
        ...
        LOOP
            ...
            WHILE (BATCH_COUNT > 0 AND PROCESSED_RECORDS < MAX_RECORDS)
                ...
                LOOP
                    BEGIN
                        ...
                        MERGE INTO PSPADM.PSP_EVENT_AS400_SYNC c
                        using (SELECT /*+ parallel(8) */ci.EVENT_AS400_SYNC_SEQ, pi.COMPANY_FK
                               FROM PSPADM.PSP_EVENT_AS400_SYNC ci,
                                    PSPADM.PSP_COMPANY_EVENT pi
                               WHERE pi.EVENT_TIME_STAMP between START_DATE and PARTITION_END_DATE
                                 AND pi.COMPANY_EVENT_SEQ = ci.COMPANY_EVENT_FK
                                 AND ci.COMPANY_FK is null
                                 and ROWNUM <= MAX_FETCH_RECORDS) p
                        on (c.EVENT_AS400_SYNC_SEQ = p.EVENT_AS400_SYNC_SEQ)
                        when matched then
            ...
            PARTITION_END_DATE := START_DATE + 2;
