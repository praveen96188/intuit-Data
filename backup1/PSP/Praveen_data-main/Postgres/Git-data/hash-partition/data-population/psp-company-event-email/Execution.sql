DECLARE
    ...
    START_DATE         DATE := TO_DATE('2009-07-28', 'yyyy-mm-dd');
            ...
            WHILE (BATCH_COUNT > 0 AND PROCESSED_RECORDS < MAX_RECORDS)
                ...
                LOOP
                    BEGIN
                        ...
                        MERGE INTO PSPADM.PSP_COMPANY_EVENT_EMAIL c
                        using (SELECT /*+ parallel(8) */ci.COMPANY_EVENT_EMAIL_SEQ, pi.COMPANY_FK
                               FROM PSPADM.PSP_COMPANY_EVENT_EMAIL ci,
                        ...
                        on (c.COMPANY_EVENT_EMAIL_SEQ = p.COMPANY_EVENT_EMAIL_SEQ)
                        when matched then
