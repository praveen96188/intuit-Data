. /l/orcl
# Usage: ./stop_A_to_B_tasks.sh <Source_DBName> <Target_DBName>
...
if [ $# -eq 2 ]; then
  ...
  echo "# Usage: ./stop_A_to_B_tasks.sh <Source_DBName> <Target_DBName>"
...
# stop A=>B all tasks 
aws dms describe-replication-tasks --region us-west-2 --filter=Name=migration-type,Values=full-load-and-cdc,cdc |grep ReplicationTaskIdentifier|awk {'print $2'}|sed 's/\"//g; s/,//'|grep ^"$source-$target" > b_to_c_repl_tasks_to_stop.txt
cat b_to_c_repl_tasks_to_stop.txt
echo ""
...
if [ "$answer" == "y" ]; then
    ...
    done <b_to_c_repl_tasks_to_stop.txt 
fi