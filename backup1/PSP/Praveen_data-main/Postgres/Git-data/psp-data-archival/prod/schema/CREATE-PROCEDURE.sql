...
CREATE PROCEDURE pspadm.allocate_realm_lock$at(INOUT aws_params JSON, INOUT aws_session_env JSON)
AS 
$BODY$
DECLARE
    lockHandle CHARACTER VARYING(128);
    lockname TEXT;
    f$result TEXT;
BEGIN
    SELECT
        ...
        FROM json_to_record(aws_params)
            AS t (lockname TEXT);
    SELECT
        aws_oracle_ext.dbms_lock$allocate_unique(lockname, lockHandle)
        INTO lockHandle;
    f$result := lockHandle;
    allocate_realm_lock$at.aws_params := json_build_object('f$result', f$result);
    RETURN;
END;
$BODY$
LANGUAGE plpgsql;
...
COMMENT ON PROCEDURE pspadm.allocate_realm_lock$at(INOUT aws_params JSON, INOUT aws_session_env JSON)
     IS 'Belongs to: pspadm.allocate_realm_lock';



CREATE PROCEDURE pspadm.off_change_mast$prc_off_change_mast_asst(IN p_asst_jira_no TEXT)
AS 
$BODY$
DECLARE
    v_count DOUBLE PRECISION := 0;
    v_date TIMESTAMP(0) WITHOUT TIME ZONE := aws_oracle_ext.TO_DATE('03-MAR-2014');
    i RECORD;
BEGIN
    /* v_count number :=0; */
    /* v_date DATE := TO_DATE('03-MAR-2014'); */
    PERFORM PSPADM.OFF_CHANGE_MAST$Init();
    PERFORM aws_oracle_ext.setglobalvariable(proutinename => 'PSPADM.OFF_CHANGE_MAST', pvariable => 'g_asst_jira_no', pval => p_asst_jira_no);
    DELETE FROM pspadm.temp_a_prc_inc
        WHERE offering = 'OFFERING';
    COMMIT;
    RAISE DEBUG USING MESSAGE = '==========ASSISTED OFERING CHANGE STARTED===================';
    SELECT
        MAX(change_date)
        INTO STRICT v_date
        FROM pspadm.temp_a_prc_inc;
    RAISE DEBUG USING MESSAGE = CONCAT_WS('', 'CHANGE DATE for ASSISTED OFERING CHANGE IS===', v_date);

    IF v_date = aws_oracle_ext.TRUNC(aws_oracle_ext.SYSDATE()) + (1::NUMERIC || ' days')::INTERVAL THEN
        FOR i IN
        SELECT DISTINCT
            offering, new_offering
            FROM pspadm.temp_a_prc_inc
        LOOP
            RAISE DEBUG USING MESSAGE = CONCAT_WS('', 'OFERING CHANGE IS GOING FROM OLD OFFERING=====: ', i.offering);
            RAISE DEBUG USING MESSAGE = CONCAT_WS('', 'TO NEW OFFERING===> ', i.new_offering);
            /* dbms_output.put_line('CHECKINKG THE OFFERING COUNT IN PRODUCTION FOR OLD_OFFERING====> '||i.offering); */
            SELECT
                COUNT(co.company_fk)
                INTO STRICT v_count
                FROM pspadm.psp_company_offering AS co, pspadm.psp_offering AS o
                WHERE co.offering_fk = o.offering_seq AND co.company_fk IN (SELECT
                    c.company_seq
                    FROM pspadm.psp_company AS c
                    WHERE c.source_company_id IN (SELECT
                        tmp.psid
                        FROM pspadm.temp_a_prc_inc AS tmp
                        WHERE offering = i.offering AND new_offering = i.new_offering)) AND o.service_code = 'DirectDeposit' AND o.reporting_type = 'Tax' AND o.offering_code = i.offering;
            RAISE DEBUG USING MESSAGE = CONCAT_WS('', 'TOT NO OF OLD OFERING RECORD====', ' ', i.offering, '  ', '====THAT WILL BE UPD TO NEW OFFERING====>', ' ', i.new_offering, ' ', '====IN PRODUCTION WILL BE= ', v_count);
            RAISE DEBUG USING MESSAGE = '==========================================================================';
            /* OFFERING_CHANGE(i.offering,i.new_offering); */
            CALL pspadm.prc_offering_change(i.offering, i.new_offering, aws_oracle_ext.getglobalvariable(proutinename => 'PSPADM.OFF_CHANGE_MAST', pvariable => 'g_asst_jira_no', ptp => NULL::CHARACTER VARYING(30)));
        END LOOP;
        RAISE DEBUG USING MESSAGE = '==========ASSISTED OFERING CHANGE END============================';
        RAISE DEBUG USING MESSAGE = '==========ASSISTED OFERING CHANGE DATA CHECK START============================';
        ...
        SELECT DISTINCT
            ...
            FROM pspadm.temp_a_prc_inc
        LOOP
            RAISE DEBUG USING MESSAGE = CONCAT_WS('', '===###OFERING CHANGE CHECK IS GOING FOR====: ', i.offering);
            RAISE DEBUG USING MESSAGE = CONCAT_WS('', '====####OFERING CHANGE CHECK IS GOING FOR===####: ', i.new_offering);
            /* dbms_output.put_line('CHECK THE OFFERING COUNT IN PRODUCTION FOR OLD_OFFERING: '||i.offering); */
            SELECT
                COUNT(co.company_fk)
                INTO STRICT v_count
                FROM pspadm.psp_company_offering AS co, pspadm.psp_offering AS o
                WHERE co.offering_fk = o.offering_seq AND co.company_fk IN (SELECT
                    ...
                    WHERE c.source_company_id IN (SELECT
                        tmp.psid
                        FROM pspadm.temp_a_prc_inc AS tmp
                        WHERE offering = i.offering AND new_offering = i.new_offering)) AND o.service_code = 'DirectDeposit' AND o.reporting_type = 'Tax' AND o.offering_code = i.new_offering;
            RAISE DEBUG USING MESSAGE = CONCAT_WS('', '######OFERING COUNT OF PROD DATA  AFTER CHANGE IS DONE FOR#####: ', i.new_offering, ' is  ', v_count);
        END LOOP;
        RAISE DEBUG USING MESSAGE = '==========ASSISTED OFERING CHANGE DATA CHECK END============================';
    END IF;
END;
$BODY$
LANGUAGE plpgsql;
...
CREATE PROCEDURE pspadm.off_change_mast$prc_off_change_mast_dd(IN p_dd_jira_no TEXT)
AS 
$BODY$
DECLARE
    v_count DOUBLE PRECISION := 0;
    v_date TIMESTAMP(0) WITHOUT TIME ZONE := aws_oracle_ext.TO_DATE('03-MAR-2014');
    i RECORD;
/* v_jira varchar2(30):='jira-9394'; */
BEGIN
    /* v_count number :=0; */
    /* v_date DATE := TO_DATE('03-MAR-2014'); */
    /* v_jira varchar2(30):='jira-9394'; */
    PERFORM PSPADM.OFF_CHANGE_MAST$Init();
    PERFORM aws_oracle_ext.setglobalvariable(proutinename => 'PSPADM.OFF_CHANGE_MAST', pvariable => 'g_dd_jira_no', pval => p_dd_jira_no);
    DELETE FROM pspadm.temp_dd_prc_inc
        WHERE new_offering = 'NEW_OFFERING';
    /*
    ...
    commit
    */
    RAISE DEBUG USING MESSAGE = '==========DD OFERING CHANGE STARTED===================';
    SELECT
        MAX(change_date)
        INTO STRICT v_date
        FROM pspadm.temp_dd_prc_inc;
    RAISE DEBUG USING MESSAGE = CONCAT_WS('', 'CHANGE DATE for DD OFERING CHANGE IS====', v_date);
    ...
    IF v_date = aws_oracle_ext.TRUNC(aws_oracle_ext.SYSDATE()) + (1::NUMERIC || ' days')::INTERVAL THEN
        ...
        LOOP
            ...
            SELECT
                ...
                WHERE co.offering_fk = o.offering_seq AND co.company_fk IN (SELECT
                    ...
                    WHERE c.source_company_id IN (SELECT
                        tmp.psid
                        FROM pspadm.temp_dd_prc_inc AS tmp
                        WHERE offering = i.offering AND new_offering = i.new_offering)) AND o.service_code = 'DirectDeposit' AND o.reporting_type = 'DirectDeposit' AND o.offering_code = i.offering;
            RAISE DEBUG USING MESSAGE = CONCAT_WS('', 'TOT NO OF OLD OFERING RECORD====', ' ', i.offering, '  ', '====THAT WILL BE UPD TO NEW OFFERING====>', ' ', i.new_offering, ' ', '====IN PRODUCTION WILL BE= ', v_count);
            RAISE DEBUG USING MESSAGE = '==========================================================================';
            /* OFFERING_CHANGE(i.offering,i.new_offering); */
            CALL pspadm.prc_offering_change_dd(i.offering, i.new_offering, aws_oracle_ext.getglobalvariable(proutinename => 'PSPADM.OFF_CHANGE_MAST', pvariable => 'g_dd_jira_no', ptp => NULL::CHARACTER VARYING(30)));
        END LOOP;
        RAISE DEBUG USING MESSAGE = '==========DD OFERING CHANGE END============================';
        RAISE DEBUG USING MESSAGE = '==========DD OFERING CHANGE DATA CHECK START============================';
        ...
        SELECT DISTINCT
            ...
            FROM pspadm.temp_dd_prc_inc
        LOOP
            RAISE DEBUG USING MESSAGE = CONCAT_WS('', '####OFERING CHANGE CHECK IS GOING FOR DD ####: ', i.offering);
            RAISE DEBUG USING MESSAGE = CONCAT_WS('', '#######OFERING CHANGE CHECK IS GOING FOR  DD#####: ', i.new_offering);
            /* dbms_output.put_line('CHECK THE OFFERING COUNT IN PRODUCTION FOR OLD_OFFERING: '||i.offering); */
            SELECT
                COUNT(co.company_fk)
                INTO STRICT v_count
                ...
                WHERE co.offering_fk = o.offering_seq AND co.company_fk IN (SELECT
                    ...
                    WHERE c.source_company_id IN (SELECT
                        tmp.psid
                        FROM pspadm.temp_dd_prc_inc AS tmp
                        WHERE offering = i.offering AND new_offering = i.new_offering)) AND o.service_code = 'DirectDeposit' AND o.reporting_type = 'DirectDeposit' AND o.offering_code = i.new_offering;
            RAISE DEBUG USING MESSAGE = CONCAT_WS('', '####OFERING COUNT OF PROD DATA  AFTER CHANGE IS DONE FOR####: ', i.new_offering, ' is  ', v_count);
        END LOOP;
        RAISE DEBUG USING MESSAGE = '==========DD OFERING CHANGE DATA CHECK END============================';
    END IF;
    EXCEPTION
        WHEN others THEN
...
CREATE PROCEDURE pspadm.off_change_mast$prc_off_change_mast_dd4v(IN p_dd4v_jira_no TEXT)
AS 
$BODY$
DECLARE
    v_count DOUBLE PRECISION := 0;
...
BEGIN
    /* v_jira varchar2(30):='jira-9394'; */
    PERFORM PSPADM.OFF_CHANGE_MAST$Init();
    PERFORM aws_oracle_ext.setglobalvariable(proutinename => 'PSPADM.OFF_CHANGE_MAST', pvariable => 'g_dd4v_jira_no', pval => p_dd4v_jira_no);
    DELETE FROM pspadm.temp_dd4v_prc_inc
        WHERE new_offering = 'NEW_OFFERING';
    /*
    [5035 - Severity CRITICAL - A transaction cannot be ended inside a block with exception handlers. Revise your code to try move transaction control on side of application.]
    commit
    */
    RAISE DEBUG USING MESSAGE = '==========DD4V OFERING CHANGE STARTED===================';
    SELECT
        MAX(change_date)
        INTO STRICT v_date
        FROM pspadm.temp_dd4v_prc_inc;
    RAISE DEBUG USING MESSAGE = CONCAT_WS('', 'CHANGE DATE for DD4V OFERING CHANGE IS===', v_date);
        ...
        LOOP
            ...
            SELECT
                ...
                WHERE co.offering_fk = o.offering_seq AND co.company_fk IN (SELECT
                    ...
                    WHERE c.source_company_id IN (SELECT
                        tmp.psid
                        FROM pspadm.temp_dd4v_prc_inc AS tmp
                        WHERE offering = i.offering AND new_offering = i.new_offering)) AND o.service_code = 'BillPayment' AND o.reporting_type = 'DirectDeposit' AND o.offering_code = i.offering;
            RAISE DEBUG USING MESSAGE = CONCAT_WS('', 'TOT NO OF OLD OFERING RECORD====', ' ', i.offering, '  ', '====THAT WILL BE UPD TO NEW OFFERING====>', ' ', i.new_offering, ' ', '====IN PRODUCTION WILL BE= ', v_count);
            RAISE DEBUG USING MESSAGE = '==========================================================================';
            /* OFFERING_CHANGE(i.offering,i.new_offering); */
            CALL pspadm.prc_offering_change_billpay(i.offering, i.new_offering, aws_oracle_ext.getglobalvariable(proutinename => 'PSPADM.OFF_CHANGE_MAST', pvariable => 'g_dd4v_jira_no', ptp => NULL::CHARACTER VARYING(30)));
        END LOOP;
        RAISE DEBUG USING MESSAGE = '==========DD4V OFERING CHANGE END============================';
        RAISE DEBUG USING MESSAGE = '=======####DD4V OFERING CHANGE DATA CHECK START#####=====================';
            ...
            SELECT
                ...
                WHERE co.offering_fk = o.offering_seq AND co.company_fk IN (SELECT
                    ...
                    WHERE c.source_company_id IN (SELECT
                        tmp.psid
                        FROM pspadm.temp_dd4v_prc_inc AS tmp
                        WHERE offering = i.offering AND new_offering = i.new_offering)) AND o.service_code = 'BillPayment' AND o.reporting_type = 'DirectDeposit' AND o.offering_code = i.new_offering;
            RAISE DEBUG USING MESSAGE = CONCAT_WS('', 'OFERING COUNT OF PROD DATA  AFTER CHANGE IS DONE FOR: ', i.new_offering, ' is  ', v_count);
        END LOOP;
        RAISE DEBUG USING MESSAGE = '==========DD4V OFERING CHANGE DATA CHECK END============================';
    END IF;
    EXCEPTION
        WHEN others THEN
            RAISE DEBUG USING MESSAGE = SQLERRM::TEXT;
...
CREATE PROCEDURE pspadm.prc_offering_change(IN v_old_offer TEXT, IN v_new_offer1 TEXT, IN v_change_req TEXT)
AS 
$BODY$
DECLARE
    tmpVar DOUBLE PRECISION;
    v_ce_seq CHARACTER VARYING(100);
    v_ce_ins_cnt DOUBLE PRECISION := 0;
    v_ced_ins_cnt DOUBLE PRECISION := 0;
    v_new_offering_seq CHARACTER VARYING(100);
    v_new_offering_code CHARACTER VARYING(100);
    /*
    *****************************************************************************
       ...
       Ver        Date        Author           Description
       ---------  ----------  ---------------  ------------------------------------
       1.0        9/28/2015   Swarnashis.Sarkar       1. Created this procedure.
    ...
    *****************************************************************************
    */
    v_error_code DOUBLE PRECISION;
    v_error_message CHARACTER VARYING(1000) := NULL;
    vr_sqlerrm CHARACTER VARYING(1000) := NULL;
    v_step CHARACTER VARYING(200) := 'before begin';
    v_cur CURSOR FOR
    SELECT
        co.company_fk, co.company_offering_seq, o.offering_code, o.s_k_u
        FROM pspadm.psp_company_offering AS co, pspadm.psp_offering AS o
        WHERE co.offering_fk = o.offering_seq AND co.company_fk IN (SELECT
            c.company_seq
            FROM pspadm.psp_company AS c
            WHERE c.source_company_id IN (SELECT
                tmp.psid
                FROM pspadm.temp_a_prc_inc AS tmp
                WHERE offering = v_old_offer AND new_offering = v_new_offer1)) AND o.service_code = 'DirectDeposit' AND o.reporting_type = 'Tax' AND o.offering_code = v_old_offer;
    v_cur$ATTRIBUTES aws_oracle_data.TCursorAttributes := ROW (FALSE, NULL, NULL, NULL);
    aws$frmt_err_bcktrc CHARACTER VARYING(2000);
    sql$rowcount BIGINT;
BEGIN
    FOR i IN v_cur LOOP
        BEGIN
            SELECT
                o.offering_seq
                INTO STRICT v_new_offering_seq
                FROM pspadm.psp_offering AS o
                WHERE offering_code = (SELECT DISTINCT
                    new_offering
                    FROM pspadm.temp_a_prc_inc
                    WHERE offering = v_old_offer AND new_offering = v_new_offer1);
            SELECT
                o.offering_code
                INTO STRICT v_new_offering_code
                FROM pspadm.psp_offering AS o
                WHERE offering_code = (SELECT DISTINCT
                    new_offering
                    FROM pspadm.temp_a_prc_inc
            ...
            /*
            [5340 - Severity CRITICAL - PostgreSQL doesn't support the STANDARD.SYS_EXTRACT_UTC(TIMESTAMP_TZ_UNCONSTRAINED) function. Use suitable function or create user defined function.]
            update pspadm.psp_company_offering co set co.offering_fk= v_new_offering_seq,modifier_id=v_change_req,modified_date=sys_extract_utc(systimestamp)  where co.company_offering_seq=i.company_offering_seq
            */
            SELECT
                pspadm.fn_format_sysguid(aws_oracle_ext.sys_guid())
                INTO STRICT v_ce_seq;
            /*
            [5340 - Severity CRITICAL - PostgreSQL doesn't support the STANDARD.SYS_EXTRACT_UTC(TIMESTAMP_TZ_UNCONSTRAINED) function. Use suitable function or create user defined function., 5340 - Severity CRITICAL - PostgreSQL doesn't support the STANDARD.SYS_EXTRACT_UTC(TIMESTAMP_TZ_UNCONSTRAINED) function. Use suitable function or create user defined function., 5340 - Severity CRITICAL - PostgreSQL doesn't support the STANDARD.SYS_EXTRACT_UTC(TIMESTAMP_TZ_UNCONSTRAINED) function. Use suitable function or create user defined function., 5340 - Severity CRITICAL - PostgreSQL doesn't support the STANDARD.SYS_EXTRACT_UTC(TIMESTAMP_TZ_UNCONSTRAINED) function. Use suitable function or create user defined function.]
            Insert into PSPADM.PSP_COMPANY_EVENT
               (COMPANY_EVENT_SEQ,VERSION, CREATOR_ID, CREATED_DATE, MODIFIER_ID, MODIFIED_DATE,REALM_ID, EVENT_TIME_STAMP, STATUS_EFFECTIVE_DATE, STATUS_CD, EVENT_TYPE_CD,EVENT_TOKEN, SOURCE_ID, COMPANY_FK, NOTE_LAST_UPDATED_DATE)
             Values(v_ce_seq, 0, v_change_req,sys_extract_utc(systimestamp) ,v_change_req,sys_extract_utc(systimestamp) , -1, sys_extract_utc(systimestamp) , sys_extract_utc(systimestamp) , 'Active', 'OfferingUpdated', 0, NULL, i.company_fk, NULL)
            */
            ...
            Insert into PSPADM.PSP_COMPANY_EVENT_DETAIL
               (COMPANY_EVENT_DETAIL_SEQ,VERSION, CREATOR_ID, CREATED_DATE, MODIFIER_ID, MODIFIED_DATE,REALM_ID, VALUE, EVENT_DETAIL_TYPE_CD, COMPANY_EVENT_FK, EVENT_DETAIL_SUBTYPE,COMPANY_FK)
             Values(fn_format_sysguid(sys_guid()) , 0, v_change_req,sys_extract_utc(systimestamp) ,v_change_req,sys_extract_utc(systimestamp)  , -1,i.s_k_u , 'OldStringValue', V_CE_SEQ,NULL,I.COMPANY_FK)
            */
            GET DIAGNOSTICS sql$rowcount = ROW_COUNT;
            ...
            Insert into PSPADM.PSP_COMPANY_EVENT_DETAIL
               (COMPANY_EVENT_DETAIL_SEQ,VERSION, CREATOR_ID, CREATED_DATE, MODIFIER_ID, MODIFIED_DATE,REALM_ID, VALUE, EVENT_DETAIL_TYPE_CD, COMPANY_EVENT_FK, EVENT_DETAIL_SUBTYPE,COMPANY_FK)
             Values(fn_format_sysguid(sys_guid()) , 0, v_change_req,sys_extract_utc(systimestamp) ,v_change_req,sys_extract_utc(systimestamp)  , -1,v_new_offering_code, 'NewStringValue', V_CE_SEQ,NULL, I.COMPANY_FK)
            */
            GET DIAGNOSTICS sql$rowcount = ROW_COUNT;
            v_ced_ins_cnt := v_ced_ins_cnt + sql$rowcount;
            COMMIT;
            PERFORM aws_oracle_ext.dbms_application_info$set_action(action_name => CONCAT_WS('', 'Total processed count->', v_ce_ins_cnt, ' as of ->', aws_oracle_ext.TO_CHAR(aws_oracle_ext.SYSDATE(), 'HH:MI')));
        END;
    END LOOP;
    RAISE DEBUG USING MESSAGE = CONCAT_WS('', 'insereted ', v_ce_ins_cnt, ' events   ');
    RAISE DEBUG USING MESSAGE = CONCAT_WS('', 'insereted ', v_ced_ins_cnt, ' event_details   ');
    RAISE DEBUG USING MESSAGE = CONCAT_WS('', 'Old OFFERING IS: ', v_old_offer);
    RAISE DEBUG USING MESSAGE = CONCAT_WS('', 'NEW OFFERING IS : ', v_new_offering_code);
    /* end; */
    EXCEPTION
        WHEN no_data_found THEN
            /* dbms_output.put_line('No Data Found excep-: '||sqlerrm); */
            GET STACKED DIAGNOSTICS aws$frmt_err_bcktrc = PG_CONTEXT;
            vr_sqlerrm := aws_oracle_ext.substr(CONCAT_WS('', SQLERRM, aws$frmt_err_bcktrc), 0, 999);
            RAISE USING hint = -20002, message = vr_sqlerrm, detail = 'User-defined exception';
        WHEN others THEN
            v_error_code := SQLSTATE;
            v_error_message := SQLERRM;
            GET STACKED DIAGNOSTICS aws$frmt_err_bcktrc = PG_CONTEXT;
            vr_sqlerrm := aws_oracle_ext.substr(CONCAT_WS('', SQLERRM, aws$frmt_err_bcktrc), 0, 999);
            /* INSERT INTO list_errors VALUES ( v_error_code, vr_sqlerrm); */
            /* INSERT INTO list_step_errors(v_step,v_error_code,v_error_message) VALUES (v_step, v_error_code, vr_sqlerrm); */
            RAISE USING hint = -20003, message = vr_sqlerrm, detail = 'User-defined exception';
            /* ROLLBACK; */
...
CREATE PROCEDURE pspadm.prc_offering_change_billpay(IN v_old_offer TEXT, IN v_new_offer1 TEXT, IN v_change_req TEXT)
AS 
$BODY$
DECLARE
    tmpVar DOUBLE PRECISION;
    ...
    SELECT
        ...
        WHERE co.offering_fk = o.offering_seq AND co.company_fk IN (SELECT
            ...
            WHERE c.source_company_id IN (SELECT
                tmp.psid
                FROM pspadm.temp_dd4v_prc_inc AS tmp
                WHERE offering = v_old_offer AND new_offering = v_new_offer1)) AND o.service_code = 'BillPayment' AND o.reporting_type = 'DirectDeposit' AND o.offering_code = v_old_offer;
    v_cur$ATTRIBUTES aws_oracle_data.TCursorAttributes := ROW (FALSE, NULL, NULL, NULL);
    aws$frmt_err_bcktrc CHARACTER VARYING(2000);
    sql$rowcount BIGINT;
BEGIN
...
CREATE PROCEDURE pspadm.prc_offering_change_dd(IN v_old_offer TEXT, IN v_new_offer1 TEXT, IN v_change_req TEXT)
AS 
$BODY$
DECLARE
    tmpVar DOUBLE PRECISION;
    ...
    SELECT
        ...
        WHERE co.offering_fk = o.offering_seq AND co.company_fk IN (SELECT
            ...
            WHERE c.source_company_id IN (SELECT
                tmp.psid
                FROM pspadm.temp_dd_prc_inc AS tmp
                WHERE offering = v_old_offer AND new_offering = v_new_offer1)) AND o.service_code = 'DirectDeposit' AND o.reporting_type = 'DirectDeposit' AND o.offering_code = v_old_offer;
    v_cur$ATTRIBUTES aws_oracle_data.TCursorAttributes := ROW (FALSE, NULL, NULL, NULL);
    aws$frmt_err_bcktrc CHARACTER VARYING(2000);
    sql$rowcount BIGINT;
BEGIN
...
--    parent_constraint_collection UserConstraints%TYPE;
    child_constraint_collection c1%TYPE;
    parent_constraint_collection c1%TYPE;
...
BEGIN
    ...
    DELETE FROM pspadm.psp_smsmigration
        WHERE company_fk IN (uniqueid);
...
CREATE PROCEDURE pspadm.tmp_prc_eftps_payments_sent(IN p_user_id TEXT, IN p_app_server_date TIMESTAMP WITHOUT TIME ZONE, IN p_payment_file_id DOUBLE PRECISION)
AS 
$BODY$
/* for audit purposes */
/* UTC Date */
