# Usage: ./create_rds.sh  <AppEnv> <DBType> <DBName> <Region> <VPC> <MultiAZ>
...
#              <MultiAZ> should be true or false
...
#  multi_az=$6
  ...
  echo "Usage: ./create_rds.sh <AppEnv> <DBType> <DBName> <Region> <VPC> <MultiAZ>"
  ...
  echo "             <MultiAZ> should be true or false"
  exit 1
fi
...
security_group_id=`aws --profile ${profile} --region ${region} ec2 describe-security-groups --filters "Name=group-name,Values=${app_env_b}-${db_type}-${vpc}" --query 'SecurityGroups[*].{GroupId: GroupId}' |grep "GroupId" |cut -d'"' -f4`
...
# Get root id
root_id=`aws --profile ${profile} --region ${region} iam get-group --group-name "Admins" --query 'Group.Arn' |cut -d: -f5`
root_id=893547637742
query="Aliases[?AliasName==\`alias/rds-${db_name}\`].TargetKeyId"
kmskeyid=`aws --profile ${profile} --region ${region} kms list-aliases --query "$query" |grep '"' |cut -d'"' -f2`
...
if [ "$kmskeyid" != "" ]; then
  ...
  kmskeyid=`aws --profile ${profile} --region ${region} kms create-key --description "Used for RDS ${db_name}" --policy file://kms_policy_${vpc_env}.json --query "KeyMetadata.KeyId" |cut -d'"' -f2`

  echo kmskeyid=$kmskeyid

  aws --profile ${profile} --region ${region} kms create-alias --alias-name alias/rds-${db_name} --target-key-id $kmskeyid
  aws --profile ${profile} --region ${region} kms enable-key-rotation --key-id $kmskeyid
fi
...
echo "  { \"ParameterKey\": \"EngineVersion\", \"ParameterValue\": \"19.0.0.0.ru-2020-04.rur-2020-04.r1\" }," >> cf_rds_parameter_${app_env}_${db_type}-${db_name}-${vpc}.json
...
#echo "  { \"ParameterKey\": \"MultiAZ\", \"ParameterValue\": \"${multi_az}\" }," >> cf_rds_parameter_${app_env}_${db_type}-${db_name}-${vpc}.json
echo "  { \"ParameterKey\": \"DBParameterGroupName\", \"ParameterValue\": \"oracle-ee-19-psp-${app_env_b}-${db_type}-${vpc}\" }," >> cf_rds_parameter_${app_env}_${db_type}-${db_name}-${vpc}.json
...
echo "  { \"ParameterKey\": \"OptionGroupName\", \"ParameterValue\": \"oracle-ee-19-${app_env_b}-${db_type}-${vpc}\" }," >> cf_rds_parameter_${app_env}_${db_type}-${db_name}-${vpc}.json
...
aws --profile ${profile} --region ${region} cloudformation create-stack --stack-name rds-${db_name}-`date +\%Y\%m\%dT\%H\%M\%S` \
