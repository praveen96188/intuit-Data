...
do
   ...
   do
    ./run_postgres_instance.sh ${instance_name1} $db_name chk_instance_db_mode.sql "-v dbname=${db_name}" > ${logdir}/${instance_name}_${db_name}_chk_instance_db_mode.log
    if [ "`grep "WRITER" ${logdir}/${instance_name}_${db_name}_chk_instance_db_mode.log | wc -l`" -eq "2" ] ; then
        ...
        ./run_postgres_instance.sh ${instance_name1} $db_name chk_long_running_queries_pg.sql "-v dbname=${db_name}" > ${logdir}/${instance_name}_${db_name}_chk_long_running_queries.log
                if [ `cat ${logdir}/${instance_name}_${db_name}_chk_long_running_queries.log |grep -i "0 rows" |wc -l` -eq 1 ]; then
                        echo "good"
