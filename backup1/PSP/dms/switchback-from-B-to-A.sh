. /l/orcl
# Usage: ./switchover-from-B-to-A.sh.sh <Source_DBName> <Target_DBName>
...
if [ $# -eq 2 ]; then
  ...
  echo "# Usage: ./switchover-from-B-to-A.sh.sh <Source_DBName> <Target_DBName>"
...
# stop A=>B all tasks 
aws dms describe-replication-tasks --region us-west-2 --filter=Name=migration-type,Values=full-load-and-cdc,cdc |grep ReplicationTaskIdentifier|awk {'print $2'}|sed 's/\"//g; s/,//'|grep ^"$source-$target"|grep -v apg > b_to_a_repl_tasks_to_stop.txt
...
if [ "$answer" == "y" ]; then
    ...
    aws dms describe-replication-tasks --region us-west-2 --filter=Name=migration-type,Values=full-load-and-cdc,cdc |grep ReplicationTaskIdentifier|awk {'print $2'}|sed 's/\"//g; s/,//'|grep ^"$source-$target"|grep -v apg > b_to_a_repl_tasks_to_delete.txt
    cat b_to_a_repl_tasks_to_delete.txt
    ...
    if [ "$answer" == "y" ]; then
        while read line; do
            ...
            if [ $rep_tast_status = "stopped" ]; then
                ...
                rep_task_del_status=$(aws dms describe-replication-tasks --region us-west-2 --filter=Name=migration-type,Values=full-load-and-cdc,cdc |grep ReplicationTaskIdentifier|awk {'print $2'}|sed 's/\"//g; s/,//'|grep ^"$source-$target"|grep -v apg|wc -l)
                if [ $rep_task_del_status -ne 0 ]; then                
                    ...
                    do
                        ...
                        sleep 5
                        rep_task_del_status=$(aws dms describe-replication-tasks --region us-west-2 --filter=Name=migration-type,Values=full-load-and-cdc,cdc |grep ReplicationTaskIdentifier|awk {'print $2'}|sed 's/\"//g; s/,//'|grep ^"$source-$target"|grep -v apg|wc -l)
                        if [ $rep_task_del_status -eq $rep_task_del_status-1 ]; then
        ...
        done <b_to_a_repl_tasks_to_delete.txt 
    fi

# create new  cdc A=>B replication tasks

    # Switchover (A=>B)
    source_endpoint_arn=$(aws dms describe-endpoints --region us-west-2 --filter="Name=endpoint-id,Values=pspsysib-source" --query="Endpoints[0].EndpointArn" --output text); echo $source_endpoint_arn
    target_endpoint_arn=$(aws dms describe-endpoints --region us-west-2 --filter="Name=endpoint-id,Values=spsp-sys-db1-target" --query="Endpoints[0].EndpointArn" --output text); echo $target_endpoint_arn
    rep_instance_arn="arn:aws:dms:us-west-2:152430470825:rep:ZDXY4F3MPFQCZOQFI32GU7VJAG57KBZZ3XQAFGI"

    aws dms create-replication-task \
    --region us-west-2 --replication-task-identifier pspsysib-spspsysdb-ibobadm-qri-hcm-cdc \
    ...
    --migration-type cdc --table-mappings file://pspsysib-spspsysdb-ibobadm-qri-hcm-cdc-table-mappings.json \
    --replication-task-settings file://pspsysib-spspsysdb-ibobadm-qri-hcm-cdc-rep-settings.json 1> /dev/null
    sleep 5
    aws dms create-replication-task \
    --region us-west-2 --replication-task-identifier pspsysib-spspsysdb-ibobadm-smc-cdc \
    --source-endpoint-arn $source_endpoint_arn --target-endpoint-arn $target_endpoint_arn --replication-instance-arn $rep_instance_arn \
    --migration-type cdc --table-mappings file://pspsysib-spspsysdb-ibobadm-smc-cdc-table-mappings.json \
    --replication-task-settings file://pspsysib-spspsysdb-ibobadm-smc-cdc-rep-settings.json 1> /dev/null
    sleep 5
    aws dms create-replication-task \
    --region us-west-2 --replication-task-identifier pspsysib-spspsysdb-ibobadm-sst-cdc \
    --source-endpoint-arn $source_endpoint_arn --target-endpoint-arn $target_endpoint_arn --replication-instance-arn $rep_instance_arn \
    --migration-type cdc --table-mappings file://pspsysib-spspsysdb-ibobadm-sst-cdc-table-mappings.json \
    --replication-task-settings file://pspsysib-spspsysdb-ibobadm-sst-cdc-rep-settings.json 1> /dev/null
    sleep 5
