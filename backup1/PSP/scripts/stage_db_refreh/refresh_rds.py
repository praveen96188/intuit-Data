    ...
    try:
        ...
        for user in required_users:
            ...
            if cursor.fetchone():
                ...
                reset_password_with_psql(user, db_host)  # Pass db_host here
...
def reset_password_with_psql(user, password):
    ...
    # Try to spawn the psql process
    spawn env PGPASSWORD="cJ86WjLGoZheFG5NRo97gg.68lOu.--7" psql -h {db_host} -d psppdarcl -U intuadmin

    # Check for successful spawn by waiting for a prompt or error
    expect {{
     "intuadmin=#" {{
...
def execute_sql_from_file(db_host, db_port, db_name, postgres, password_file_path, sql_file_path):
    user_passwords = read_passwords_from_file(password_file_path)  # Read passwords from the file
    password = user_passwords.get(postgres, None)  # Get password for the user from the file
