SELECT current_database() shard_id,
       queryid sql_id,
       substring(query, 1, 90) AS sql_text,
       calls num_execs,
       round((total_exec_time/calls)::numeric, 3) AS avg_time,
       round((blk_read_time/calls)::numeric, 3) AS avg_pio_time,
       round((total_exec_time/calls)::numeric, 3)-round((blk_read_time/calls)::numeric, 3) avg_cpu_n_wait_time,
       ...
       round((max_exec_time/1000)::numeric, 2) AS max_exec_time,
       ...
       round((100 * (round((total_exec_time/calls)::numeric, 3)-round((blk_read_time/calls)::numeric, 3))/sum((round((total_exec_time/calls)::numeric, 3)-round((blk_read_time/calls)::numeric, 3))) OVER ())::numeric, 2) AS pct_cpu_n_wait_time
