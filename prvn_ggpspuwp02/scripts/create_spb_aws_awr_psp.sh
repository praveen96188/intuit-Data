...
# env file
source /l/orcl
sqlplus -s intuadmin/"xxx"@psppp001 @create_spb_aws_psp.sql $SQL_ID $SQL_PLAN_HASH_VLAUE $SOURCE_CLUSTER $TARGET_CLUSTER $log_file $BEGIN_SNAP $END_SNAP
...
if [ `egrep "Usage 1: sqlplus|ORA-" $log_dir/$log_file | grep -v "ORA-14552"|wc -l` -gt 0 ]; then
  ...
  echo "PERF-PSP: Successfully copied plan for $SQL_ID from $SOURCE_CLUSTER to $TARGET_CLUSTER for plan hash
