...
DECLARE
  ...
  MAX_RECORDS        INTEGER   := 35000000;
  PROCESSED_RECORDS  INTEGER   := 0;
  ERROR_RECORDS      INTEGER   := 0;
  MAX_FETCH_RECORDS  INTEGER   := 5000;
  BATCH_COUNT        INTEGER   := 0;
  START_DATE         DATE := TO_DATE('2019-01-01', 'yyyy-mm-dd');
  END_DATE           DATE := TO_DATE('2019-02-01', 'yyyy-mm-dd');
      ...
      WHILE (BATCH_COUNT > 0 AND PROCESSED_RECORDS < MAX_RECORDS)
        ...
        LOOP
          BEGIN
            ...
            WHERE T1.table_name ='PSP_PSTUB_PAY_ITEM_01_2019';
            COMMIT;
