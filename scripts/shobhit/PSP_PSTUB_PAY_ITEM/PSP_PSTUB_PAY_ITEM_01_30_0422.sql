...
DECLARE
  ...
  PROCESSED_RECORDS  INTEGER   := 22000263;
  ERROR_RECORDS      INTEGER   := 0;
  MAX_FETCH_RECORDS  INTEGER   := 5000;
  BATCH_COUNT        INTEGER   := 0;
  START_DATE         DATE := TO_DATE('2022-04-01', 'yyyy-mm-dd');
  END_DATE           DATE := TO_DATE('2022-05-01', 'yyyy-mm-dd');
  PARTITION_START_DATE DATE := END_DATE - 1/48;
BEGIN
      ...
      WHILE (BATCH_COUNT > 0 AND PROCESSED_RECORDS < MAX_RECORDS)
        ...
        LOOP
          BEGIN
            ...
            WHERE T1.table_name ='PSP_PSTUB_PAY_ITEM_01_30_0422';
            COMMIT; 
          ...
          END;
        END LOOP;
      END_DATE := END_DATE - 1/48;
      PARTITION_START_DATE := END_DATE - 1/48;
