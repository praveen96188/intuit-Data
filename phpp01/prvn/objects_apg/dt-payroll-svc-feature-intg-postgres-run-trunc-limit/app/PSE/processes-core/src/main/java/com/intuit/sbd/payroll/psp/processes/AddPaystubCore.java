 ...
 */
public class AddPaystubCore extends Process implements IProcess {
    ...
    public AddPaystubCore(Paycheck pPaycheck, Employee pEmployee, PaystubDTO pPaystubDTO) {
    ...
    public ProcessResult process() {
        ...
        // find or create employerinfo
        PstubEmployerInfo pstubEmployer = PaystubHelper.findOrCreatePstubEmployerInfo(mPaystubDTO.getEmployerInfoDTO());

        // find or create employeeinfo
        PstubEmployeeInfo pstubEmployee = PaystubHelper.findOrCreatePstubEmployeeInfo(mEmployee, mPaystubDTO.getEmployeeInfoDTO(), mEEFedExtra,
                mEEFedClaimDependents, mEEFedOtherIncome, mEEFedDeductions,  mEEStateExtra);

        Paystub paystub = Paystub.createPaystub(mPaycheck, mPaystubDTO.getModTS().intValue(), pstubEmployer, pstubEmployee, Boolean.FALSE);

        paystub.setGrossPay(mGrossPay);
        paystub.setCheckNumber(mPaystubDTO.getChkNum());
        paystub.setAdjNetPay(mAdjNetPay);
        paystub.setNetPay(mNetPay);
        paystub.setPayBeginDate(mPayBeginDate);
        paystub.setPaycheckDate(mPaycheckDate);
        paystub.setPayEndDate(mPayEndDate);
        paystub.setPreTaxDeductions(mPreTaxDeduct);
        paystub.setTaxes(mTax);
        paystub.setYTDTaxes(mYTDTax);
        paystub.setYTDAdjNetPay(mYTDAdjNetPay);
        paystub.setYTDGrossPay(mYTDGrossPay);
        paystub.setYTDNetPay(mYTDNetPay);
        paystub.setYTDPreTaxDeductions(mYTDPreTaxDeduct);

        Application.save(paystub);

        // create line items
        for (PstubDDItemDTO pstubDDItemDTO : mPaystubDTO.getDDItemDTOs()) {
            PaystubHelper.createPstubDDItem(paystub, pstubDDItemDTO);
        }

        for (PstubPaidTimeoffItemDTO pstubPaidTimeoffItemDTO : mPaystubDTO.getPaidTimeoffItemDTOs()) {
            PaystubHelper.createPstubPaidTimeoffItem(paystub, pstubPaidTimeoffItemDTO);
        }

        for (PstubPayItemDTO pstubPayItemDTO : mPaystubDTO.getPayItemDTOs()) {
            PaystubHelper.createPstubPayItem(paystub, pstubPayItemDTO);
        }
        ...
        for (PstubMsgDTO pstubMsgDTO : mPaystubDTO.getMsgDTOs()) {
            if (pstubMsgDTO.getType() == PstubMsgType.Company) {
                PaystubHelper.createPstubMsg(paystub, pstubMsgDTO, companyMsgSequence++);
            } else if (pstubMsgDTO.getType() == PstubMsgType.User) {
                PaystubHelper.createPstubMsg(paystub, pstubMsgDTO, userMsgSequence++);
            }
        }
        CompanyEvent.createPaystubCreatedEvent(mEmployee, paystub);

        processResult.setResult(paystub);
    ...
    public ProcessResult validate() {
        ...
        if (mPaystubDTO.getPstubEmployeeInfoDTO() == null) {
            validationResult.getMessages().InvalidValue(EntityName.Paystub, "", "EmployeeInfo", null);
        }
        */
