 ...
 */
public class ReinitiateBankAccountRandomDebitsCoreTests {
    @Before
    ...
    public void testNullSourceSystemId() {
        ...
        ProcessResult processResult = PayrollServices.companyManager.reinitiateBankAccountRandomDebits(null, "123272727", null);
    ...
    public void testNullCompany() {
        Application.beginUnitOfWork();
        ProcessResult processResult = PayrollServices.companyManager.reinitiateBankAccountRandomDebits(SourceSystemCode.QBOE, null, null);
    ...
    public void testInvalidCompany() {
        ...
        ProcessResult processResult = PayrollServices.companyManager.reinitiateBankAccountRandomDebits(SourceSystemCode.QBOE,
    ...
    /**
     * Test message 1102 - verification transactions have not been issued to the bank account yet
     */
    @Test
    public void testVerificationTransactionsNotOffloaded() {
        PayrollServices.beginUnitOfWork();
        // Load Company Data
        Company company = CompanyBankAccountDataLoader.loadCompany();
        PayrollServices.commitUnitOfWork();
        ...
        assertEquals("Messages size", 2, processResult.getMessages().size());
        Message errorMessage = processResult.getMessages().get(0);
        assertEquals("Error message code", "1102", errorMessage.getMessageCode());
        assertEquals("Error message", "Bank Account 123123 for company QBOE:123272727 cannot have random debits " +
                "reissued - verification transactions have not been issued to the bank account yet.",
                errorMessage.getMessage());
    }

    @Test
    public void testReinitiateRandomDebits_Success(){
        PayrollServices.beginUnitOfWork();
        // Load Company Data
        Company company = CompanyBankAccountDataLoader.loadCompany();
        PayrollServices.commitUnitOfWork();
        ...
        DomainEntitySet<FinancialTransaction> verificationTransactions = FinancialTransaction.
        ...
        Assert.assertEquals("Number of Verification Transactions: ", 2, verificationTransactions.size());
        Assert.assertEquals("Number of Verification Transactions Offloaded: ", 2, offLoadedTransactions.size());
    }
