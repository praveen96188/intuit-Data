...
public class AddLiabilityAdjustmentsCoreTests {
    ...
    private static final String NYRES = "56";
    private static final String NYNONRES = "57";
    ...
    public void testLiabilityAdjustmentsOnly() throws Exception {
        ...
        DomainEntitySet<CompanyEvent> ledgerEvent = CompanyEvent.findCompanyEvents(company, EventTypeCode.ManualLedgerEntry);
        assertEquals("Number of Manual ledger events should be zero.", 0, ledgerEvent.size());
        PayrollServices.rollbackUnitOfWork();
    }

    @Test
    public void testManualLedgerKeyingEvent() throws Exception {
        PayrollServices.beginUnitOfWork();
        PSPDate.setPSPTime(SpcfCalendar.createInstance(2011, 01, 05, SpcfTimeZone.getLocalTimeZone()));
        PayrollServices.commitUnitOfWork();
        ...
        liabilityAdjustmentDTOs.add(liabilityAdjustmentDTO);
        companyAdjustmentSubmissionDTO.setLiabilityAdjustmentDTOs(liabilityAdjustmentDTOs);
        companyAdjustmentSubmissionDTO.setMemo("Test Memo");
        ...
        PayrollServices.beginUnitOfWork();
        DomainEntitySet<CompanyEvent> ledgerEvent = CompanyEvent.findCompanyEvents(company, EventTypeCode.ManualLedgerEntry);
        assertEquals("Incorrect number of manual ledger events returned.", 1, ledgerEvent.size());
        assertEquals("Incorrect event note.", "Test Memo", ledgerEvent.get(0).getCompanyEventDetailValue(EventDetailTypeCode.NoteText));
        PayrollServices.rollbackUnitOfWork();
    }
    ...
    @Test
    public void testMultipleLiabilityAdjustments_Recall() throws Exception {
        ...
        PayrollServices.beginUnitOfWork();
        CompanyAdjustmentSubmission companyAdjustmentSubmission = Application.findById(CompanyAdjustmentSubmission.class, processResult.getResult().getId());
        String companyAdjustmentSubmissionId = companyAdjustmentSubmission.getId().toString();
        assertNotNull("Company Adjustment Submission", companyAdjustmentSubmission);
        DomainEntitySet<LiabilityAdjustment> liabilityAdjustments = companyAdjustmentSubmission.getLiabilityAdjustmentCollection();
        assertEquals("Liability Adjustments", 4, liabilityAdjustments.size());
        ...
        // Now recall the adjustment
        PayrollServices.beginUnitOfWork();
        Collection<String> casIds = new ArrayList<String>();
        casIds.add(companyAdjustmentSubmissionId);
        ProcessResult recallProcessResult = PayrollServices.payrollManager
        ...
        debitTransactions = payrollRun.getFinancialTransactionCollection().find(FinancialTransaction.TransactionType().TransactionTypeCd().equalTo(TransactionTypeCode.EmployerTaxDebit));
        assertEquals("Debit Transaction Count", 1, debitTransactions.size());
        debitTransaction = debitTransactions.get(0);
        assertEquals("Debit Amount", "337.67", debitTransaction.getFinancialTransactionAmount().toString());
        assertEquals("Debit Transaction state", debitTransaction.getCurrentTransactionState(), TransactionState.findTransactionState(TransactionStateCode.Cancelled));
        taxPaymentTransactions = payrollRun.getFinancialTransactionCollection().find(FinancialTransaction.TransactionType().TransactionTypeCd().equalTo(TransactionTypeCode.AgencyTaxCredit));
        assertEquals("Agency Tax Credit Transaction Count", 4, taxPaymentTransactions.size());
        for (FinancialTransaction taxPaymentTransaction : taxPaymentTransactions) {
            assertEquals("Agency Tax Credit state", taxPaymentTransaction.getCurrentTransactionState(), TransactionState.findTransactionState(TransactionStateCode.Cancelled));
        }
        PayrollServices.rollbackUnitOfWork();
    }

    @Test
    public void testMultipleLiabilityAdjustments_WithCOBRA() throws Exception {
        ...
        liabilityAdjustmentDTO = DataLoadServices.createLiabilityAdjustmentDTO(Medicare, Medicare, null, new DateDTO(PSPDate.getPSPTime()), new SpcfMoney("10.09"), new SpcfMoney("0.0"), new SpcfMoney("0.0"), false);
        liabilityAdjustmentDTOs.add(liabilityAdjustmentDTO);
        liabilityAdjustmentDTO = DataLoadServices.createLiabilityAdjustmentDTO(COBRA, COBRA, null, new DateDTO(PSPDate.getPSPTime()), new SpcfMoney("-100.09"), new SpcfMoney("0.0"), new SpcfMoney("0.0"), false);
        liabilityAdjustmentDTOs.add(liabilityAdjustmentDTO);
        liabilityAdjustmentDTO = DataLoadServices.createLiabilityAdjustmentDTO(FUTA, FUTA, null, new DateDTO(PSPDate.getPSPTime()), new SpcfMoney("100.11"), new SpcfMoney("0.0"), new SpcfMoney("0.0"), false);
        liabilityAdjustmentDTOs.add(liabilityAdjustmentDTO);
        companyAdjustmentSubmissionDTO.setLiabilityAdjustmentDTOs(liabilityAdjustmentDTOs);
        ...
        for (LiabilityAdjustment liabilityAdjustment : companyAdjustmentSubmission.getLiabilityAdjustmentCollection()) {
            ...
            if (liabilityAdjustment.getLaw().isCOBRA()) {
                assertEquals("Amount", "-100.09", liabilityAdjustment.getAmount().toString());
            }
            assertEquals("Taxable Wages", "0.00", liabilityAdjustment.getTaxableWages().toString());
            assertEquals("Total Wages", "0.00", liabilityAdjustment.getTotalWages().toString());
            if (payrollRun == null) {
        ...
        assertEquals("Debit Transaction Count", 1, debitTransactions.size());
        FinancialTransaction debitTransaction = debitTransactions.get(0);
        assertEquals("Debit Amount", "237.58", debitTransaction.getFinancialTransactionAmount().toString());
        DomainEntitySet<FinancialTransaction> taxPaymentTransactions = payrollRun.getFinancialTransactionCollection().find(FinancialTransaction.TransactionType().TransactionTypeCd().equalTo(TransactionTypeCode.AgencyTaxCredit));
        assertEquals("Agency Tax Credit Transaction Count", 4, taxPaymentTransactions.size());
        for (FinancialTransaction taxPaymentTransaction : taxPaymentTransactions) {
            if (taxPaymentTransaction.getLaw().isFIT()) {
                assertEquals("Amount", "27.20", taxPaymentTransaction.getFinancialTransactionAmount().toString());
                // Check MMT Amount for 941 - FIT + FICA + MED - COBRA
                PaymentTemplate paymentTemplate = PayrollServices.entityFinder.findById(PaymentTemplate.class, "IRS-941-PAYMENT");
                MoneyMovementTransaction mmtPayrollTaxPayment = paymentTemplate.getTaxPayment(payrollRun);
                assertEquals("MMT Amount", "137.47", mmtPayrollTaxPayment.getMoneyMovementTransactionAmount().toString());
            }
            if (taxPaymentTransaction.getLaw().isFIT()) {
                assertEquals("Amount", "27.20", taxPaymentTransaction.getFinancialTransactionAmount().toString());
            }
    ...
    @Test
    public void testAdjustmentsExceed100KNoOffload() throws Exception {
        ...
        CompanyAdjustmentSubmissionDTO companyAdjustmentSubmissionDTO = DataLoadServices.createCompanyAdjustmentSubmissionDTO("Adjust_1", new DateDTO("2011-01-24"));
        Collection<LiabilityAdjustmentDTO> liabilityAdjustmentDTOs = new ArrayList<LiabilityAdjustmentDTO>();
        LiabilityAdjustmentDTO liabilityAdjustmentDTO = DataLoadServices.createLiabilityAdjustmentDTO(FIT, FIT, null, new DateDTO("2011-01-24"), new SpcfMoney("35227.20"), new SpcfMoney("0.0"), new SpcfMoney("0.0"), false);
        liabilityAdjustmentDTOs.add(liabilityAdjustmentDTO);
        liabilityAdjustmentDTO = DataLoadServices.createLiabilityAdjustmentDTO(FICA, FICA, null, new DateDTO("2011-01-24"), new SpcfMoney("200.27"), new SpcfMoney("0.0"), new SpcfMoney("0.0"), false);
        liabilityAdjustmentDTOs.add(liabilityAdjustmentDTO);
        companyAdjustmentSubmissionDTO.setLiabilityAdjustmentDTOs(liabilityAdjustmentDTOs);
        ...
        DomainEntitySet<FinancialTransaction> debitTransactions = payrollRun.getFinancialTransactionCollection()
        ...
        FinancialTransaction debitTransaction = assertOne(payrollRun.getFinancialTransactionCollection().find(FinancialTransaction.TransactionType().TransactionTypeCd().equalTo(TransactionTypeCode.EmployerTaxDirectDebit)));
        assertEquals("Debit Amount", "35427.47", debitTransaction.getFinancialTransactionAmount().toString());

        DomainEntitySet<FinancialTransaction> taxPaymentTransactions = payrollRun.getFinancialTransactionCollection()
                                                                                 .find(FinancialTransaction.TransactionType().TransactionTypeCd().equalTo(TransactionTypeCode.AgencyTaxCredit)
                                                                                                           .And(FinancialTransaction.CurrentTransactionState().TransactionStateCd().in(TransactionStateCode.Created, TransactionStateCode.Executed, TransactionStateCode.Completed)));
        assertEquals("Agency Tax Credit Transaction Count", 0, taxPaymentTransactions.size());

        DomainEntitySet<FinancialTransaction> taxPaymentDirectTransactions = payrollRun.getFinancialTransactionCollection().find(FinancialTransaction.TransactionType().TransactionTypeCd().equalTo(TransactionTypeCode.AgencyDirectCredit));
        assertEquals("Agency Tax Credit Transaction Count", 2, taxPaymentDirectTransactions.size());

        assertEquals("Number of EFTPSDirectDebit MMTs", 3, Application.find(MoneyMovementTransaction.class, MoneyMovementTransaction.MoneyMovementPaymentMethod().equalTo(PaymentMethod.EFTPSDirectDebit)).size());
    ...
    @Test
    public void testAdjustmentsExceed100K() throws Exception {
        ...
        DomainEntitySet<FinancialTransaction> debitTransactions = payrollRun.getFinancialTransactionCollection().find(FinancialTransaction.TransactionType().TransactionTypeCd().equalTo(TransactionTypeCode.EmployerTaxDirectDebit));
        assertEquals("Debit Transaction Count", 1, debitTransactions.size());
        FinancialTransaction debitTransaction = debitTransactions.get(0);
        assertEquals("Debit Amount", "35427.47", debitTransaction.getFinancialTransactionAmount().toString());
        DomainEntitySet<FinancialTransaction> taxPaymentTransactions = payrollRun.getFinancialTransactionCollection().find(FinancialTransaction.TransactionType().TransactionTypeCd().equalTo(TransactionTypeCode.AgencyDirectCredit));
        assertEquals("Agency Tax Credit Transaction Count", 2, taxPaymentTransactions.size());

        // Tax transactions from the adjustments will not be be linked to the MMT created for the Tax transactions for Payroll 2, as Payroll1 and Payroll 2 are offloaded.
        payrollRun = PayrollRun.findPayrollRun(company, payrollRunDTO1.getPayrollTXBatchId());
        PaymentTemplate paymentTemplate = PayrollServices.entityFinder.findById(PaymentTemplate.class, "IRS-941-PAYMENT");
        MoneyMovementTransaction mmtPayrollTaxPayment = paymentTemplate.getTaxPayment(payrollRun);
        assertTrue("MMTs are same", !mmtPayrollTaxPayment.equals(taxPaymentTransactions.get(0).getMoneyMovementTransaction()));
        PayrollServices.rollbackUnitOfWork();
...
//        assertEquals("Debit Amount", "227.47", debitTransaction.getFinancialTransactionAmount().toString());
        ...
        assertEquals("Agency Tax Overpayment Transaction Count", 2, taxPaymentTransactions.size());
        for (FinancialTransaction taxPaymentTransaction : taxPaymentTransactions) {
            assertNotNull("Settlement Date", taxPaymentTransaction.getSettlementDate());
        }
        PayrollServices.rollbackUnitOfWork();
    ...
    @Test
    public void testNetPositiveLiabilityAdjustments_NoDebit() throws Exception {
    ...
    @Test
    public void testETDNullError() throws Exception {
        DataLoadServices.updateIRSPaymentTemplateSupportDate(SpcfCalendar.createInstance(2008, 1, 1));
        DataLoadServices.updateIRSPaymentTemplateSupportDate(SpcfCalendar.createInstance(2008, 1, 1));
        DataLoadServices.updatePaymentTemplateSupportedDate("NY-1MN-PAYMENT", SpcfCalendar.createInstance(2008, 1, 1));
        DataLoadServices.updatePaymentTemplateSupportedDate("NY-45MN-PAYMENT", SpcfCalendar.createInstance(2008, 1, 1));
        DataLoadServices.updatePaymentTemplateSupportedDate("NY-MTA305-PAYMENT", SpcfCalendar.createInstance(2008, 1, 1));
        DataLoadServices.updatePaymentTemplateSupportedDate("NY-SDI-PAYMENT", SpcfCalendar.createInstance(2008, 1, 1));
        PayrollServices.beginUnitOfWork();
        PSPDate.setPSPTime(SpcfCalendar.createInstance(2011, 01, 05, SpcfTimeZone.getLocalTimeZone()));
        PayrollServices.commitUnitOfWork();
        Company company = createAssistedCompanyWithRates("NY", psid, "987654321", "06502929");
        ...
        ProcessResult<PayrollRun> payrollRunProcessResult = null;
        //new
        CompanyAdjustmentSubmissionDTO companyAdjustmentSubmissionDTO = DataLoadServices.createCompanyAdjustmentSubmissionDTO("Adjust_1", new DateDTO("2011-01-05"));
        Collection<LiabilityAdjustmentDTO> liabilityAdjustmentDTOs = new ArrayList<LiabilityAdjustmentDTO>();
        LiabilityAdjustmentDTO liabilityAdjustmentDTO = DataLoadServices.createLiabilityAdjustmentDTO(NYRES, NYRES, null, new DateDTO("2011-01-05"), new SpcfMoney("999.20"), new SpcfMoney("0.0"), new SpcfMoney("0.0"), false);
        liabilityAdjustmentDTOs.add(liabilityAdjustmentDTO);
        companyAdjustmentSubmissionDTO.setLiabilityAdjustmentDTOs(liabilityAdjustmentDTOs);
        Collection<CompanyAdjustmentSubmissionDTO> companyAdjustmentSubmissionDTOs = new ArrayList<CompanyAdjustmentSubmissionDTO>();
        companyAdjustmentSubmissionDTOs.add(companyAdjustmentSubmissionDTO);
        //end new
        PayrollServices.beginUnitOfWork();
        DataLoadServices.addAssistedBankAccounts(company, payrollRunDTO);
        payrollDTO = DataLoadServices.createPayrollRunWithLawsAndAmounts(payrollRunDTO, company, new DateDTO("2011-01-05"), emps, new String[]{"1", "61", "63", "66"}, new String[]{"6000", "2500", "3000", "4500"});
        payrollDTO.setPayrollTXBatchId("Payroll_1");
        payrollDTO.setTargetPayrollTXDate(new DateDTO("2011-01-05"));
        payrollDTO.setCompanyAdjustmentSubmissionDTOs(companyAdjustmentSubmissionDTOs);
        payrollRunProcessResult = PayrollServices.payrollManager.submitPayroll(SourceSystemCode.QBDT, psid, payrollDTO);
        assertSuccess(payrollRunProcessResult);
        PayrollServices.commitUnitOfWork();

        //2nd try start
        PayrollServices.beginUnitOfWork();
        PSPDate.setPSPTime(SpcfCalendar.createInstance(2011, 1, 6, SpcfTimeZone.getLocalTimeZone()));
        PayrollServices.commitUnitOfWork();
        PayrollServices.beginUnitOfWork();
        companyAdjustmentSubmissionDTO = DataLoadServices.createCompanyAdjustmentSubmissionDTO("Adjust_2", new DateDTO("2011-01-06"));
        liabilityAdjustmentDTOs = new ArrayList<LiabilityAdjustmentDTO>();

        liabilityAdjustmentDTO = DataLoadServices.createLiabilityAdjustmentDTO(NYRES, NYRES, null, new DateDTO("2011-01-06"), new SpcfMoney("-2.27"), new SpcfMoney("0.0"), new SpcfMoney("0.0"), false);
        liabilityAdjustmentDTOs.add(liabilityAdjustmentDTO);
        companyAdjustmentSubmissionDTO.setLiabilityAdjustmentDTOs(liabilityAdjustmentDTOs);
        ...
        DataLoadServices.addAssistedBankAccounts(company, payrollRunDTO);
        payrollDTO = payrollRunDTO;
        payrollDTO.setPayrollTXBatchId("Payroll_2");
        payrollDTO.setTargetPayrollTXDate(new DateDTO("2011-01-06"));
        payrollDTO.setCompanyAdjustmentSubmissionDTOs(companyAdjustmentSubmissionDTOs);
        payrollRunProcessResult = PayrollServices.payrollManager.submitPayroll(SourceSystemCode.QBDT, psid, payrollDTO);
        assertSuccess(payrollRunProcessResult);
       // assertSuccess(processResult);
        ...
        //2nd try end
        PayrollServices.beginUnitOfWork();
        PSPDate.setPSPTime(SpcfCalendar.createInstance(2011, 01, 13, SpcfTimeZone.getLocalTimeZone()));
        PayrollServices.commitUnitOfWork();
        Application.beginUnitOfWork();
        PayrollServices.systemParameterManager.updateSystemParameterValue(SystemParameter.Code.PMT_TMPLT_SPLIT_ETD_NULL_BACKDATED_AND_TIMELY_TAXES, "CA-PITSDI-PAYMENT");
        Application.commitUnitOfWork();
        PayrollServices.beginUnitOfWork();
        companyAdjustmentSubmissionDTO = DataLoadServices.createCompanyAdjustmentSubmissionDTO("Adjust_3", new DateDTO("2011-01-06"));
        liabilityAdjustmentDTOs = new ArrayList<LiabilityAdjustmentDTO>();
        liabilityAdjustmentDTO = DataLoadServices.createLiabilityAdjustmentDTO(NYRES, NYRES, null, new DateDTO("2011-01-06"), new SpcfMoney("-2.27"), new SpcfMoney("0.0"), new SpcfMoney("0.0"), false);
        liabilityAdjustmentDTOs.add(liabilityAdjustmentDTO);
        companyAdjustmentSubmissionDTO.setLiabilityAdjustmentDTOs(liabilityAdjustmentDTOs);
        ...
        liabilityAdjustmentOptionsDTO.setDebitCustomer(true);
        liabilityAdjustmentOptionsDTO.setRecordFinancialTransactions(true);
        boolean isEmployerTaxDebitError = false;
        ...
        try {
            ...
            isEmployerTaxDebitError = true;
            assertFalse(ex.getMessage().toString().contains("No EmployerTaxDebit record for payrollrun"));
        }
        assertTrue("There is a change in code  mmt.java in updateDueDate() related to EmployerTaxDebit.Please check", isEmployerTaxDebitError);
        PayrollServices.rollbackUnitOfWork();
        ...
        assertFalse("There is a change in code  mmt.java in updateDueDate() related to EmployerTaxDebit.Please check", isEmployerTaxDebitError);
        PayrollServices.rollbackUnitOfWork();
