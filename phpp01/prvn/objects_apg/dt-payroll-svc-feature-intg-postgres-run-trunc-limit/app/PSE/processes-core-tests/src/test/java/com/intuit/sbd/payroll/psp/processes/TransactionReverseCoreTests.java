 ...
 * Time: 2:48:24 PM
 */
public class TransactionReverseCoreTests {
    ...
    public void testInvalidCompanyId() {
        ...
        PayrollServices.beginUnitOfWork();
        {
            ...
            processResult.merge(PayrollServices.payrollManager.reverseTransaction(SourceSystemCode.QBOE, null, dto));

            // invalid company id (too short - valid range = 1..50)
            processResult.merge(PayrollServices.payrollManager.reverseTransaction(SourceSystemCode.QBOE, "", dto));
            ...
            processResult.merge(PayrollServices.payrollManager.reverseTransaction(SourceSystemCode.QBOE, "INVALID", dto));
        }
        PayrollServices.commitUnitOfWork();
    ...
    public void testNullDto() {
        ...
        PayrollServices.beginUnitOfWork();
        // invalid dto (null)
        processResult.merge(PayrollServices.payrollManager.reverseTransaction(SourceSystemCode.QBOE, "123272727", null));
        PayrollServices.commitUnitOfWork();
    ...
    public void testInvalidDdTxBatchId() {
        ...
        PayrollServices.beginUnitOfWork();
        {
            ...
            processResult.merge(PayrollServices.payrollManager.reverseTransaction(SourceSystemCode.QBOE, "123272727", dto));
    ...
    private void testInvalidPayrollStatus(PayrollStatus pPayrollStatus) {
        ...
        String messageText1 = "Action DDTransactionReverse is not valid for payroll run with DDTxBatchID BatchId01, which has status of " + pPayrollStatus.toString() + ".";
        assertEquals("Error Message", messageText1, message1.getMessage());
    }
...
//        testInvalidPayrollStatus(PayrollStatus.OffloadedAll);
    ...
    public void testInvalidCompanyBankAccount() {
        ...
        PayrollServices.companyManager.deactivateCompanyBankAccount(SourceSystemCode.QBOE, "123272727",
        ...
        // Verify that the correct message strings were returned
        String messageText1 = "Company QBOE:123272727 does not have an active bank account.";
        assertEquals("Error Message", messageText1, message1.getMessage());
    }
    ...
    public void testInvalidSourceTransactionId() {
        ...
        TransactionReverseDTO dto = buildTransactionReverseDTO("BatchId01", dtoTxnList);
        ...
        PayrollServices.beginUnitOfWork();
        processResult1.merge(PayrollServices.payrollManager.reverseTransaction(SourceSystemCode.QBOE, "123272727", dto));
        PayrollServices.commitUnitOfWork();
        ...
        PayrollServices.beginUnitOfWork();
        processResult2.merge(PayrollServices.payrollManager.reverseTransaction(SourceSystemCode.QBOE, "123272727", dto));
        PayrollServices.commitUnitOfWork();
        ...
        // Verify that the correct message strings were returned
        messageText1 = "Transaction " + txn1 + " does not exist for company QBOE:123272727.";
        assertEquals("Error Message", messageText1, message1.getMessage());
        messageText2 = "Transaction " + txn2 + " does not exist for company QBOE:123272727.";
        assertEquals("Error Message", messageText2, message2.getMessage());
        messageText3 = "Transaction " + txn3 + " does not exist for company QBOE:123272727.";
        assertEquals("Error Message", messageText3, message3.getMessage());
    }
    ...
    public void testAttemptedReversalForInvalidTransactionStates() {
        ...
        // validate error count
        assertEquals("Number of Errors", 3, processResult.getMessages().size());

        // validate error codes
        Message message2 = processResult.getMessages().get(0);
        assertEquals("Error Code", "261", message2.getMessageCode());
        Message message3 = processResult.getMessages().get(1);
        assertEquals("Error Code", "262", message3.getMessageCode());
        Message message4 = processResult.getMessages().get(2);
        assertEquals("Error Code", "263", message4.getMessageCode());

        // Verify that the correct message strings were returned
        String messageText2 = "Transaction " + dtoTxnList.get(0) + " cannot be reversed, because it was previously canceled.";
        assertEquals("Error Message", messageText2, message2.getMessage());
        String messageText3 = "Transaction " + dtoTxnList.get(1) + " cannot be reversed, because an ACH Return was received for this transaction.";
        assertEquals("Error Message", messageText3, message3.getMessage());
        String messageText4 = "Transaction " + dtoTxnList.get(2) + " cannot be reversed, because a reversal has already been attempted.";
        assertEquals("Error Message", messageText4, message4.getMessage());
    }
    ...
    public void testAchReversalOfSingleTransactionWithFee() {
        ...
        PayrollServices.beginUnitOfWork();
        {
            ...
            dtoTxnList.add(txnList.get(0).getPaycheckSplit().getSourceDdTxnId());
        ...
        PayrollServices.beginUnitOfWork();
        {
            ...
            // verify the SKU type
            assertEquals("SKU:", "408177", feeTxn.getSku()); // reversal fee sku for "QBOE DD" offering
    ...
    private void testNonAchReversalOfSingleTransactionWithFee(SettlementTypeDTO pSettlementType) {
        ...
        PayrollServices.beginUnitOfWork();
        {
            ...
            // any transaction will for for this test, so use the first one in the list.
            dtoTxnList.add(txnList.get(0).getPaycheckSplit().getSourceDdTxnId());
            System.out.println("dtoTxnList " + txnList.get(0).getPaycheckSplit().getSourceDdTxnId());
            dto.setDdTransactionIdList(dtoTxnList);
            dto.setChargeFee(true);
            dto.setTxSettlementTypeCd(pSettlementType);
        ...
        PayrollServices.beginUnitOfWork();
        {
            ...
            for (FinancialTransaction txn : txnList) {
                ...
                if (txn.getPaycheckSplit().getSourceDdTxnId().equals(dtoTxnList.get(0))) {
                    //if (isFirstTxn) {
                    //        isFirstTxn = false;
    ...
    public void testImplicitAchReversalOfAllTransactions() {
        ...
        DomainEntitySet<CompanyEvent> companyEvents =
                CompanyEvent.findCompanyEvents(company, EventTypeCode.ReversalRequested, null, null, null);
        assertEquals("Number of Reversal Requested Events", 4, companyEvents.size());
        int i = 0;
    ...
    public void testImplicitAchReversal_IntuitInitiated() {
        ...
        for (FinancialTransaction currDDRedebit : cancelledRedebits) {
            assertEquals("Payroll Run Id ", "BatchId01", currDDRedebit.getPayrollRun().getSourcePayRunId());
...
//                Assert.assertEquals("Event Detail Value", reversalTxn.getId().toString(), eventDetails.get(0).getValue());
    ...
    public void testAchReversalOfPartialPaycheck_IntuitInitiated() {
        ...
        PayrollServices.beginUnitOfWork();
        PSPDate.setPSPTime(TransactionReverseCoreDataLoader.prTxnReverseSubmitDate);
        processResult = PayrollServices.payrollManager.reverseTransaction(SourceSystemCode.QBOE, "123272727", dto);
        PSPDate.resetPSPTime();
        PayrollServices.commitUnitOfWork();
...
//                Assert.assertEquals("Event Detail Value", reversalTxn.getId().toString(), eventDetails.get(0).getValue());
    ...
    public void testBug800() {
        ...
        PayrollServices.commitUnitOfWork();
        assertEquals("payroll status before reversal", PayrollStatus.Complete, payroll.getPayrollRunStatus());

        // reverse the payroll
        PayrollServices.beginUnitOfWork();
        PSPDate.setPSPTime(TransactionReverseCoreDataLoader.prTxnReverseSubmitDate);
        TransactionReverseDTO dto = buildTransactionReverseDTO("BatchId01", null);
        ProcessResult prReverse = PayrollServices.payrollManager.reverseTransaction(SourceSystemCode.QBOE, "123272727", dto);
        PayrollServices.commitUnitOfWork();
        ...
        PayrollServices.beginUnitOfWork();
        reversalDebits = FinancialTransaction.findFinancialTransactions(SourceSystemCode.QBOE,
                                                                                              "123272727",
                                                                                              TransactionTypeCode.EmployeeDdReversalDebit,
    ...
    public void testIntuitInitiatedReversal_ReturnEECreditTransaction() {
        ...
        for(FinancialTransaction finTxn  : employeeFinTxns){
            dtoTxnList.add(finTxn.getPaycheckSplit().getSourceDdTxnId());
        }
        ...
        PayrollServices.beginUnitOfWork();
        ProcessResult processResult = PayrollServices.payrollManager.reverseTransaction(SourceSystemCode.QBOE, "123272727", dto);
        PayrollServices.commitUnitOfWork();
    ...
    //PSRV001028
    @Test
    public void testIntuitInitiatedReversal_CancelEECreditTransaction() {
        PayrollServices.beginUnitOfWork();
        ...
        // offload QBOE EE CR
        PayrollServices.beginUnitOfWork();
        PSPDate.setPSPTime("20070727000000");
        PayrollServices.commitUnitOfWork();
        ...
        // offload QBOE ER DB
        PayrollServices.beginUnitOfWork();
        PSPDate.setPSPTime("20070724000000");
        PayrollServices.commitUnitOfWork();
        ...
        List<String> dtoTxnList = new Vector<String>(4);
       TransactionReverseDTO dto = buildTransactionReverseDTO("BatchTest05", null);
       ...
       //Intuit Initiated Reversal
       PayrollServices.beginUnitOfWork();
       processResult = PayrollServices.payrollManager.reverseTransaction(SourceSystemCode.QBOE, "1234567", dto);
       PayrollServices.commitUnitOfWork();
       ...
       assertSuccess("Result ", processResult) ;
        ...
        assertEquals("Number of ACH ER DD Redebit Financial Txs", 1, cancelledRedebits.size());
        for (FinancialTransaction currDDRedebit : cancelledRedebits) {
            assertEquals("Payroll Run Id ", "BatchTest05", currDDRedebit.getPayrollRun().getSourcePayRunId());
            assertEquals("Financial Transaction State ", TransactionStateCode.Cancelled, currDDRedebit.getCurrentTransactionState().getTransactionStateCd());
            assertEquals("Settlement Type ", SettlementType.ACH, currDDRedebit.getSettlementTypeCd());
        }
