 ...
 */
@ScheduledJob(name = "ATFEmployeeTotalsExtract", resourcePath = "/normal", autoScheduleGenerator = JSSAutoScheduleGenerator.class, scheduleGenerator = JSSScheduleGenerator.class)
public class EmployeeTotalsExtractProcess extends BaseATFExtractFileProcess {
    ...
    public EmployeeTotalsExtractProcess(String[] pArguments) {
        super(pArguments);
        init();
    }

    public EmployeeTotalsExtractProcess(String[] pArguments, String pJobId) {
    ...
    protected void writeData(PrintWriter pPW) throws Throwable {
        ...
        if (mExtractBatch.getRunType().equals(ATFDataExtractRunType.QuarterlyData)) {
            ...
            SpcfCalendar upperBoundDate = getEndDate();
            ...
            SpcfCalendar lastRunDate = super.getLastSuccessfulExtractBatchStartdate(true).toLocal();
    ...
    private void writeEmployeeLawQuarterlyInfo(PrintWriter pPW, ScrollableResults pEmployeeLawQtrTotals) throws Throwable {
        ...
        try {
            while (pEmployeeLawQtrTotals.next()) {
                ...
                //Month one indicator
                writeFormatted(pPW,(pEmployeeLawQtrTotals.get(11) != null &&  new BigDecimal(String.valueOf(pEmployeeLawQtrTotals.get(11))).toString().equals("1")) ? "Y" : "N");

                //Month two indicator
                writeFormatted(pPW,(pEmployeeLawQtrTotals.get(11) != null &&  new BigDecimal(String.valueOf(pEmployeeLawQtrTotals.get(12))).toString().equals("1")) ? "Y" : "N");

                //Month three indicator
                writeFormatted(pPW,(pEmployeeLawQtrTotals.get(11) != null &&  new BigDecimal(String.valueOf(pEmployeeLawQtrTotals.get(13))).toString().equals("1")) ? "Y" : "N");
                ...
                //SourceItemId
                writeFormatted(pPW, pEmployeeLawQtrTotals.get(15) != null ? (String) pEmployeeLawQtrTotals.get(15) : "");

                //QB Unique Id
                writeFormatted(pPW, String.valueOf(pEmployeeLawQtrTotals.get(16)),true);

                mRecordCount++;// Increment for the record count in the trailer record

                if (mRecordCount % 100000 == 0) {
                    getLogger().info("EE Totals written :" + mRecordCount + " Records.");
                }
        ...
        getLogger().info(String.format("EE Totals finished writing %d records to EE Totals extract file.", mRecordCount));
    }
     ...
     * @param pEmployeeLawQtrTotals EmployeeLawQtrTotals
     */
    private void writeEmployeeLawQuarterlyInfo(PgpWriter pPW, ScrollableResults pEmployeeLawQtrTotals) throws Throwable {
        ...
        try {
            while (pEmployeeLawQtrTotals.next()) {
                ...
                //Month two indicator
                writeFormatted(pPW,(pEmployeeLawQtrTotals.get(12) !=null &&  new BigDecimal(String.valueOf(pEmployeeLawQtrTotals.get(12))).toString().equals("1")) ? "Y" : "N");

                //Month three indicator
                writeFormatted(pPW,(pEmployeeLawQtrTotals.get(13) !=null &&  new BigDecimal(String.valueOf(pEmployeeLawQtrTotals.get(13))).toString().equals("1")) ? "Y" : "N");
    ...
    private ScrollableResults getEmployeeLawQtrTotals(SpcfCalendar pFromUpdateDate, SpcfCalendar pLowerBoundDate, SpcfCalendar pUpperBoundDate) {
        ...
        queryBuilder.append("  (select company.SOURCE_COMPANY_ID as srcCompanyId, employee.SOURCE_EMPLOYEE_ID as srcEmpId, elqt.QUARTER as qtr, elqt.YEAR as yr, \n")
                ...
                .append("      qei.LIST_ID as QB_UNIQUE_ID \n")
        ...
        queryBuilder.append("select company.SOURCE_COMPANY_ID as srcCompanyId, srcEmpId.SOURCE_EMPLOYEE_ID as srcEmpId, qet.QUARTER as qtr, qet.YEAR as yr,'' as srcLawCd, \n")
                ...
                .append("inner join PSP_QBDT_EMPLOYEE_INFO qei on qei.EMPLOYEE_FK=srcEmpId.EMPLOYEE_SEQ \n")
        ...
        queryBuilder.append(" Group by company.SOURCE_COMPANY_ID, srcEmpId.SOURCE_EMPLOYEE_ID, srcPayrollItemId.SOURCE_PAYROLL_ITEM_ID, qet.QUARTER,qet.YEAR, qei.LIST_ID) \n");
        queryBuilder.append(" order by srcCompanyId, srcEmpId, srcLawCd,  qtr \n");
    ...
    private ScrollableResults getEmployeeLawQtrTotals(int pYear, int pQuarter) {
        ...
        queryBuilder.append("(select company.SOURCE_COMPANY_ID as srcCompanyId, emp.SOURCE_EMPLOYEE_ID as srcEmpId, eetotals.QUARTER qtr,\n ")
                ...
                .append(" qei.LIST_ID QB_UNIQUE_ID \n")
                ...
                .append("   and qei.EMPLOYEE_FK=emp.EMPLOYEE_SEQ  \n")
        ...
        queryBuilder.append(" UNION \n")
                ...
                .append(" select company.SOURCE_COMPANY_ID as srcCompanyId, emp.SOURCE_EMPLOYEE_ID as srcEmpId, qet.QUARTER qtr, qet.YEAR yr, ").append("''" + " srcLawCd, \n")
                ...
                .append("inner join PSP_QBDT_EMPLOYEE_INFO qei on qei.EMPLOYEE_FK=emp.EMPLOYEE_SEQ \n")
                .append("inner join PSP_INDIVIDUAL ind on emp.EMPLOYEE_SEQ=ind.INDIVIDUAL_SEQ \n")
        ...
        queryBuilder.append("group by company.SOURCE_COMPANY_ID , emp.SOURCE_EMPLOYEE_ID , compPayrollItem.SOURCE_PAYROLL_ITEM_ID,qet.QUARTER,qet.YEAR,qei.LIST_ID)  \n")
