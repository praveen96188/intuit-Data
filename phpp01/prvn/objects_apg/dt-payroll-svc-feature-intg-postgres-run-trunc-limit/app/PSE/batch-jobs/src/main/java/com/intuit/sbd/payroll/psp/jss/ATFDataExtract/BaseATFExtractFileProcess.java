 ...
 */
abstract public class BaseATFExtractFileProcess extends JSSBatchJob {
     ...
     * Constructor
     */
    public BaseATFExtractFileProcess(String[] pArguments) {
        super(pArguments);
        init();
    }

    public BaseATFExtractFileProcess(String[] pArguments, String pJobId) {
    ...
    @Override
    public void execute() {
        executeStep(ExtractFileStep.class);
    }
    ...
    public static class ExtractFileStep extends JSSBatchJobStep<BaseATFExtractFileProcess> {
        public void execute() {
            ATFDataExtractFile extractFile = null;
            getLogger().info(getBatchJobProcessor().getBatchJobName() + "running.. "+getBatchJobProcessor().getJobId());
            try {
                String batchFolderStr = getBatchJobProcessor().mExtractOutputDir + File.separator + "PSP_" + getBatchJobProcessor().mExtractBatch.getBatchId();
                String fileName =  batchFolderStr + File.separator + getBatchJobProcessor().genExtractFileName(true);
                ...
                PgpWriter pgpw = getPgpWriter(fileName);

                // create the ATFDataExtractFile record;
                extractFile = getBatchJobProcessor().createATFDataExtractFileRecord(getBatchJobProcessor().mExtractBatch, getBatchJobProcessor().mExtractFileType, getBatchJobProcessor().getFileName(pgpw));

                // write the header record
                getBatchJobProcessor().writeHeaderRecord(pgpw);

                //get Unit of Work
                PayrollServices.beginUnitOfWork();
                // Write the Actual Data
                getBatchJobProcessor().writeData(pgpw);

                // write the trailer record
                getBatchJobProcessor().writeTrailerRecord(pgpw);

                // update status of file to Extracted
                getBatchJobProcessor().updateATFExtractFileStatus(extractFile.getId(), ATFDataExtractFileStatus.Extracted);
                ...
                getLogger().error("Exception trying to write data for " + getBatchJobProcessor().mATFExtractTypeID, t);
                getLogger().info("Updating status of ATFExtractFile to '" + ATFDataExtractFileStatus.Failed + "'");
                System.out.println(t.toString()); //add some quick debugging for the teamcity logs
                t.printStackTrace(System.out);
                if (extractFile != null){

                    PayrollServices.beginUnitOfWork();
                    getBatchJobProcessor().updateATFExtractFileStatus(extractFile.getId(), ATFDataExtractFileStatus.Failed);
                    PayrollServices.commitUnitOfWork();

                } else {
                    getLogger().error("ATFDataExtractFile record is not created.");
                }
    ...
    protected String getFileName(PgpWriter pgpw) {
        return ((PgpCommonEncryptedWriter) pgpw).getEncryptedFileName();
    }

    protected static PgpWriter getPgpWriter(String fileName) throws Exception {
        PgpWriter pgpw = new PgpCommonEncryptedWriter(BatchUtils.getTfsPgpKeys());
        pgpw.open(fileName);
        return pgpw;
    }
    ...
    abstract protected void writeData(PgpWriter pPW) throws Throwable;
     ...
     * Format: "HDR","Payroll System Name","YYYYMMDD","intRevisionNumber"
     *
     * @param pPW PgpWriter Extract file print writer
     *
     */
    protected void writeHeaderRecord(PgpWriter pPW) throws IOException {

        pPW.write(DOUBLE_QUOTE);
        pPW.write(HEADER_REC_IDENTIFIER);
        pPW.write(DELIMITER);
        pPW.write(PAYROLL_SRC_SYSTEM_NAME);
        pPW.write(DELIMITER);
        pPW.write(mProcessCode);
        pPW.write(DELIMITER);
        pPW.write(mCreateTime);
        pPW.write(DELIMITER);
        pPW.write(INT_REVISION_NUM);
        pPW.write(DOUBLE_QUOTE);
     ...
     * @param pPW PgpWriter
     */
    protected void writeTrailerRecord(PgpWriter pPW) throws IOException {

        pPW.write(DOUBLE_QUOTE);
        pPW.write(TRAILER_RECORD_IDENTIFIER);
        pPW.write(DELIMITER);
        //include trailer record in the record count
        pPW.write(Integer.toString(++mRecordCount));
        pPW.write(DELIMITER);
        pPW.write(mCreateTime);
        pPW.write(DELIMITER);
        pPW.write(INT_REVISION_NUM);
    ...
    *//**
    ...
    /**
     * This method escapes any double quotes and writes to the print writer
     * @param pPW  PgpWriter Extract file writer
     * @param pField the field to write
     * @throws Throwable
     */
    protected void writeFormatted(PgpWriter pPW, String pField) throws Throwable {
        writeFormatted(pPW, pField, false);
    }
     ...
     * @throws Throwable
     */
    protected void writeFormatted(PgpWriter pPW, String pField, boolean isLastColumn) throws Throwable {
        if (pField != null) {
            String strToWrite = pField.trim();
            //Replace a single quote with double quotes
            strToWrite = strToWrite.replaceAll("\"", "\"\"");
...
//    }
    ...
    protected SpcfCalendar getEndDate(){
        int daysForEndDate = SystemParameter.findIntValue(SystemParameter.Code.ATF_DAYS_FOR_END_DATE);
        SpcfCalendar date = PSPDate.getPSPTime();
        date.addDays(daysForEndDate);
        return date;
    }
