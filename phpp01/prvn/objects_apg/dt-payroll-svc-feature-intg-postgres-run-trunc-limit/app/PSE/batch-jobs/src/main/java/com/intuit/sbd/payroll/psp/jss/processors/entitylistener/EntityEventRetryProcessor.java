 ...
 *
 */
@ScheduledJob(name = "EntityEventRetry", resourcePath = "/normal", autoScheduleGenerator = JSSAutoScheduleGenerator.class, scheduleGenerator = JSSScheduleGenerator.class)
public class EntityEventRetryProcessor extends JSSBatchJob{

    private SpcfCalendar mRunDate;
    private Boolean mPriorDate = false;

	public EntityEventRetryProcessor(String[] pArguments) {
        super(pArguments);
    }

    public EntityEventRetryProcessor(String[] pArguments, String pJobId) {
    ...
    protected void execute() {
        getLogger().info("Starting Entity Event Retry processor");

        StopWatch timer = StopWatch.startTimer();

        PayrollServices.setCurrentPrincipal(new PspPrincipal(SystemPrincipal.EntityEventRetryBatchJob));

        executeStep(EntityEventRetryPublish.class);

        getLogger().info("Completed Entity Event Retry processor. Elapsed time: " + timer.stop().getElapsedTimeString());	
    }
    
    public static class EntityEventRetryPublish extends JSSBatchJobStep<EntityEventRetryProcessor> {
        ...
        public void execute() {
        	try{
	                ...
	                batchSize = SystemParameter.findIntValue(SystemParameter.Code.ENTITY_RETRY_BATCH_SIZE, 50);
	                ...
	                getLogger().info("system properties: rettry batch size"+batchSize+" "+interval+" "+maxWait+" "+minPoolSize+" "+maxPoolSize);	
					...
					List<RangeMessage> rangeMessages =findUnprocessedEntityMessage(batchSize);
	                
					getLogger().info("rangeMessages {}", rangeMessages);
	                

	                JmsTemplate jmstemplate=null;
	                if(rangeMessages!=null && rangeMessages.size()>0){
        ...
        private void updateLastProcessedTime(DateTime lastProcessedTime) {
    		...
    		getLogger().info("Update the SystemParameter Code LAST_EVENT_RETRY_PROCESSED_TIME to {} ",
    				lastProcessedTime);
    		DateTimeFormatter DATE_FORMAT = ISODateTimeFormat.dateTime().withZoneUTC();
    		SystemParameter.update(SystemParameter.Code.LAST_EVENT_RETRY_PROCESSED_TIME,
         ...
         * @return
         */
		private List<RangeMessage> findUnprocessedEntityMessage( int batchSize) {
    		DateTime lastProcessedTime = findLastProcessedTime(
    				SystemParameter.Code.LAST_PROCESSED_TIME); //LAST_EVENT_PROCESSED_TIME

    		getLogger().info("SystemParameter Code LAST_PROCESSED_TIME is {} and  isLocal {}",
    				lastProcessedTime, lastProcessedTime);
    		
    		DateTime lastRetryProcessedTime = findLastRetryProcessedTime(
    				SystemParameter.Code.LAST_EVENT_RETRY_PROCESSED_TIME);
    		
    		//lastRetryProcessedTime > lastProcessedTime
    		if(lastRetryProcessedTime.compareTo(lastProcessedTime)>0) {
    			getLogger().info("lastRetryProcessedTime is > lastProcessedTime");
    			return null;
    		}
    		getLogger().info("lastRetryProcessedTime {}",lastRetryProcessedTime);
    		

    		Timestamp sqltimestamp = jodaToSQLTimestamp(lastRetryProcessedTime);
    		
    		getLogger().info("Last processed time in timestamp {}",sqltimestamp);
    		
    		Timestamp sqltimestampnow = jodaToSQLTimestamp(lastProcessedTime);
    	...
    	private DateTime findLastRetryProcessedTime(SystemParameter.Code pCode) {
			getLogger().info("findLastRetryProcessedTime start");

    		String lastProcessedTimeString = SystemParameter
    				.findStringValue(SystemParameter.Code.LAST_EVENT_RETRY_PROCESSED_TIME);
    		...
    		try {
    			...
    			lastProcessedTime = null;
    			getLogger().info("findLastRetryProcessedTime didnt find"+lastProcessedTime+" "+e.getMessage());
    		}
			getLogger().info("findLastRetryProcessedTime end"+lastProcessedTime);

    		return lastProcessedTime;
    	}

		private String getConnectionUrl() {
				getLogger().info("OMS3: Pulsar in getConnectionURL in EntityEventRetryProcessor");
		...
		public ConnectionFactory getConnectionFactory(String username, String password, String connectionUrl) throws URISyntaxException {
				getLogger().info("OMS3: Pulsar in getConnectionFactory in EntityEventRetryProcessor");
        ...
        private JmsTemplate initializepublisher() {
            ...
            for (int x = 0; x < maxAttempts; x++) {
                try {
						...
						getLogger().info("OMS3: Pulsar in default destination in EntityEventRetryProcessor");
