 ...
 */
@ScheduledJob(name = "ATFCompanyPayrollItemExtract", resourcePath = "/normal", autoScheduleGenerator = JSSAutoScheduleGenerator.class, scheduleGenerator = JSSScheduleGenerator.class)
public class CompanyPayrollItemExtractProcess extends BaseATFExtractFileProcess {
    ...
    public CompanyPayrollItemExtractProcess(String[] pArguments) {
        super(pArguments);
        init();
    }

    public CompanyPayrollItemExtractProcess(String[] pArguments, String pJobId) {
    ...
    protected void writeData(PrintWriter pPW) throws Throwable {
        ...
        getLogger().info("Beginning CompanyPayrollItemExtractProcess for: " + mExtractBatch.getRunType());
        writeCompanyPayrollItemInfo(pPW, getCompanyPayrollItemExtract(minLastQuarterToFile, lastRunDate));
        getLogger().info("Completed writing file");
    }
    ...
    private void writeCompanyPayrollItemInfo(PrintWriter pPW, ScrollableResults pCompanyPayrollItems) throws Throwable {
        ...
        try {
            while (pCompanyPayrollItems.next()) {
                ...
                //Source Company ID
                writeFormatted(pPW, String.valueOf(pCompanyPayrollItems.get(0)));

                //Source Description
                writeFormatted(pPW, String.valueOf(pCompanyPayrollItems.get(1)));

                //Source Payroll Item Id
                writeFormatted(pPW, String.valueOf(pCompanyPayrollItems.get(2)));
                ...
                //Tax Form Line
                writeFormatted(pPW, String.valueOf(pCompanyPayrollItems.get(4)),true);


                mRecordCount++;// Increment for the record count in the trailer record

                if (mRecordCount % 100000 == 0) {
                    getLogger().info("Payroll Items written :" + mRecordCount + " Records.");
                }
        ...
        getLogger().info(String.format("Company Payroll Item Extract finished writing %d records to Payroll Items extract file.", mRecordCount));
    }
     ...
     * @param pCompanyPayrollItems CompanyPayrollItems
     */
    private void writeCompanyPayrollItemInfo(PgpWriter pPW, ScrollableResults pCompanyPayrollItems) throws Throwable {
        String w2Code=null;
        try {
            while (pCompanyPayrollItems.next()) {
