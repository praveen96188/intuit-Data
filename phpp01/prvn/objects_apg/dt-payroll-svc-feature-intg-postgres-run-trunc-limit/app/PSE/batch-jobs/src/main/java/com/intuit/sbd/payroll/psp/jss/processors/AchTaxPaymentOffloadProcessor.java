 ...
 */
@ScheduledJob(name = "AchTaxPaymentOffload", resourcePath = "/high", autoScheduleGenerator = JSSAutoScheduleGenerator.class, scheduleGenerator = JSSScheduleGenerator.class)
public class AchTaxPaymentOffloadProcessor extends DailyBatchJobsProcessor {

    public AchTaxPaymentOffloadProcessor(String[] pArguments) {
        super(pArguments);
    }

    public AchTaxPaymentOffloadProcessor(String[] pArguments, String pJobId) {
...
//        executeStep(new StartAchTaxPaymentOffloadMonitor());
        ...
        executeStep(UpdateFinancialTransaction.class); // uses batch id from batch job context (or finds it if reentering)
        ...
        getLogger().info("Completed ACH Tax Payment Offload. Elapsed time: " + timer.stop().getElapsedTimeString());
    }
