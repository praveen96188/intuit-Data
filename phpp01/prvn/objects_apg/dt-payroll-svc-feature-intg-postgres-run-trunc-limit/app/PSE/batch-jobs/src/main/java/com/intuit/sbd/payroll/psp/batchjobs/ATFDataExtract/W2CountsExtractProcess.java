...
public class W2CountsExtractProcess extends BaseATFExtractFileProcess {
    ...
    public W2CountsExtractProcess(RunMode pRunMode, BatchJobType pBatchJobType, String pJobId, String pJobInstanceParameters) {
    ...
    protected void writeData(PrintWriter pPW) throws Throwable {
            ...
            while (w2Counts.next()) {
                if (!w2Counts.get(1).toString().equals("IRS")) {
                    ...
                    if (mRecordCount % 100000 == 0) {
                        logger.info("W2 Counts written :" + mRecordCount + " Records.");
                    }
                }
        ...
        logger.info(String.format("W2 Counts finished writing %d records to W2 Counts extract file.", mRecordCount));
    }
