 ...
 * Time: 3:49:45 PM
 */
@ScheduledJob(name = "ATFDepositFrequencyExtract", resourcePath = "/normal", autoScheduleGenerator = JSSAutoScheduleGenerator.class, scheduleGenerator = JSSScheduleGenerator.class)
public class DepositFrequencyExtractProcess extends BaseATFExtractFileProcess {
    ...
    private static final String WA_BUSINESS_TAX_NAME = "WA UBI";

    public DepositFrequencyExtractProcess(String[] pArguments) {
        super(pArguments);
        init();
    }

    public DepositFrequencyExtractProcess(String[] pArguments, String pJobId) {
    ...
    private void writeResultsToFile(ScrollableResults pQueryResults, String pRecordType, PgpWriter pPW) throws IOException {
        ...
        try {
            while (pQueryResults.next()) {
                ...
                // Record type.
                pPW.write(DOUBLE_QUOTE + pRecordType + DELIMITER);

                // Source Company Id.
                pPW.write(sourceCompanyId + DELIMITER);
                ...
                if (RECORD_TYPE_DEP_FREQ.equals(pRecordType)) {
                    ...
                    pPW.write(formattedRegion + DELIMITER);
                    pPW.write(getDepositFrequencyAbbreviation(region.substring(0, 2), freq) + DELIMITER);
                    pPW.write(effectiveDate.format("yyyyMMdd") + DELIMITER);
                    ...
                    pPW.write(region + DELIMITER);
                    pPW.write(TXACI_CODE + DELIMITER);
                    pPW.write(getFilerTypeAbbreviation(value) + DELIMITER);
                    pPW.write(effectiveDate.format("yyyyMMdd") + DOUBLE_QUOTE);
                    pPW.write(System.lineSeparator());
                    ...
                    pPW.write(region + DELIMITER);
                    pPW.write(ADDL_INFO_CODE + DELIMITER);
                    pPW.write(value + DELIMITER);
                    pPW.write(effectiveDate.format("yyyyMMdd") + DOUBLE_QUOTE);
                    pPW.write(System.lineSeparator());
                }
    ...
    private static ScrollableResults findBusinessTaxIdResults(SpcfCalendar startDate) {
        ...
        StringBuilder builder = new StringBuilder();
        // Query for deposit frequency records that do not have an invalid date.
        builder.append(" select co.SourceCompanyId, SUBSTR(ag.AgencyId, 1, 2), coptaid.AgencyTaxpayerIdEnc, coptaid.ModifiedDate\n")
                ...
                .append(" join ca.Company co\n")
                .append(" join ca.Agency ag\n")
                .append(" where coptaid.Name in ('" + IA_BUSINESS_TAX_NAME + "','" + WA_BUSINESS_TAX_NAME + "')")
                .append("    and cs.Company=co \n")
                .append("    and cs.Service.ServiceCd = 'Tax'\n");
    ...
    private String getDepositFrequencyAbbreviation(String state, DepositFrequencyCode depFreqCode) {
        ...
        if ("ID".equals(state) && depFreqCode == DepositFrequencyCode.SPLITMONTHLY) {
            ...
            getLogger().error("Unable to determine a deposit frequency abbreviation for " + state + " and " + depFreqCode + ".");
        }
    ...
    private String getFilerTypeAbbreviation(String payTemplateName) {
        if (FILERTYPE_FT_941.equals(payTemplateName)) {
            ...
            getLogger().error("Unable to determine a filer type abbreviation for " + payTemplateName + ".");
            return null;
        }
    }
}
