 ...
 * User: mvillani
 */
public class EoqSUIAdjustmentsProcessor extends BatchJobProcessor {
    ...
    public EoqSUIAdjustmentsProcessor(BatchJobProcessor.RunMode pRunMode, BatchJobType pBatchJobType, String pJobId, String pJobInstanceParameters) {
    ...
    public void execute() {
        ...
        executeStep(new SUIRatePaymentsCleanupStep());
        executeStep(new EoqEmailStep());
        executeStep(new EoqSUITaxAdjustmnentsStep());
        executeStep(new LiabilityAdjustmentsCleanupStep());
        logger.info("Completed EOQ job. Elapsed time: " + timer.stop().getElapsedTimeString());

    }

    public class SUIRatePaymentsCleanupStep extends BatchJobProcessor.BatchJobProcessorStep {
        public void execute() {
            try {
    ...
    public class EoqEmailStep extends BatchJobProcessorStep {
        @Override
        public void execute() {
            if (!BatchUtils.getConfigString("psp_eoq_email_list").isEmpty()) {
                try {
                    ...
                    SendEmailToRecipients(ledgerBalances);
    ...
    private void SendEmailToRecipients(ScrollableResults pQueryResults) {
        ...
        if (!tempEoqReportFile.getParentFile().exists()) {
            ...
            if (!created) {
                logger.error("Unable to create directory for temp EOQ files.");
                return;
        ...
        if (numberOfRecords > 0) {
            ...
            logger.info("No records found to email in SUI Rate Cleanup");

        }

    }

    public class EoqSUITaxAdjustmnentsStep extends BatchJobProcessor.BatchJobProcessorStep {
        public void execute() {
            try {
                PayrollServices.setCurrentPrincipal(SystemPrincipal.EoqSUITaxAdjustmentsBatchJob);
                try {
    ...
    public class LiabilityAdjustmentsCleanupStep extends BatchJobProcessor.BatchJobProcessorStep {
        public void execute() {
            try {
    ...
    public static void main(String[] args) {
        BatchJobManager.runJob(BatchJobType.EoqSUIAdjustments, args);
