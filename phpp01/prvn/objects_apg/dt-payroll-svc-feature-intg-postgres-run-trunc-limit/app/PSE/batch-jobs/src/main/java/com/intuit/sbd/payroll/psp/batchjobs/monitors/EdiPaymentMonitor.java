...
import com.intuit.sbd.payroll.psp.batchjobs.processors.EdiPaymentProcessor;
 ...
 */
public class EdiPaymentMonitor extends BatchJobProcessorMonitor {
    public EdiPaymentMonitor(BatchJobProcessor.RunMode pRunMode, BatchJobType pBatchJobType, String pJobId, String pJobIdToMonitor) {
        super(pRunMode,
              pBatchJobType,
              pJobId,
              pJobIdToMonitor,
              BatchJobType.EdiPayment,
              EdiPaymentProcessor.RecordPaymentEvents.class);
    }
    ...
    public void execute() {
        ...
        if (paymentCount == 0) {
            logger.info(getClass().getSimpleName() + " skipped (no pending EDI tax payments) ");
            return;
        }
