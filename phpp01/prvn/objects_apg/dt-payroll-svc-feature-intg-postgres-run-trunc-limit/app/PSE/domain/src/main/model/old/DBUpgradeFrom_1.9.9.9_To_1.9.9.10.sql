...
Prompt Function FN_GET_PSP_TIMESTAMP;
--
-- FN_GET_PSP_TIMESTAMP  (Function) 
--
CREATE OR REPLACE FUNCTION FN_GET_PSP_TIMESTAMP RETURN TIMESTAMP
AS
ts TIMESTAMP;
...
BEGIN
   ...
   SELECT SYSTIMESTAMP + NUMTODSINTERVAL(TO_NUMBER(v_offset)/1000, 'SECOND')
    INTO ts
    FROM DUAL;
...
Prompt Function FN_FORMAT_SYSGUID;
--
-- FN_FORMAT_SYSGUID  (Function) 
--
...
IS
	...
	IS
		...
		BEGIN
		   ...
		   VALUES (v_nacha_file_id, 0, p_user_id, v_utc_date, p_user_id, v_utc_date, -1, NULL, NULL,
		...
		LOOP
			...
			FORALL i IN event_id_table.first..event_id_table.last
				...
				VALUES (event_id_table(i), 0, p_user_id, v_utc_date, p_user_id, v_utc_date, -1,
				        v_psp_date, v_psp_date, 'Active', 'FeeOffloaded', event_token_tbl(i), NULL, NULL, NULL, NULL, company_id_tbl(i));
		    FORALL i IN event_id_table.first..event_id_table.last
...
PROMPT finishedDBUpgradeFrom_1.9.9.9_To_1.9.9.10.sql