...
ALTER TABLE PSP_COMPANY_EVENT
 ADD (EMAIL_STATUS_EFFECTIVE_DATE  TIMESTAMP(6));

Prompt Column EMAIL_RETRY_COUNT;
ALTER TABLE PSP_COMPANY_EVENT
 ADD (EMAIL_RETRY_COUNT  NUMBER(10));

Prompt Column SOURCE_ID;
ALTER TABLE PSP_COMPANY_EVENT
 ADD (SOURCE_ID  VARCHAR2(4000 CHAR));
...
ALTER TABLE PSP_COMPANY_EVENT
 ...
 CHECK (EMAIL_STATUS IN('Pending', 'Sent', 'Ignore', 'Resend', 'SendFailed'));
...
 select 'finished DBUpgradeFrom_1.9.0.0_To_1.9.1.0.sql ' || to_char(sysdate, 'MM/DD/YYYY HH24:MI:SS') from dual