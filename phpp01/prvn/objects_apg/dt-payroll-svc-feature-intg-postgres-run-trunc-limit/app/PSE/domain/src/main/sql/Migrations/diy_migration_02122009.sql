-- UPDATED: 02.09.2009 EMR
--
-- this script identifieds the remaining DD companies on the AS400 that 
-- will be targeted to be migrated to PSP on Thursday February 12 2009. 
-- The following criteria will be met, as published by Andy Walker and
-- Use Case PSPDDMGU39: Conditions for Migration - DIY, authored by
-- Linda F.
--
--   a. Migrate any remaining terminated accounts - either Assisted, or DD only
--   b. Migrate the remainder of active DD companies who have submitted at least 1 payroll since 02.12.2008
--   b. DD account is active, and on hold and not MI or UP activation status
--   e. No limit on number of paychecks per payroll
--   f. No limit on payroll frequencies - eg weekly, monthy, etc.
--   g. Allow multi-EIN companies 
--   h. Allow pending payrolls
--
...
SPOOL diy_migration_02122009.log
...
-- 02.09.2009 EMR   
-- allow multi-EIN.  however run a query to see if any DD and Assisted multi-agreements.
...
-- 02.09.2009 EMR   
-- changed the date to 1 year prior
-- 12.07.2008 EMR   
...
CREATE TABLE Z_TEMP_IQACHSEL AS (
  SELECT ACH_USERID,
   ...
   WHERE RECENT_PR_DATE > 20080211
);


prompt Now loading pending payrolls or more than 100 paychecks in ACH table
-- 02.09.2009 EMR   
-- no limits pending payrolls or number of checks
...
-- 02.09.2009 EMR   
-- these copmanies will be fixed via a post migration script.
-- 12.01.2008 EMR
...
-- 02.09.2009 EMR   
-- this last migration essentially has no limits
...
BEGIN
   ...
   -- added migration of any additional terminated companies.
	 INSERT INTO DIYMIGADM.DIY_MIG_QUEUE 
	   SELECT CLI_USERID, 'R', 20090212, 0 
   ...
   DBMS_OUTPUT.PUT_LINE ('Now loading active companies into the AS400 migration queue.');
	 ...
	 FOR rec IN (
	   -- 02.09.2009 EMR
	   -- there are no limits for this final migration
	 ...
	 LOOP
	 	 ...
	 	 INSERT INTO DIYMIGADM.DIY_MIG_QUEUE values (rec.cli_userid, 'R', 20090212, 0);
...
-- 02.10.2009 EMR
-- confirm this index is loaded, should have been in prior migration
-- 12.07.2008 EMR
