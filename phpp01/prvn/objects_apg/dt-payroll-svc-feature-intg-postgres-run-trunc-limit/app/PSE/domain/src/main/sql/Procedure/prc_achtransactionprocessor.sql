CREATE OR REPLACE PROCEDURE prc_achtransactionprocessor (
   p_source_system_cd   IN       VARCHAR2,   
   p_user_id            IN       VARCHAR2,               -- For audit purposes
   p_processing_date    IN       TIMESTAMP,              -- UTC Date
   p_application_server_date IN  TIMESTAMP,
   p_number_of_records       OUT INT   
)
IS   
...
BEGIN      
      ...
      INSERT INTO psp_financial_trans_state
           ...
           (SELECT fn_format_sysguid (SYS_GUID ()), 0, p_user_id, p_application_server_date,
                 p_user_id, p_application_server_date, -1, TRUNC (v_psp_date) + .5, NULL,
           ...
           WHERE c.source_system_cd = p_source_system_cd
                 ...
                 and ft.settlement_date between (p_processing_date - 45) and p_processing_date
      ...
      p_number_of_records := SQL%ROWCOUNT;                   
END prc_achtransactionprocessor;                   
/
