...
BEGIN
 ...
 parent_select_stmt_str := 'SELECT table_name,constraint_name,constraint_type,r_constraint_name FROM user_constraints' ||
 		 				   		   ' WHERE constraint_name IN (SELECT r_constraint_name FROM user_constraints' ||
   					   		   			  	 ' WHERE table_name = ''' ||tablename||''')' ||
 											 'AND table_name NOT IN' ||
 											 '(''PSP_ACH_TRANSACTION_CODE'', ''PSP_ACTION_EVENT'', ''PSP_AGENCY'', ''PSP_AGENCY_CHECK_LIST_ITEM'', ''PSP_AGENCY_CHECK_LIST_STATUS'', ''PSP_AUTH_DOMAIN'', ''PSP_AUTH_OPERATION'', ''PSP_AUTH_ROLE'', ''PSP_AUTHROLE_OPERATION_ASSOC'', ''PSP_AUTH_USER'',' ||
											 '''PSP_BANK_HOLIDAY'', ''PSP_COLLECTION_STAGE'', ''PSP_AGENCY_STATUS'', ''PSP_DEPOSIT_FREQUENCY_CODE'', ''PSP_EVENT_TYPE'', ''PSP_FEE'', ''PSP_FINANCIAL_TXN_ACTION'', ''PSP_FORM_TEMPLATE'', ''PSP_FUNDING_MODEL'',' ||
											 '''PSP_GEMS_UPLOAD_BATCH'', ''PSP_GEMS_LEDGER_POSTING_RULE'', ''PSP_INTUIT_BANK_ACCOUNT'', ''PSP_INTUIT_BANK_ACC_TXN_TYPE'', ''PSP_INTUIT_BA_BT_FT'', ''PSP_LAW'', ''PSP_LEDGER_ACCOUNT'',' ||
											 '''PSP_LEDGER_ACCOUNT_ACTION'', ''PSP_MONEY_MOVEMENT_TRANSACTION'', ''PSP_NACHAFILE'', ''PSP_OFFER'', ''PSP_OFFERING'', ''PSP_OFFERING_SVCCHG'', ''PSP_OFFERING_SVCCHG_GRP'', ''PSP_OFFERING_SVC_ASSOC'', ''PSP_OFFER_SVCCHG_ASSOC'', ''PSP_OFFLOAD_BATCH'', ''PSP_OFFLOAD_GROUP'', ''PSP_PAYMENT_TEMPLATE'', ''PSP_PMT_TEMPLATE_FREQUENCY'', ''PSP_PAYROLL_FREQUENCY'', ''PSP_PAYROLL_RUN_ACTION'',' ||
											 '''PSP_POSTING_RULE'', ''PSP_REPORTING_AGENT'', ''PSP_SERVICE'', ''PSP_SERVICE_CHECK_LIST_ITEM'', ''PSP_SERVICE_CHECK_LIST_STATUS'', ''PSP_SERVICE_STATUS'', ''PSP_SVCSTAT_SRCSYS_ASSOC'', ''PSP_SVCSTAT_SVC_ASSOC'', ''PSP_SVCSTAT_SYSCAP_ASSOC'', ''PSP_SVCSTAT_TXNTYPE_ASSOC'', ''PSP_SYSTEM_CAPABILITY'',' ||
