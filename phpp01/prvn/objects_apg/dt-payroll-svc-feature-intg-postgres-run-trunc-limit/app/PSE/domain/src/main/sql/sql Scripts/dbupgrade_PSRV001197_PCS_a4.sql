-- FILE NAME    : dbupgrade_PSRV001197_PCS_a4.sql
...
SPOOL dbupgrade_PSRV001197_PCS_a4.log
...
BEGIN
  ...
  SELECT DISTINCT ui.INDEX_NAME 
   ...
   WHERE ui.INDEX_NAME   = uic.INDEX_NAME
     AND ui.TABLE_NAME   = 'PSP_PAYCHECK_SPLIT'
     AND ui.UNIQUENESS   = 'UNIQUE'
     AND uic.COLUMN_NAME = 'PAYCHECK_SPLIT_SEQ';

  -- primary key index
	DBMS_REDEFINITION.REGISTER_DEPENDENT_OBJECT (
    uname            => 'PSPADM',        
    orig_table       => 'PSP_PAYCHECK_SPLIT',
    int_table        => 'INT_PSP_PAYCHECK_SPLIT',
    dep_type         => DBMS_REDEFINITION.CONS_INDEX,
    dep_owner        => 'PSPADM',
    dep_orig_name    => v_src_pk_idx_id, 
    dep_int_name     => 'INT_XPKPAYCHECK_SPLIT'
  );	  
  ...
  SELECT DISTINCT uc.CONSTRAINT_NAME
   ...
   WHERE uc.CONSTRAINT_NAME = ucc.CONSTRAINT_NAME
     AND uc.TABLE_NAME      = 'PSP_PAYCHECK_SPLIT'
     AND uc.CONSTRAINT_TYPE = 'P'
     AND ucc.COLUMN_NAME    = 'PAYCHECK_SPLIT_SEQ';
