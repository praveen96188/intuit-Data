...
Spool pspadm_tab_row_cnt_size.sql
select 'set heading on' from dual;
select 'set feedback off' from dual;
select 'col TABLE for a50' from dual;
...
Select 'Select '''||segment_name||''' "TABLE", ''ROWS: ''||count(*) "ROWS", '' MB: '||bytes/1024/1024||''' "SIZE" from '||owner||'.'||segment_name||';', to_char(sysdate, 'YYYYMMDDHH24MISS') d, owner u
from dba_segments
where owner = 'PSPADM'
...
Spool &owner_._tab_row_cnt_size_&date_..log
@@pspadm_tab_row_cnt_size.sql
Spool off
