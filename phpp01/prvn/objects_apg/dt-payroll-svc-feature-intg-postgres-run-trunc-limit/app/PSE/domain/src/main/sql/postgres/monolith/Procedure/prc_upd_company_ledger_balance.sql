CREATE OR REPLACE PROCEDURE PRC_UPD_COMPANY_LEDGER_BALANCE
    ...
    financial_trans_state_cursor CURSOR FOR
        select distinct date_trunc('day', transaction_state_eff_date) as transaction_state_eff_date
    ...
    BEGIN
        ...
        FOR financial_trans_state_rec IN financial_trans_state_cursor
            LOOP
              INSERT /*+ APPEND */  INTO psp_ledger_balance  (ledger_balance_seq, VERSION, creator_id, created_date,
                                                                modified_date, realm_id, balance_amount, balance_date,
                                                                ledger_account_fk, company_fk,reporting_type)
                    (SELECT /*+ index(fts PSP_FINANCIALTRANSACTIONST_FK1) */
                            gen_random_uuid(),
                            1,'COMPANYLEDGERBALANCERECALCJOB', v_utc_date,
                                ...
                                ILB.COMPANY_FK=fts.company_fk and ilb.reporting_type=po.reporting_type
                                                                                                                                                          and date_trunc('day', ILB.BALANCE_DATE) = (select max(date_trunc('day', olb.balance_date)) from psp_ledger_balance olb where
                                    OLB.COMPANY_FK=ILB.COMPANY_FK and OLB.LEDGER_ACCOUNT_FK=ilb.ledger_account_fk and olb.reporting_type=ilb.reporting_type )), 0)+ SUM (
                                                                                                         (select ft.financial_transaction_amount from psp_financial_transaction ft where ft.financial_transaction_seq = fts.financial_transaction_fk)
                                                                                                         * (CASE  pr.credit_debit_ind
                                                                                                              ...
                                                                                                              WHEN 'D' THEN
                                                                                                                CASE WHEN la.balance_calculation_rule = 'DebitAddsToBalance'
                                                                                                                       THEN 1 ELSE -1 END
                                                                                                            END)
                                                                                                 ) amount, date_trunc ('day', fts.transaction_state_eff_date) newbal_date,
                     ...
                     WHERE pr.transaction_state_fk = fts.transaction_state_fk
                       ...
                       AND date_trunc ('day', fts.transaction_state_eff_date) = date_trunc('day', financial_trans_state_rec.transaction_state_eff_date)
                     ...
                     GROUP BY fts.company_fk, date_trunc('day', fts.transaction_state_eff_date),
