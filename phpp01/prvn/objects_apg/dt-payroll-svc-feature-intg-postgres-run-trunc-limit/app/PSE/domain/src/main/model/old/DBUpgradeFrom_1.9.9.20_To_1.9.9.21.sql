...
Prompt Column EVENT_GROUP_CD;
ALTER TABLE PSP_EVENT_TYPE
 ADD (EVENT_GROUP_CD  VARCHAR2(255 CHAR));
...
BEGIN
 ...
 parent_select_stmt_str := 'SELECT table_name,constraint_name,constraint_type,r_constraint_name FROM user_constraints' ||
		 				   		   ' WHERE constraint_name IN (SELECT r_constraint_name FROM user_constraints' ||
  					   		   			  	 ' WHERE table_name = ''' ||tablename||''')' ||
											 'AND table_name NOT IN' ||
											 '(''PSP_ACTION_EVENT'', ''PSP_BANK_HOLIDAY'', ''PSP_COLLECTION_STAGE'', ''PSP_DEPOSIT_FREQUENCY'', ''PSP_EVENT_TYPE'', ''PSP_FEE'', ''PSP_FINANCIAL_TXN_ACTION'', ''PSP_FUNDING_MODEL'',' ||
		...
		FOR k IN 1..parent_columns.count
 	 		LOOP
				...
				IF (parent_constraint_collection(j).table_name = 'PSP_BANK_ACCOUNT') THEN
                    select_stmt_str := select_stmt_str || ' AND ' || parent_columns(k) || ' NOT IN (SELECT BANK_ACCOUNT_FK FROM PSP_INTUIT_BANK_ACCOUNT)';
                END IF;
                --DBMS_OUTPUT.PUT_LINE(select_stmt_str);
...
Prompt Column NOTE_LAST_UPDATED_DATE;
ALTER TABLE PSP_COMPANY_EVENT
 ADD (NOTE_LAST_UPDATED_DATE  TIMESTAMP(6));

Prompt Column COMPANY_EVENT_FK;
ALTER TABLE PSP_COMPANY_NOTE
 ADD (COMPANY_EVENT_FK  VARCHAR2(255 CHAR) NULL);

Prompt Index PSP_COMPANYNOTE_FK1;
--
-- PSP_COMPANYNOTE_FK1  (Index)
--
CREATE INDEX PSP_COMPANYNOTE_FK1 ON PSP_COMPANY_NOTE
...
ALTER TABLE PSP_EVENT_TYPE
 ADD CONSTRAINT C_PSP_EVENT_TYPE1
...
ALTER TABLE PSP_COMPANY_NOTE
 ...
 PROMPT finishedDBUpgradeFrom_1.9.9.20_To_1.9.9.21.sql