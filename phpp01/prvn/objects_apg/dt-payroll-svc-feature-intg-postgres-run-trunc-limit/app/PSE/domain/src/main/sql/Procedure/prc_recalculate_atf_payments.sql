...
BEGIN
    ...
    IF p_payment_method in ('EFTPS', 'EFTPSDirectDebit', 'HPDE', 'HPDERefund', 'CheckPayment', 'ACHCredit', 'ACHDebit', 'EDI', 'SuperCheck') AND
        ...
        LOOP
            ...
            FOR ftRec IN ( select ft.settlement_date, FT.TRANSACTION_TYPE_FK, FT.FINANCIAL_TRANSACTION_AMOUNT, ft.current_transaction_state_fk
                                    ...
                                    where  ft.company_fk = p_company_seq
                                           ...
                                           and trunc(FT.SETTLEMENT_DATE) >= trunc(p_initiation_date))
            LOOP
            ...
            IF v_settlement_date IS NOT NULL THEN
              ...
              insert into PSP_ATFPAYMENTS_TO_PROCESS (atfpayments_to_process_seq, version, creator_id, created_date, modifier_id, modified_date,
                  ...
                  values (FN_FORMAT_SYSGUID(SYS_GUID()), 1, p_user_id, p_app_server_date, p_user_id, p_app_server_date, -1, v_settlement_date,
...
END prc_recalculate_atf_payments;
/
