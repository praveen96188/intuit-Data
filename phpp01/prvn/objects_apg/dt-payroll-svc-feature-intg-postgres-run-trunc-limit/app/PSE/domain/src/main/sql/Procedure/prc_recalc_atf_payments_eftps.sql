...
BEGIN
  PRC_SET_PSP_EVENT_LOG (
    ...
    select
      FN_FORMAT_SYSGUID(SYS_GUID()), 1, p_user_id, p_app_server_date, p_user_id, p_app_server_date,
      -1, settlement_date, quarter_end, law_fk, money_movement_transaction_seq, company_fk, amount
    from (
        select
          mmt.company_fk, mmt.money_movement_transaction_seq, FN_GET_LAST_DAY_OF_QUARTER(mmt.payment_period_end) as quarter_end, ft.law_fk, median(ft.settlement_date) as settlement_date,
          sum(
            case
...
END prc_recalc_atf_payments_eftps;
/
