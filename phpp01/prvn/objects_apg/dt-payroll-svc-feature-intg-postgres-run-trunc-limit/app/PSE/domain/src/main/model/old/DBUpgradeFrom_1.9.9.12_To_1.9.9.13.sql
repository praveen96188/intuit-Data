...
BEGIN
 ...
 FOR i IN 1..child_constraint_collection.count
    LOOP
    	DECLARE
			...
			SELECT ucc.COLUMN_NAME BULK COLLECT INTO columnNames FROM User_cons_columns ucc where ucc.constraint_name = child_constraint_collection(i).constraint_name
 ...
 parent_select_stmt_str := 'SELECT table_name,constraint_name,constraint_type,r_constraint_name FROM user_constraints' ||
		 				   		   ' WHERE constraint_name IN (SELECT r_constraint_name FROM user_constraints' ||
  					   		   			  	 ' WHERE table_name = ''' ||tablename||''')' ||
											 'AND table_name NOT IN' ||
											 '(''PSP_ACTION_EVENT'', ''PSP_BANK_HOLIDAY'', ''PSP_BANK_ACCOUNT'', ''PSP_COLLECTION_STAGE'', ''PSP_DEPOSIT_FREQUENCY'', ''PSP_EVENT_TYPE'', ''PSP_FEE'', ''PSP_FINANCIAL_TXN_ACTION'', ''PSP_FUNDING_MODEL'',' ||
											 '''PSP_GEMS_UPLOAD_BATCH'', ''PSP_GEMS_LEDGER_POSTING_RULE'', ''PSP_INTUIT_BANK_ACCOUNT'', ''PSP_INTUIT_BANK_ACC_TXN_TYPE'', ''PSP_INTUIT_BA_BT_FT'', ''PSP_LEDGER_ACCOUNT'',' ||
 ...
 FOR j IN 1..parent_constraint_collection.count
	...
	DECLARE
    	...
    	--identify the parent column names
		   ...
		   SELECT ucc.COLUMN_NAME BULK COLLECT INTO parent_columns FROM User_cons_columns ucc where ucc.constraint_name = parent_constraint_collection(j).constraint_name
		   AND ucc.COLUMN_NAME NOT IN ('REALM_ID');
...
Prompt Trigger TR_UPD_DD_LIMITS;
--
-- TR_UPD_DD_LIMITS  (Trigger)
--
...
DECLARE
  ...
  v_source_system_cd VARCHAR2(255);
BEGIN

IF :NEW.CLASS_NAME = 'DDCompanyServiceInfo' AND :NEW.PROPERTY_NAME='OverrideCompanyLimitAmount' THEN

  SELECT SOURCE_SYSTEM_CD INTO v_source_system_cd FROM PSP_COMPANY WHERE COMPANY_SEQ = :NEW.COMPANY_FK;

  SELECT PARAMETER_VALUE INTO v_default_company_limit FROM PSP_SOURCE_PAYROLL_PARAMETER
   WHERE PARAMETER_CD = 'DefaultDDCompanyLimit' and SOURCE_SYSTEM_CD = v_source_system_cd;
...
IF :NEW.CLASS_NAME = 'DDCompanyServiceInfo' AND :NEW.PROPERTY_NAME='OverrideEmployeeLimitAmount' THEN
  ...
  SELECT PARAMETER_VALUE INTO v_default_employee_limit FROM PSP_SOURCE_PAYROLL_PARAMETER
   WHERE PARAMETER_CD = 'DefaultDDEmployeeLimit' and SOURCE_SYSTEM_CD = v_source_system_cd;
		...
		LOOP
			...
			FORALL i IN event_id_table.first..event_id_table.last
				...
				VALUES (event_id_table(i), 0, p_user_id, v_utc_date, p_user_id, v_utc_date, -1,
				        v_psp_date, v_psp_date, 'Active', 'FeeOffloaded', event_token_tbl(i), v_psp_date, 0, 'Ignore', NULL, company_id_tbl(i));
		    FORALL i IN event_id_table.first..event_id_table.last
...
 PROMPT finishedDBUpgradeFrom_1.9.9.12_To_1.9.9.13.sql