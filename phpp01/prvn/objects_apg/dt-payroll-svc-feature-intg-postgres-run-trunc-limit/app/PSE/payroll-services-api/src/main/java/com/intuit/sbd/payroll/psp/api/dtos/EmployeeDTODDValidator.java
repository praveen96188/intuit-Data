 ...
 * Time: 10:29:36 PM
 */
public class EmployeeDTODDValidator extends EmployeeDTOValidator {
    public ProcessResult validate(EmployeeDTO employeeDTO) {
        ...
        if ((employeeDTO.getFirstName()== null || !(Validator.isValidLength(employeeDTO.getFirstName().trim(), 1, 80)))
            ...
            validationResult.getMessages().InvalidValue(EntityName.Employee, employeeDTO.getEmployeeId(), "FirstNameLastName");
        }

        if (!Validator.isValidLength(employeeDTO.getMiddleName(), 0, 80)) {
            validationResult.getMessages().InvalidValue(EntityName.Employee, employeeDTO.getEmployeeId(), "MiddleName");
        }

        if (employeeDTO.getSocialSecurityNumber() !=null &&
                !Validator.isMatchingPattern(employeeDTO.getSocialSecurityNumber(), "^[0-9]{9}$|^[0-9]{3}-[0-9]{2}-[0-9]{4}$")) {
            validationResult.getMessages().InvalidValue(EntityName.Employee, employeeDTO.getEmployeeId(), "SocialSecurityNumber");
        }

        if (!Validator.isValidLength(employeeDTO.getSuffix(), 0, 20)) {
            validationResult.getMessages().InvalidValue(EntityName.Employee, employeeDTO.getEmployeeId(), "Suffix");
        }
        ...
        if (employeeDTO.getFedAllowances() < 0) {
            validationResult.getMessages()
                    .InvalidValue(EntityName.Employee, employeeDTO.getEmployeeId(), "Federal Allowances");
        }
