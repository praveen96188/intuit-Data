...
//AN public class BigDecimal extends java.lang.Number implements java.io.Serializable,java.lang.Comparable{'
/**
 * Class used internally for SpcfDecimal 
 */
class BigDecimal extends java.lang.Number implements java.io.Serializable,java.lang.Comparable{
  ...
  * @stable ICU 2.0
  */
 public static final BigDecimal ZERO=new BigDecimal((long)0); // use long as we want the int constructor
  ...
  * @stable ICU 2.0
  */
 public static final BigDecimal ONE=new BigDecimal((long)1); // use long as we want the int constructor
 // .. to be able to use this, for speed
  ...
  * @stable ICU 2.0
  */
 public static final int ROUND_CEILING=MathContext.ROUND_CEILING;
  ...
  * @stable ICU 2.0
  */
 public static final int ROUND_DOWN=MathContext.ROUND_DOWN;
  ...
  * @stable ICU 2.0
  */
 public static final int ROUND_FLOOR=MathContext.ROUND_FLOOR;
  ...
  * @stable ICU 2.0
  */
 public static final int ROUND_HALF_DOWN=MathContext.ROUND_HALF_DOWN;
  ...
  * @stable ICU 2.0
  */
 public static final int ROUND_HALF_EVEN=MathContext.ROUND_HALF_EVEN;
  ...
  * @stable ICU 2.0
  */
 public static final int ROUND_HALF_UP=MathContext.ROUND_HALF_UP;
  ...
  * @stable ICU 2.0
  */
 public static final int ROUND_UNNECESSARY=MathContext.ROUND_UNNECESSARY;
  ...
  * @stable ICU 2.0
  */
 public static final int ROUND_UP=MathContext.ROUND_UP;
 ...
 private static final MathContext plainMC=new MathContext(0,MathContext.PLAIN); // context for plain unlimited math
  ...
  * @serial
  */
 private byte form=(byte)MathContext.PLAIN; // assumed PLAIN
 ...
 public BigDecimal(char inchars[],int offset,int length){super();
  ...
  if (mant[0]==0) 
   ...
   { // non-zero
    ...
    if (hadexp) 
     {
      form=(byte)MathContext.SCIENTIFIC;
  ...
  */
  ...
  {/*setform:*/do{/*select*/
  if (exdigits==(-1))
   num.form=(byte)MathContext.PLAIN;
 ...
 public BigDecimal setScale(int scale,int round){
  ...
  res.form=(byte)MathContext.PLAIN; // by definition
  return res;
  }
 ...
 public java.math.BigInteger toBigInteger(){
  ...
  byte newmant[]=null;
  {/*select*/
  if ((exp>=0)&(form==MathContext.PLAIN))
   res=this; // can layout simply
  else if (exp>=0)
   {
    res=clone(this); // safe copy
    res.form=(byte)MathContext.PLAIN; // .. and request PLAIN
   }
  else{
   { // exp<0; scale to be truncated
    ...
    if (((int)-this.exp)>=this.mant.length) 
    ...
    else 
     {
      ...
      res.mant=newmant;
      res.form=(byte)MathContext.PLAIN;
      res.exp=0;
     }
   }
  }
 ...
 private char[] layout(){
  ...
  if (form!=MathContext.PLAIN) 
   {/* exponential notation needed */
    ...
    //AN rec=new char[sb.length()];
    //Utility.getChars(sb, 0,sb.length(),rec,0);
    rec = sb.toString().toCharArray();
    return rec;
   }
 ...
 private BigDecimal finish(MathContext set,boolean strip){
  ...
  form=(byte)MathContext.PLAIN; // preset
  ...
  //AN
  BigDecimal(int exp, byte ind, byte[] mant)
  {
  	this.exp = exp;
  	this.ind = ind;
  	this.mant = mant;
  	this.form = MathContext.PLAIN;
  }
 }
