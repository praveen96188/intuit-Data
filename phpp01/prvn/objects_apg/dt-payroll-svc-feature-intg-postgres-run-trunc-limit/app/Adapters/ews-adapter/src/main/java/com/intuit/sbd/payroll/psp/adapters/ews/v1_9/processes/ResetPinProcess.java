...
import com.intuit.sbd.payroll.psp.adapters.ews.v1_9.dtos.psp.EwsResetPin;
import com.intuit.sbd.payroll.psp.adapters.ews.v1_9.dtos.psp.EwsResetPinResponse;
 ...
 * @author Jeff Jones
 */
public class ResetPinProcess extends BaseProcess {
    ...
    public ResetPinProcess(EwsResetPin pRequest) {
        mRequest = pRequest;
        ...
        logger.info("Processing Reset_Pin Request / PSID: " + this.mPSID);
    }

    @Override
    public EwsResetPinResponse execute() {
        EwsResetPinResponse response = null;

        try {
            PayrollServices.beginUnitOfWork(FlushMode.MANUAL);

            mPSPTransmission.setInitializeDateTime(PSPDate.getPSPTime());
            mPSPTransmission.setTransmissionType(TransmissionType.ResetPIN);
            ...
            response = new EwsResetPinResponse();
    ...
    @Override
    protected EwsResetPinResponse process() throws Exception {


        ProcessResult<HashMap<String,String>> resetPinPR = PayrollServices.subscriptionManager.resetCompanyPIN
                (SourceSystemCode.QBDT, mPSID, mRequest.getPinSignature(), mRequest.getUserName());
        if (!resetPinPR.isSuccess()) {
            //Commit psp validation logic
            PayrollServices.commitUnitOfWork();
            PayrollServices.beginUnitOfWork(FlushMode.MANUAL);
            MessageList messageList = resetPinPR.getMessages();
        ...
        return EwsFactory.createEwsResetPinResponse(mPspCompany, resetPinPR.getResult());
    }
}
