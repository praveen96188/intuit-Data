...
import com.intuit.sbd.payroll.psp.adapters.ews.v1_10.utils.IJaxBManager;
import com.intuit.sbd.payroll.psp.adapters.ews.v1_10.utils.JaxBFactory;
...
public class QueryPayrollStatusProcess {
    ...
    public static final String NON_1099_BILLPAYMENT_REJECTION_MESSAGE = "Only 1099 vendors can be paid with this service. Remove the non-1099 vendor payments and resend the payment transaction. For additional explanation, instructions, and options,see http://payroll.com/DD4V.";
    ...
    private PayrollStatusWSDTO createPayrollStatusWSDTO(Company pCompany) {
        ...
        payrollStatusWSDTO.setNon1099LatestFailedTransmissionsWSDTO(createNon1099LatestFailedTransmissionsWSDTO(pCompany, roleId));
    ...
    /**
     * This method will get latest failed transmission of  DD4V/Bill payments. If latest DD4V is success then this will return empty list.
     *
     * @param pCompany
     * @param pRoleId
     * @return
     */
    private List<FailedTransmissionWSDTO> createNon1099LatestFailedTransmissionsWSDTO(Company pCompany, UserRoleEnum pRoleId) {
        List<FailedTransmissionWSDTO> failedTransmissions = new ArrayList<FailedTransmissionWSDTO>();
        DomainEntitySet<SourceSystemTransmission> billPaymentSourceSystemTransmissions = PspFactory.getLatestSourceSystemTransmissionWithBillPaymentSecondary(pCompany);
        if (CollectionUtils.isEmpty(billPaymentSourceSystemTransmissions)) {
            return failedTransmissions;
        }

        SourceSystemTransmission sourceSystemTransmissionHolder = billPaymentSourceSystemTransmissions.getFirst();

        if (!doesRoleMatchTransmissionType(pRoleId, sourceSystemTransmissionHolder)) {
            return failedTransmissions;
        }

        FailedTransmissionWSDTO failedTransmissionWSDTO = new FailedTransmissionWSDTO();
        failedTransmissionWSDTO.setDate(formatDate(sourceSystemTransmissionHolder.getInitializeDateTime()));
        failedTransmissionWSDTO.setTime(formatTime(sourceSystemTransmissionHolder.getInitializeDateTime()));

        DomainEntitySet<CompanyEventDetail> companyEventDetails = CompanyEvent.findCompanyEventDetails
                (pCompany, EventTypeCode.TransmissionError, EventDetailTypeCode.TransmissionId,
                        sourceSystemTransmissionHolder.getTransmissionIdentifier());
        if (!NON_1099_BILLPAYMENT_REJECTION_MESSAGE.equals(PspFactory.getFailedTransmissionMessage(companyEventDetails))) {
            return failedTransmissions;
        }
        failedTransmissionWSDTO.setDescription(PspFactory.getFailedTransmissionMessage(companyEventDetails));
        failedTransmissions.add(failedTransmissionWSDTO);
