...
import com.intuit.sbd.payroll.psp.domain.EntitlementPaymentMethodType;
...
import com.intuit.sbd.payroll.psp.domain.NumberOfEmployeesType;
import com.intuit.sbd.payroll.psp.domain.OfferingCode;
import com.intuit.sbd.payroll.psp.domain.OrderSourceCode;
...
public class PspFactory {
    ...
    public static Collection<Company> findCompanyByEinAndAuthId(String pEIN, String pAuthId) throws Exception {
        ...
        if(Company.isDGDeleteFeatureEnabled()){
            contactCriterion = contactCriterion.And(Contact.Company().IsDgDisassociated().equalTo(Boolean.FALSE));
        }
    ...
    public static CompanyDTO updateCompanyDTO(EwsCompany pEwsCompany, Company pCompany) throws Exception {
        ...
        if(pEwsCompany.getIndustry() != null){
            CompanyAdditionalInfoDTO companyAdditionalInfoDTO = createCompanyAdditionalInfoDTO(pEwsCompany);
    ...
    public static CompanyAdditionalInfoDTO createCompanyAdditionalInfoDTO(EwsCompany pEwsCompany){
        CompanyAdditionalInfoDTO companyAdditionalInfoDTO = new CompanyAdditionalInfoDTO();
        companyAdditionalInfoDTO.setIndustry(pEwsCompany.getIndustry());
    ...
    public static ContactDTO createContactDTO(EwsContact pEwsContact, ContactRole pContactRole) {
        ...
        if(pEwsContact.getSocialSecurityNumber() != null) {
            contactDTO.setSocialSecurityNumber(pEwsContact.getSocialSecurityNumber());
        }
        if(pEwsContact.getDateOfBirth() != null){
            Date birthDate = pEwsContact.getDateOfBirth().getTime();
            contactDTO.setDateOfBirth(new DateDTO(birthDate));
        }
    ...
    public static ContactDTO updateContactDTO(EwsContact pEwsContact, ContactDTO pContactDTO, ContactRole pContactRole) {
        ...
        if(pEwsContact.getSocialSecurityNumber() != null) {
            pContactDTO.setSocialSecurityNumber(pEwsContact.getSocialSecurityNumber());
        }
        if(pEwsContact.getDateOfBirth() != null){
            Date birthDate = pEwsContact.getDateOfBirth().getTime();
            pContactDTO.setDateOfBirth(new DateDTO(birthDate));
        }
    ...
    public static QuickbooksInfoDTO createQuickbooksInfoDTO(EwsQuickBooks pEwsQuickBooks) {
        ...
        //new column to store the actual SKU of quickbooks
        ...
        if(isWalletEnabled){
            try {
                ...
                PaymentProfile paymentProfile = paymentProfileGateway.getPaymentProfileDetailsFromEOCLIC(
                        pEwsEntitlement.getEntitlementOfferingCode(),
                        pEwsEntitlement.getLicenseNumber(),
                        pEwsEntitlement.getBillingAccountId());
                    ...
                    if (Objects.nonNull(paymentProfile.getPaymentMethodType())
                        ...
                        if (paymentMethod.isEFTPaymentMethod()) {
                            ...
                            if (Objects.nonNull(paymentMethod.getCCPaymentMethod())) {
                                entitlementUnitDTO.setCreditCardType(
                                        EwsCreditCardType.mapPaymentProfileCardType(ccPaymentMethod.getCardBrand()).toString());
                                String ccNo = ccPaymentMethod.getTokenizedAccountNumber();
                                entitlementUnitDTO.setCreditCardNumber(ccNo.length() > 4 ? ccNo.substring(ccNo.length()-4): ccNo);
                                entitlementUnitDTO.setCreditCardExpiration(ccPaymentMethod.getExpirationMonth()
    ...
    /**
     * Get Latest Bill payment transmission for given company
     * @param pCompany
     * @return
     */
    public static DomainEntitySet<SourceSystemTransmission> getLatestSourceSystemTransmissionWithBillPaymentSecondary(Company pCompany) {
        if (pCompany == null) {
            return null;
        }
        if (SourceSystemTransmission.isBillPaymentServiceEnabled() && CompanyService.findCompanyService(pCompany, ServiceCode.BillPayment) == null) {
            logger.info("CompanyService not found for CompanyId=" + pCompany.getId().toString() + ",psid=" + pCompany.getSourceSystemCompanyId());
            return null;
        }
        return SourceSystemTransmission.getSSTByCompanyAndTransmissionType(pCompany, TransmissionType.WSBillPaySendPaymentsToPayees);
    }
}
