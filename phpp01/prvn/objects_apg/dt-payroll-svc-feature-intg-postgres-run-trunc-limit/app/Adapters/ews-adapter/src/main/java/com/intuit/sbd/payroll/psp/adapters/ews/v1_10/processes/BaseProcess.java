...
import com.intuit.sbd.payroll.psp.adapters.ews.v1_10.utils.DebugUtil;
...
public abstract class BaseProcess {
    ...
    protected String mRealmID;
    protected boolean mAddEin;
    protected CompanyBankAccount mCompanyBankAccount;
    protected String mSubscriptionNumber;
    protected RequestSourceIdentifier requestSourceIdentifier = PayrollApplicationBeanFactory.getBean(RequestSourceIdentifier.class);
    ...
    protected CompanyService addService(ServiceInfoDTO pServiceInfoDTO) throws Exception {
        ...
        if (!servicePR.isSuccess()) {
            ...
            //Handling message for VMPGrantFailure
            if (messageList.get(0).getMessageCode().equals("10129")) {
                throw new EwsException(EwsMessages.VMPGrantAdditionFailed());
            }
    ...
    protected CompanyBankAccount addCompanyBankAccount(CompanyBankAccountDTO pCompanyBankAccountDTO, Boolean pOverrideRandomDebits) throws Exception {
        return addCompanyBankAccount(pCompanyBankAccountDTO, pOverrideRandomDebits, true);
    }

    protected CompanyBankAccount addCompanyBankAccount(CompanyBankAccountDTO pCompanyBankAccountDTO, Boolean pOverrideRandomDebits, Boolean pIsPSPRandomDollarVerificationRequired) throws Exception {
        ProcessResult<CompanyBankAccount> companyBankAccountPR = PayrollServices.companyManager.addCompanyBankAccount(mPspCompany.getSourceSystemCd(),
                ...
                true,
                true, pIsPSPRandomDollarVerificationRequired);
    ...
    protected TaxCompanyServiceInfo addAssistedService(EwsCompany pEwsCompany,
        ...
        return addAssistedService(pEwsCompany, pEwsBaseServices, pEntitlementUnits, pForceRandomDollar, true);
    }

    //Method is getting called when company is activated from QB
    protected TaxCompanyServiceInfo addAssistedService(EwsCompany pEwsCompany,
                                                       EwsBaseServices pEwsBaseServices,
                                                       DomainEntitySet<EntitlementUnit> pEntitlementUnits,
                                                       Boolean pForceRandomDollar, boolean pIsPSPRandomDollarVerificationRequired) throws Exception {
        ...
        if (companyBankAccount == null) {
            ...
            mCompanyBankAccount = addCompanyBankAccount(companyBankAccountDTO, pForceRandomDollar, pIsPSPRandomDollarVerificationRequired);
        }
    ...
    protected CompanyService addDirectDepositService(EwsBaseServices pEwsBaseServices,
                                                     DomainEntitySet<EntitlementUnit> pEntitlementUnits,
                                                     boolean pForceRandomDollar) throws Exception {
        return addDirectDepositService(pEwsBaseServices, pEntitlementUnits, pForceRandomDollar, true);
    }
    ...
    protected void setLoggerContext(){
        if (Objects.nonNull(mPspCompany)) {
            MoneyMovementLogHelper.setLoggerContext(mPspCompany);
        } else if (Objects.nonNull(mPSID)){
            MoneyMovementLogHelper.setLoggerContext(mPSID,null,null,null);
        }
    }

    protected void removeLoggerContext(){
        MoneyMovementLogHelper.removeLoggerContext();
    }

    protected void moneyMovementTrackingLog(SpcfLogger pLogger, MoneyMovementLogHelper.EventType eventType, String customeMessage) {
        moneyMovementTrackingLog(pLogger, eventType,customeMessage,null);
    }

    protected void moneyMovementTrackingLog(SpcfLogger pLogger, MoneyMovementLogHelper.EventType eventType, String customeMessage, String errorMessage) {
        if (Objects.nonNull(mPspCompany) && (mPspCompany.isMoneyMovementOnboardingEnabled() || requestSourceIdentifier.isPayrollPlugin())) {
            MoneyMovementLogHelper.logMoneyMovementEventMessage(pLogger, eventType, customeMessage, errorMessage);
        }
    }
