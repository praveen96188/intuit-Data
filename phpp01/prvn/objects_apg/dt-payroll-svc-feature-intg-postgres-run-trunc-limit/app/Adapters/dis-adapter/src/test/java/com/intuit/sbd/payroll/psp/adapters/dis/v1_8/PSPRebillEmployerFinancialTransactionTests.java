...
public class PSPRebillEmployerFinancialTransactionTests {
    ...
    public FinancialTransaction getHappyPathFinancialTransactionToRebill() throws Throwable {
        try {
            ...
            //we are returning direct deposit fee instead of monthly fee FT to keep consistency in test cases. So that everytime
            //ft linked to billing detail having total amount 2.98, tax = 0.08, unitPrice=1.45 and qty 2 is returned
            for (FinancialTransaction ft : erDebitFnTxns) {
                if (ft.getBillingDetail().getOfferingServiceChargeType().equals(OfferingServiceChargeType.DirectDepositFee)) {
                    return ft;
                }
            }
    ...
    private RebillEmployerFinancialTransactionRequestDISDTO getRebillRequest(FinancialTransaction pFinancialTransactionToRebill) throws Throwable {
        ...
        RequestAttributesUtils.setAttribute("CorpId",sapUser.getCorpId());
