...
import com.intuit.sbd.payroll.psp.adapters.dis.v1_8.disdtos.requests.GetAgencyRulesRequestDISDTO;
import com.intuit.sbd.payroll.psp.adapters.dis.v1_8.disdtos.responses.GetAgencyRulesResponseDISDTO;
                        ...
                        if (laws.size() == 1) {
                            ...
                            if (!pspLaw.isFUTA()) {
                                try {
                                    ...
                                    logger.error(law.getLawID() + " not found in PSP Get Agency Rules for AS400.");
                                }
                            }
                            try {
                                String cepLawId = SourceSystemLawAssoc.findSourceIdBySourceSystemAndLaw(SourceSystemCode.ADE,pspLaw);
                                agencyRulesLawDISDTO.setCepLawId(cepLawId);
                            } catch (Exception e) {
                                // If law id not found, log error but do not stop rest of agency rules from loading.
                                //Commented fro PSP-4549
                               // logger.error(law.getLawID() + " not found in PSP Get Agency Rules for CEP.");
                            }
