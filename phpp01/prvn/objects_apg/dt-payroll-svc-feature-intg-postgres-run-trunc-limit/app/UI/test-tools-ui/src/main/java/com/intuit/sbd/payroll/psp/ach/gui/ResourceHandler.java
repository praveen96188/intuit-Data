...
import com.intuit.sbd.payroll.psp.ach.util.XmlResourceHandler;
import com.intuit.sbd.payroll.psp.ach.util.ResourceListener;
...
public class ResourceHandler extends XmlResourceHandler
{
						 ...
						 Attributes atts) throws SAXException
	{
		if (XML_COMPONENT.equals (qName)) {
			...
			invokeSetMethod (m_stack.peek (), atts.getValue (XML_NAME),
				new Object[] { createArg (atts) });
		} else if (XML_ENTRY.equals (qName)) {
			try {
			Helper.invoke (m_stack.peek (), "put",
				new Object[] { atts.getValue (XML_KEY), atts.getValue (XML_VALUE) });
			} catch (Exception ex) {
			}
		}
		m_lastElement = qName;
	...
	public void characters(char[] ch, int start, int length)
	{
		if (XML_DESCRIPTION.equals (m_lastElement)) {
			...
			invokeSetMethod (m_stack.peek (), "Description",
