/*****************************************************************
 ...
 ActiveWidgets 2.0.1 (demo version)
    ...
    if (AW.safari) {
        ...
        function getRectangle(e) {
            ...
            function getPos(el) {
                ...
                var pp = getPos(p);
                return{x:el.offsetLeft + pp.x,y:el.offsetTop + pp.y}
            }
            var pp = getPos(e);
            return{left:pp.x,right:pp.x + e.offsetWidth,top:pp.y,bottom:pp.y + e.offsetHeight}
        }
    }
})();
(function() {
    if (AW.opera) {
        ...
        function getRectangle(e) {
            ...
            var pp = getPos(e);
            return{left:pp.x,right:pp.x + parseInt(getComputedStyle(e, null).width),top:pp.y,bottom:pp.y + parseInt(getComputedStyle(e, null).height)}
        }
    }
})();
...
AW.System.HTML.create = function() {
    ...
    function hte(i) {
        return function() {
            return "Error";//"AW Error:<a href=\"http:\/\/www.activewidgets.com/error." + i + "/\">" + errors[i] + "</a>"
        }
    }
    if (AW.opera) {
            ...
            for (i in targets) {
                if (!t[i]) {
                    ...
                    if (e && e.getAttribute("aw")) {
                        ...
                        } else if (e.getAttribute("aw") != "control") {
...
AW.UI.Controllers.Changes = (function() {
    ...
    function selection(mode) {
        switch (mode) {case "single":this.setController("selection", AW.UI.Controllers.Single);break;case "multi":this.setController("selection", AW.UI.Controllers.Multi);break}
    }
...
AW.Grid.Controllers.Navigation = (function() {
    ...
    return{onSelectedColumnsChanging:sync1,onSelectedColumnsChanged:sync2,onSelectedRowsChanging:sync1,onSelectedRowsChanged:sync2,onRowSelectedChanged:syncRows,onSelectionModeChanged:function(mode) {
        switch (mode) {case "none":this.setController("selection", {});this.setCurrentSelection("none");break;case "single-cell":this.setController("selection", AW.Grid.Controllers.SingleCell);this.setCurrentSelection("cell");break;case "single-row":this.setController("selection", AW.Grid.Controllers.SingleRow);this.setCurrentSelection("row");break;case "multi-row":this.setController("selection", AW.Grid.Controllers.MultiRow);this.setCurrentSelection("row");break;case "multi-row-marker":this.setController("selection", AW.Grid.Controllers.MultiRowMarker);this.setCurrentSelection("row");var checkbox = new AW.Templates.CheckedItem;this.setCellTemplate(checkbox, 0);break}
    }}
})();
...
AW.Grid.Controllers.Overflow = {onScrollLeftChanged:function(x) {
    ...
    /* Dawn & Wiktor hacked it
    this.setTimeout(function() {
        var e = this.getContent("box/block").element();
        if (e && !document.title.match(/ivewid/i) && (new Date).getMilliseconds() < 300) {
            AW.setOuterHTML(e, "s<ap ntsly=ed\"silpyab:olkcp;sotioi:nbaosuletw;diht8:%0h;iehg:t02xpl;fe:t01;%obttmo2:p5;xobdrre1:xps lodib;cagkornu:df#ffc;loro#:0f;0ettxa-ilngc:neet\"rA>tcviWediegst2 0.- E avultaoi neVsroi.n/<psna>".replace(/(.)(.)/g, "$2$1"))
        }
    });
    */
...
AW.Grid.Controllers.Grid = {onRowsTemplateChanged:function(rows) {
    ...
    content.mapTemplate("panel", function(i) {
        switch (i) {case 0:return this.getHeadersTemplate();case 1:return this.getRowsTemplate();case 2:return this.getFootersTemplate()}
    })
...
AW.Grid.Controllers.Extended = {onContentTemplateChanged:function(content) {
    ...
    content.mapTemplate("panel", function(i, j) {
        switch (i) {case 0:return this.$owner.getTopTemplate(j);case 1:return this.$owner.getRowsTemplate(j);case 2:return this.$owner.getBottomTemplate(j)}
    })
...
AW.Grid.Extended.create = function() {
    ...
    var splitColumns = function(p, j) {
        ...
        var i = this.$1;
        switch (p) {case "count":if (i === 0) {
            return left
        }if (i == 1) {
            return this.$owner.getColumnProperty("count") - left - right
        }if (i == 2) {
            return right
        }return 0;case "offset":if (i === 0) {
            return 0
        }if (i == 1) {
            return left
        }if (i == 2) {
            return this.$owner.getColumnProperty("count") - right
        }return 0;default:return this.$owner.getColumnProperty(p, j)}
    };
