...
set serveroutput on
spool upd_comp_data_14Oct.log
DECLARE
  RECORD_COUNT      INTEGER := 0;
  MAX_RECORDS       INTEGER := 4000000;
  PROCESSED_RECORDS INTEGER := 0;
  MAX_FETCH_RECORDS INTEGER := 25000;
BEGIN
  SELECT count(*) INTO RECORD_COUNT FROM PSPADM.PSP_COMPANY WHERE PUBLISH_STATUS is not null AND PUBLISH_STATUS not like '0%';
  ...
  WHILE RECORD_COUNT > 0 AND PROCESSED_RECORDS < MAX_RECORDS
    LOOP
      ...
      SET PUBLISH_STATUS = ('0' || substr(PUBLISH_STATUS, 2))
      WHERE company_seq IN (SELECT company_seq
                            FROM PSPADM.PSP_COMPANY
                            WHERE PUBLISH_STATUS is not null AND PUBLISH_STATUS not like '0%' AND ROWNUM <= MAX_FETCH_RECORDS);
      COMMIT;
