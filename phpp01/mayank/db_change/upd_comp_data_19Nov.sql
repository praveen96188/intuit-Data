...
BEGIN
  SELECT count(*) INTO RECORD_COUNT FROM PSPADM.PSP_COMPANY WHERE PUBLISH_STATUS is not null AND PUBLISH_STATUS not like '_00000%';
  ...
  WHILE RECORD_COUNT > 0 AND PROCESSED_RECORDS < MAX_RECORDS
    LOOP
      ...
      SET PUBLISH_STATUS = (substr(PUBLISH_STATUS, 1, 1) || '00000' || substr(PUBLISH_STATUS, 7))
      WHERE company_seq IN (SELECT company_seq
                            FROM PSPADM.PSP_COMPANY
                            WHERE PUBLISH_STATUS is not null AND PUBLISH_STATUS not like '_00000%' AND ROWNUM <= MAX_FETCH_RECORDS);
      COMMIT;
