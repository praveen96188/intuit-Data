...
set serveroutput on
spool upd_comp_data_07Dec.log

DECLARE
  RECORD_COUNT      INTEGER := 0;
  MAX_RECORDS       INTEGER := 7000000;
  PROCESSED_RECORDS INTEGER := 0;
  MAX_FETCH_RECORDS INTEGER := 25000;
BEGIN
  SELECT count(*) INTO RECORD_COUNT FROM PSPADM.PSP_COMPANY WHERE PUBLISH_STATUS is not null AND PUBLISH_STATUS not like '_0%';
