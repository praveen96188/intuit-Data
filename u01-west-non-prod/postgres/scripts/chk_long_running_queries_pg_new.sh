...
#                       the_subject="Long running query (> 5 Mins) - Individual User"
        ...
        ./run_postgres_instance.sh ${instance_name1} $db_name chk_long_running_queries_pg.sql "-v dbname=${db_name}" > ${logdir}/${instance_name1}_${db_name}_chk_long_running_queries.log
        if [ `cat ${logdir}/${instance_name1}_${db_name}_chk_long_running_queries.log |grep -i "0 rows" |wc -l` -ne 1 ]; then
            echo "good"
            # Read the log file and extract usename and query_duration_secs values
            awk '
