...
if [ "${VPC_ENV}" == "ppd" ]; then
  topic_arn="arn:aws:sns:${AWS_DEFAULT_REGION}:152430470825:DB-sbg-psp-critical-${VPC_ENV}-a-intuit-com"
...
the_subject="Blocking Queries > 10min"

if [ `cat ${logdir}/${db_name}_chk_blocking_queries_pg_10minWait.log |grep -i "0 rows" |wc -l` -ne 1 ]; then
/usr/bin/aws sns publish --topic-arn ${topic_arn} --subject " $cluster_postgres - ${the_subject}" --message "`cat ${logdir}/${db_name}_chk_blocking_queries_pg_10minWait.log`"
fi
