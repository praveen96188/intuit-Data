...
function config_gg
{
  replicat_obey=`ls $GG_HOME/diroby/create_r*oby |grep $repliact_char`
...
function upload_wallet
{
  ...
  stash put-secret --api-key-grant-policy-id ${IDPS_POLICY_ID} --vkm ${IDPS_ENDPOINT} --secret-name secrets/goldengate/${APP_ENV}/${AWS_REGION}/${CLUSTER_NAME}/dircrd/cwallet.sso --secret-value /dev/shm/goldengate/${CLUSTER_NAME}/dircrd/cwallet.sso -overwrite
  stash put-secret --api-key-grant-policy-id ${IDPS_POLICY_ID} --vkm ${IDPS_ENDPOINT} --secret-name secrets/goldengate/${APP_ENV}/${AWS_REGION}/${CLUSTER_NAME}/dirwlt/cwallet.sso --secret-value /dev/shm/goldengate/${CLUSTER_NAME}/dirwlt/cwallet.sso -overwrite
  stash put-secret --api-key-grant-policy-id ${IDPS_POLICY_ID} --vkm ${IDPS_ENDPOINT} --secret-name secrets/goldengate/${APP_ENV}/${AWS_REGION}/db/password_file --secret-value /dev/shm/goldengate/db/password_file -overwrite
}
