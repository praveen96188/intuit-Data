...
set serveroutput on
spool company_event_detail_06_Dec_Remaining

DECLARE

  RECORD_COUNT       INTEGER   := 0;
  MAX_RECORDS        INTEGER   := 1100000;
  PROCESSED_RECORDS  INTEGER   := 0;
  ERROR_RECORDS      INTEGER   := 0;
  MAX_FETCH_RECORDS  INTEGER   := 100;
  BATCH_COUNT        INTEGER   := 0;
  START_DATE         DATE := TO_DATE('2011-07-22', 'yyyy-mm-dd');
  END_DATE           DATE := TO_DATE('2011-09-30', 'yyyy-mm-dd');
      ...
      WHILE (BATCH_COUNT > 0 AND PROCESSED_RECORDS < MAX_RECORDS)
        ...
        LOOP
          BEGIN
            ...
            when matched then
              update set ced.COMPANY_FK = ce.COMPANY_FK, ced.modified_date=sys_extract_utc(systimestamp);
