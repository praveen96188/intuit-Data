...
def fetch_latest_snapshot(rds_client, db_cluster_identifier):
    snapshots = rds_client.describe_db_cluster_snapshots(
        DBClusterIdentifier=db_cluster_identifier, MaxRecords=20
    )
...
def fetch_db_cluster_info(rds_client, db_cluster_identifier):
    ...
    db_cluster_info["ClusterParameterGroup"] = db_cluster_info.get(
        "DBClusterParameterGroup", ""
    )
    writer_instance_id = next(
        (
            m["DBInstanceIdentifier"]
            for m in db_cluster_info["DBClusterMembers"]
    ...
    if writer_instance_id:
        ...
        db_cluster_info["InstanceParameterGroup"] = db_instance["DBParameterGroups"][0][
            "DBParameterGroupName"
        ]
    else:
        db_cluster_info["InstanceParameterGroup"] = None
    print(
        f"Fetched cluster parameter group: {db_cluster_info['ClusterParameterGroup']}"
    )
    print(
        f"Fetched instance parameter group: {db_cluster_info['InstanceParameterGroup']}"
    )
...
def get_instance_class_from_existing_cluster(rds_client, existing_stage_cluster_info):
    writer_instance = next(
        (
            inst
            for inst in existing_stage_cluster_info["DBClusterMembers"]
    ...
    try:
        ...
        db_instance_class = instance_details["DBInstances"][0].get(
...
def create_staging_db_from_snapshot(
    rds_client,
    snapshot_identifier,
    ...
    new_db_cluster_identifier = (
        f"{stage_cluster_identifier.lower().replace('_', '-')}-new".strip("-")
    )
    print(f"Creating new staging DB cluster from snapshot: {snapshot_identifier}")
    try:
        instance_class = get_instance_class_from_existing_cluster(
            rds_client, existing_stage_cluster_info
        )
        ...
        rds_client.restore_db_cluster_from_snapshot(
            ...
            VpcSecurityGroupIds=[
                ...
                for sg in existing_stage_cluster_info["VpcSecurityGroups"]
            ],
            ...
            EnableIAMDatabaseAuthentication=existing_stage_cluster_info[
                "IAMDatabaseAuthenticationEnabled"
            ],
            EngineMode=existing_stage_cluster_info["EngineMode"],
            DBClusterParameterGroupName=existing_stage_cluster_info[
        ...
        rds_client.create_db_instance(
            ...
            Tags=[
                tag
                for tag in existing_stage_cluster_info.get("TagList", [])
...
def kill_active_connections(db_host, db_name, master_user, db_password):
    try:
...
def run_psql_password(
    db_host, db_name, master_user, target_user, old_password, new_password
):
    try:
        child = pexpect.spawn(
            "/usr/bin/psql",
            ["-h", db_host, "-U", master_user, "-d", db_name],
            timeout=15,
        )
...
def drop_replication_slots(db_host, master_user, db_password):
    try:
        conn = psycopg2.connect(
            host=db_host,
            database="postgres",
...
def rename_database(db_host, master_user, db_password, old_name, new_name):
    try:
        conn = psycopg2.connect(
            host=db_host,
            database="postgres",
...
def execute_sql_file(db_host, db_name, master_user, db_password, sql_file):
    try:
        cmd = ["psql", "-h", db_host, "-U", master_user, "-d", db_name, "-f", sql_file]
        env = os.environ.copy()
...
def manage_psparc_owner(db_host, db_name, master_user, db_password, new_password):
    try:
        conn = psycopg2.connect(
            host=db_host,
            database=db_name,
        ...
        conn.autocommit = True
        cur = conn.cursor()
        cur.execute("SELECT 1 FROM pg_roles WHERE rolname='psparc_owner';")
        if cur.fetchone():
            print("User psparc_owner exists. Resetting password...")
        else:
            print("Creating user psparc_owner...")
            cur.execute("CREATE USER psparc_owner WITH LOGIN;")

        # Grant CONNECT privilege
        cur.execute(f"GRANT CONNECT ON DATABASE {db_name} TO psparc_owner;")
        print(f"Granted CONNECT on database {db_name} to psparc_owner.")

        cur.close()
        conn.close()

        run_psql_password(
            db_host, db_name, master_user, "psparc_owner", db_password, new_password
        )
    except Exception as e:
        print(f"Error managing psparc_owner user: {e}")
...
def manage_app_users(db_host, db_name, admin_user, admin_password, user_password_file):
    ...
    for user, password in user_passwords.items():
        try:
            ...
            run_psql_password(
                db_host, db_name, admin_user, user, admin_password, password
            )
...
def main(prod_cluster_identifier, stage_cluster_identifier, db_name, new_db_name):
    ...
    if new_db_name == "pitparmo":
        ...
        vacuum_script_path = (
            "/u01/scripts/staging_refresh/vacuum/vacuum_pitparmo-new.sh"
        )
        ...
        DB_refresh = "Monolith Database "

    elif new_db_name == "psppdarc":
    ...
    print(f"password_file_path: '{password_file_path}':")
    print(f"new_password_file_path: '{new_password_file_path}':")
    print(f"appusers_file_path: '{appusers_file_path}':")
    print(f"sql_file: '{sql_file}':")
    print(f"pitparmo_sql_file: {pitparmo_sql_file}")
    print(f"Spspadm_sql_file: {pspadm_sql_file}")
    print(f"Port: {port}")
    print(f"def_db_name: {def_db_name}")
    print(f"master_user: {master_user}")
    print(f"schema_owner: {schema_owner}")


if new_db_name in ["pitparmo", "psppdarc", "psparcdb"]:
    print(f"DB Refresh started for : {DB_refresh}")
    ...
    pspadm_password = new_passwords.get(
        schema_owner, intuadmin_password
    ...
    try:
        ...
        existing_stage_cluster_info = fetch_db_cluster_info(
            rds_client, stage_cluster_identifier
        )
        new_db_cluster_identifier = (
            f"{stage_cluster_identifier.lower().replace('_', '-')}-new".strip("-")
        )
        ...
        if current_password and intuadmin_password:
            try:
                ...
                drop_replication_slots(db_host, master_user, intuadmin_password)
                kill_active_connections(
                    db_host, db_name, master_user, intuadmin_password
                )
                rename_database(
                    db_host,
                    master_user,
        ...
        if rename_success and database_exists(
            db_host, master_user, intuadmin_password, new_db_name
        ):
            execute_sql_file(
                db_host, def_db_name, master_user, intuadmin_password, sql_file
            )
            manage_psparc_owner(
                db_host, def_db_name, master_user, intuadmin_password, pspadm_password
            )
        else:
            print("Skipping SQL execution and user management due to earlier errors.")

            # STEP 2: Execute psparc_owner SQL file
            print(
                f"Checking if user 'psparc_owner' can connect to database '{new_db_name}'..."
            )
        if database_exists(db_host, "psparc_owner", pspadm_password, new_db_name):
            ...
            try:
                ...
                print(f"Error executing SQL file as psparc_owner: {e}")
        else:
            print("Connection failed for user 'psparc_owner'. Skipping SQL execution.")

        # STEP 3: Manage app users
        print("\n=== STEP 3: Managing app users ===")
        manage_app_users(
            db_host, new_db_name, master_user, intuadmin_password, appusers_file_path
        )

        # STEP 4: Execute pitparmo_postgres.sql
        print("\n=== STEP 4: Executing psparcdb_postgres.sql ===")
        try:
            execute_sql_file(
                db_host, new_db_name, master_user, intuadmin_password, pitparmo_sql_file
            )
        except Exception as e:
            print(f"Error executing psparcdb_postgres.sql: {e}")
        ...
        if new_db_name == "psparcdb":
            ...
            print("\n Port number Update not required for monolith and archival  ")
    ...
    print("\n Not a valid database name {new_db_name} ")

if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Refresh RDS staging cluster from snapshot."
    )
    parser.add_argument(
        "--prod-cluster", required=True, help="Production cluster identifier"
    )
    parser.add_argument(
        "--stage-cluster", required=True, help="Staging cluster identifier"
    )
