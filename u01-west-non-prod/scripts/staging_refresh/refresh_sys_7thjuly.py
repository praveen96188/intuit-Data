...
def manage_pspadm_owner(db_host, db_name, master_user, db_password, new_password):
    try:
        conn = psycopg2.connect(host=db_host, database=db_name, user=master_user, password=db_password, connect_timeout=10)
        conn.autocommit = True
        cur = conn.cursor()
        ...
        if cur.fetchone():
            print("User pspadm_owner exists. Resetting password...")
        else:
            print("Creating user pspadm_owner...")
            cur.execute("CREATE USER pspadm_owner WITH LOGIN;")
        
        # Grant CONNECT privilege
        cur.execute(f"GRANT CONNECT ON DATABASE {db_name} TO pspadm_owner;")
        print(f"Granted CONNECT on database {db_name} to pspadm_owner.")

        cur.close()
        conn.close()

        run_psql_password(db_host, db_name, master_user, 'pspadm_owner', db_password, new_password)
    except Exception as e:
...
# Main function
def main(prod_cluster_identifier, stage_cluster_identifier, db_name, new_db_name, master_user, schema_owner ):
    rds_client = boto3.client('rds', region_name='us-west-2')

    password_file_path = '/u01/scripts/staging_refresh/.op'
    new_password_file_path = '/u01/scripts/staging_refresh/.np'
    appusers_file_path = '/u01/scripts/staging_refresh/.ap'
    sql_file = '/u01/scripts/staging_refresh/postgres_postgres.sql'
    pitparmo_sql_file = '/u01/scripts/staging_refresh/pitparmo_postgres.sql'
...
##    analyze_script_path = '/u01/scripts/staging_refresh/vacuum/analyze/ppsp-ppd-new.sh'
    pspadm_sql_file = '/u01/scripts/staging_refresh/pspadmowner_pitparmo.sql'
...
#        new_db_cluster_identifier = f"{stage_cluster_identifier.lower().replace('_', '-')}-new".strip('-')
        ...
        if rename_success and database_exists(db_host, master_user, intuadmin_password, new_db_name):
            ...
            manage_pspadm_owner(db_host, def_db_name, master_user, intuadmin_password, pspadm_password)
        else:
            print("Skipping SQL execution and user management due to earlier errors.")

        # STEP 2: Execute pspadm_owner SQL file
            print(f"Checking if user 'pspadm_owner' can connect to database '{new_db_name}'...")
        if database_exists(db_host, 'pspadm_owner', pspadm_password, new_db_name):
            print(f"Connection successful. Executing SQL file: {pspadm_sql_file}")
            try:
                execute_sql_file(db_host, new_db_name, 'pspadm_owner', pspadm_password, pspadm_sql_file)
            except Exception as e:
                print(f"Error executing SQL file as pspadm_owner: {e}")
        else:
            print("Connection failed for user 'pspadm_owner'. Skipping SQL execution.")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--master_user', required=True, help='Master User Name')
    parser.add_argument('--schema_owner', required=True, help='Schema Owner')
    args = parser.parse_args()

    main(args.prod_cluster, args.stage_cluster, args.db_name, args.new_db_name, args.master_user, args.schema_owner)
