...
--migration-type cdc --table-mappings file://${source}-${target}-pspadm-${task_identifier}-table-mappings.json \
--replication-task-settings file://${source}-${target}-pspadm-rep-settings.json 1> /dev/null
sleep 5

aws dms describe-replication-tasks --region us-west-2 --filter=Name=migration-type,Values=cdc |grep ReplicationTaskIdentifier|awk {'print $2'}|sed 's/\"//g; s/,//'|grep ^$source > ${source}-${target}-pspadm-${task_identifier}_repl_tasks.txt
tot_new_cdc_tasks_created=$(cat ${source}-${target}-pspadm-${task_identifier}_repl_tasks.txt|wc -l)
echo ""
