...
--
    SELECT ARRAY_AGG(row (mmt.money_movement_transaction_seq, mmt.company_fk)::mmt_record)
    INTO mmt_list
    ...
    IF p_file_type = 'Tax' THEN
        ...
        where (mmt.company_fk, mmt.money_movement_transaction_seq) = (mmt_tmp.mmt_company_fk, mmt_tmp.mmt_seq);
    ELSE
        /*+ IndexScan(mmt PSP_MONEY_MOVEMENT_TRANSAC_FK3)*/
        UPDATE psp_money_movement_transaction mmt
        SET status        = 'Executed',
