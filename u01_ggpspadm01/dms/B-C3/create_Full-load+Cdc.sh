. /l/orcl
# Usage: ./create_B_to_C3_tasks.sh <Source_DBName> <Target_DBName> <task_identifier> <replication_instance_arn>
...
# create new  full-load-and-cdc B-C3 replication tasks
...
--region us-west-2 --replication-task-identifier ${source}-${target}-pspadm-${task_identifier} \
...
--migration-type full-load-and-cdc --table-mappings file://${source}-${target}-pspadm-${task_identifier}-table-mappings.json \
--replication-task-settings file://${source}-${target}-pspadm-rep-settings.json 1> /dev/null
sleep 5

aws dms describe-replication-tasks --region us-west-2 --filter=Name=migration-type,Values=full-load-and-cdc |grep ReplicationTaskIdentifier|awk {'print $2'}|sed 's/\"//g; s/,//'|grep ^$source > ${source}-${target}-pspadm-${task_identifier}_repl_tasks.txt
tot_new_full-load-and-cdc_tasks_created=$(cat ${source}-${target}-pspadm-${task_identifier}_repl_tasks.txt|wc -l)
echo ""
echo -e "\n Created $tot_new_full-load-and-cdc_tasks_created new full-load-and-cdc replication tasks as below. Please verify"
cat ${source}-${target}-pspadm-${task_identifier}_repl_tasks.txt
