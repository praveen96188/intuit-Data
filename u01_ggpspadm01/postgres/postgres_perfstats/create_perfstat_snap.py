...
def lambda_handler(event, context):   
    ...
    sql_get_instance_role="select CASE"  + '\n' \
                          "         WHEN 'MASTER_SESSION_ID' = session_id THEN 'writer'" + '\n' \
                          "         ELSE 'reader'" + '\n'  \
                          "       END AS instance_role" + '\n' \
                          "from aurora_replica_status() a, aurora_db_instance_identifier() b"  + '\n' \
                          "where a.server_id = b.db_instance_identifier;"

    instance_role = PostgresDataManager.run_query(sql_get_instance_role, DB_CONNECTION)
    if instance_role[0][0] == 'reader':
        print('current instance is a reader. Do nothing')
        exit()
    else:
        print('current instance is writer. Continue')
