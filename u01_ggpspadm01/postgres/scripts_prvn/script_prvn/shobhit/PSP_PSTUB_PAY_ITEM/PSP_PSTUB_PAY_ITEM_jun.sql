...
DECLARE
  ...
  MAX_RECORDS        INTEGER   := 500000000; -- 5000000
  PROCESSED_RECORDS  INTEGER   := 0;
  ERROR_RECORDS      INTEGER   := 0;
  MAX_FETCH_RECORDS  INTEGER   := 5000;
  BATCH_COUNT        INTEGER   := 0;
  START_DATE         DATE := TO_DATE('2022-05-31', 'yyyy-mm-dd');
  END_DATE           DATE := TO_DATE('2022-06-02', 'yyyy-mm-dd');
      ...
      WHILE (BATCH_COUNT > 0 AND PROCESSED_RECORDS < MAX_RECORDS)
        ...
        LOOP
          BEGIN
            ...
            WHERE T1.table_name ='PSP_PSTUB_PAY_ITEM_JUN';
            COMMIT;
