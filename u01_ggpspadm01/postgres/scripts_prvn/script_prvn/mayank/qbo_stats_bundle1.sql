...
exec DBMS_STATS.SET_TABLE_PREFS('QBO_DATA', 'TXHEADERS_1', 'METHOD_OPT', 'for all columns size 1 for all indexed columns size auto for columns size 1 tx_date for columns size 10 AUDIT_ID BILL_ID BOOKKEEPING_ACTIVITY_ID DD_CHECK_ID DEPT_ID DISCOUNT_ID FILED_TAX_RETURN_ID INVOICE_ID RECUR_DATA_ID SHIPPED_FROM_ADDRESS_ID SHIPPING_ADDRESS_ID SOURCE_REIMB_TXN_ID STATEMENT_ID TAX_AGENCY_ID');

exec DBMS_STATS.SET_TABLE_PREFS ('QBO_DATA', 'TXDETAILS_1', 'METHOD_OPT', 'for all columns size 1 for all indexed columns size auto for columns size 1 tx_date for columns size 10 AUDIT_ID DEPOSIT_ID DEPT_ID DISCOUNT_ID EMPLOYEE_ID INVOICE_ID KLASS_ID OFX_TXN_ID OTHER_KLASS_ID PITEM_ID REIMB_TXN_ID SOURCE_TXN_ID STATEMENT_ID TAX_CODE_ID TAX_RATE_ID WITHHOLDING_RATE_ID WORK_JURISDICTION_ID');
...
exec QBO_STATS_PKG.SET_DATE_RANGE('QBO_DATA', 'TXHEADERS_1', 'TX_DATE', to_date('2011-01-01', 'yyyy-mm-dd'), trunc(sysdate+30));
exec QBO_STATS_PKG.SET_DATE_RANGE('QBO_DATA', 'TXDETAILS_1', 'TX_DATE', to_date('2011-01-01', 'yyyy-mm-dd'), trunc(sysdate+30));
