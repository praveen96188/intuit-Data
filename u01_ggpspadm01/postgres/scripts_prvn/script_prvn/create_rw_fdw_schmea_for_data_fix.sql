\o create_rw_fdw_schmea_for_data_fix.log
...
select 'CREATE SCHEMA imp_rw_' || :'oracle_db' || '_ops_rw_user'; \gexec

select 'IMPORT FOREIGN SCHEMA "OPS_RW_USER" FROM SERVER ora_rw_sv_' || :'oracle_db' ||  ' INTO imp_rw_' || :'oracle_db' || '_ops_rw_user'; \gexec

select 'select * from information_schema.foreign_tables where  foreign_table_schema = ''imp_rw_' || :'oracle_db' || '_ops_rw_user'' order by 3';  \gexec

\o
\! cat create_rw_fdw_schmea_for_data_fix.log
