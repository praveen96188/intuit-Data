...
aws cloudwatch put-metric-data --metric-name ${db_name}_pgcatalog_size --namespace "PostgresCatalogSize" --value $chk_pgcatalog_size --timestamp `date +\%Y-%m-%dT\%H:%M:%S%z`
aws cloudwatch put-metric-data --metric-name ${db_name}_info_schema_size --namespace "PostgresCatalogSize" --value $chk_info_schema_size --timestamp `date +\%Y-%m-%dT\%H:%M:%S%z`
