...
DECLARE
    ...
    MAX_RECORDS        INTEGER   := 470000000;
    PROCESSED_RECORDS  INTEGER   := 320001692;
    ERROR_RECORDS      INTEGER   := 0;
    MAX_FETCH_RECORDS  INTEGER   := 5000;
    BATCH_COUNT        INTEGER   := 0;
    START_DATE         DATE := TO_DATE('2013-08-01', 'yyyy-mm-dd');
    END_DATE           DATE := TO_DATE('2015-05-01', 'yyyy-mm-dd');
            ...
            WHILE (BATCH_COUNT > 0 AND PROCESSED_RECORDS < MAX_RECORDS)
                ...
                LOOP
                    BEGIN
                        ...
                        MERGE INTO PSPADM.PSP_PSTUB_DDITEM ppio
                        using (SELECT ppi.PSTUB_DDITEM_SEQ, p.COMPANY_FK
                               FROM PSPADM.PSP_PSTUB_DDITEM ppi,
                        ...
                        ON (ppio.PSTUB_DDITEM_SEQ = ppii.PSTUB_DDITEM_SEQ)
                        when matched then
                        ...
                        WHERE T1.table_name ='PSP_PSTUB_DDITEM';
                        COMMIT;
