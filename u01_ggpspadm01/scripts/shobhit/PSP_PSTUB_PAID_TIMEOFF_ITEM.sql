...
DECLARE
    ...
    MAX_RECORDS        INTEGER   := 800000000;
    PROCESSED_RECORDS  INTEGER   := 0;
    ERROR_RECORDS      INTEGER   := 0;
    MAX_FETCH_RECORDS  INTEGER   := 5000;
    BATCH_COUNT        INTEGER   := 0;
    START_DATE         DATE := TO_DATE('2013-07-01', 'yyyy-mm-dd');
    END_DATE           DATE := TO_DATE('2023-03-05', 'yyyy-mm-dd');
            ...
            WHILE (BATCH_COUNT > 0 AND PROCESSED_RECORDS < MAX_RECORDS)
                ...
                LOOP
                    BEGIN
                        ...
                        MERGE INTO PSPADM.PSP_PSTUB_PAID_TIMEOFF_ITEM ppio
                        using (SELECT ppi.PSTUB_PAID_TIMEOFF_ITEM_SEQ, p.COMPANY_FK
                               FROM PSPADM.PSP_PSTUB_PAID_TIMEOFF_ITEM ppi,
                        ...
                        ON (ppio.PSTUB_PAID_TIMEOFF_ITEM_SEQ = ppii.PSTUB_PAID_TIMEOFF_ITEM_SEQ)
                        when matched then
                        ...
                        WHERE T1.table_name ='PSP_PSTUB_PAID_TIMEOFF_ITEM';
                        COMMIT;
