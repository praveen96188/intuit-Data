...
create temp table cte_get_balance_pre_load_1 as
    (select plb.balance_amount,
     ...
     from (select balance_amount, balance_date, ledger_account_fk, company_fk from pspadm.PSP_LEDGER_BALANCE) plb
...
select count(1) from cte_get_balance_pre_load_1;
...
select count(1) from cte_get_balance_pre_load;
...
select count(1) from cte_get_balance;
...
select count(1) from CTE_GET_FIRST_ROW_PREV_BALANCE;
...
select count(1) from CTE_GET_FIRST_ROW_CURRENT_BALANCE;
...
select count(1) from sq_fact_psp_ledger_balance;
