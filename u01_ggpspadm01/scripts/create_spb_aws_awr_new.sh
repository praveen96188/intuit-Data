#!/bin/bash
# Usage /ops/bin/db/create_spb_aws_awr_new.sh $SQL_ID $SQL_PLAN_HASH_VLAUE $SOURCE_CLUSTER $TARGET_CLUSTER
if [ $# -ne 6 ]; then
  echo "Usage: create_spb_aws_awr_new.sh <sql id> <plan hash> <source cluster> <target cluster> <begin awr snap> <end awr snap>" 
  ...
  echo "Example: create_spb_aws_awr_new.sh fstn4vqmgvtb5 2792059021 C92 C92-UW2 123 124" 
  exit 2
fi
...
# env file
source /l/orcl
sqlplus -s sbg_vdba/"U4HiW#f1bQ)x"@psphpp02 @create_spb_aws_awr_new.sql $SQL_ID $SQL_PLAN_HASH_VLAUE $SOURCE_CLUSTER $TARGET_CLUSTER $log_file $BEGIN_SNAP $END_SNAP
