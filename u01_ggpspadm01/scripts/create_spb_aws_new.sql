...
BEGIN
  ...
  execute immediate 'BEGIN sbg_vdba.qbo_spm.export_plan@'||v_source_cluster_dbl||'(:sqlid); END;' using in out v_sqlid;  
  dbms_output.put_line('SQL plan export completed!');
  ...
  IF v_source_cluster_dbl=v_target_cluster_dbl THEN
    ...
    dbms_output.put_line('SQL plan import completed!');
  ELSE
