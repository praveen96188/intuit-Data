CREATE OR REPLACE FUNCTION FN_TR_UPD_DD_LIMITS()
    RETURNS TRIGGER
    AS $$
DECLARE
v_default_company_limit VARCHAR(255);
  v_default_employee_limit VARCHAR(255);
BEGIN
...
CREATE TRIGGER TR_UPD_DD_LIMITS
    BEFORE INSERT OR UPDATE ON PSP_PROPERTY_AUDIT
                         FOR EACH ROW EXECUTE FUNCTION FN_TR_UPD_DD_LIMITS();