...
for instance_name in `aws rds describe-db-clusters --region us-east-2 --db-cluster-identifier ${cluster_postgres} --query 'DBClusters[0].DBClusterMembers' | grep DBInstanceIdentifier | awk '{print $2}'|sed 's/"//'|sed 's/"//'`
do
    ...
    aws cloudwatch put-metric-data  --metric-name ${instance_name} --namespace "DatabaseActiveConnection" --value $chk_active_connection --timestamp `date +\%Y-%m-%dT\%H:%M:%S%z` --region us-east-2
done
